...
then exposed repeated immutable-object
reads plus a controller restart loop. The controller is stopped. Those concrete corrections and
deadline-safe completion on restart are the critical path; no completed work should be regenerated.
...
| Pilot — recovery read efficiency | Repeated recovery/resource checks share bounded verified immutable content; mutable refs, authenticated history, PR/base, lease and physical observations remain fresh. | Code, Testing | Recovery resources / running | Repeated-read cause identified → exact-OID cache with bounded memory, defensive copies and fresh-authority regressions. |
| Pilot — quota-safe controller recovery | Primary reset/secondary retry boundaries cover bootstrap, discovery, admission and lease retirement; errors preserve durable work and do not cause restart storms or stale-lease reuse. | Code, Testing | Controller runtime / running | Captured primary-403 release/bootstrap failure → controller-wide abortable cooldown and focused failure/restart regressions. |
...
| Recovery immutable-read efficiency | Running | Reuse bounded immutable content across proof calls without caching authority or physical absence. |
| Controller quota recovery | Running | Prevent captured release/bootstrap errors from bypassing retry deadlines or causing service restart storms. |
| Deadline-safe closure | Running | Assess completion ordering after quota-delayed restart; no timeout or spending increase. |
