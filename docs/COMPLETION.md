...
Updated: 2026-09-08. [GitHub #69](https://github.com/clockgrove/factory/issues/69) owns
...
| --- | --- | --- | --- |
| Objective-scoped authority — [#219](https://github.com/clockgrove/factory/issues/219) | Code / controller, runtime and integration owners | Captured Objective epochs, atomic shared capacity and destination-branch integration claims replace repository-wide mutation fencing. Focused acceptance covers independent sessions, same-Objective exclusion, stale-holder rejection, base revalidation, crash isolation, capacity enforcement and per-operation fence measurements. [Locking audit and targeted evidence](OBJECTIVE-AUTHORITY.md). Next: #218 uses this production model; #215 still owns the coordinated gate/install. | Code complete; live qualification pending |
...
| Durable sessions — [#123](https://github.com/clockgrove/factory/issues/123) | Testing / retained session evidence | [Read-only reconciliation](https://github.com/clockgrove/factory/issues/123#issuecomment-5577997134) passed the durable-session criterion on original source `4628331`, proving session/artifact continuity, exact merges, reported accounting and scoped resource absence without replacement model work. The original incomplete qualifier result remains preserved; this is not evidence for a newer candidate or the other #123 criteria. | Criterion complete |
| Scheduling and priority — [#76](https://github.com/clockgrove/factory/issues/76), [#77](https://github.com/clockgrove/factory/issues/77), [#123](https://github.com/clockgrove/factory/issues/123) | Code then testing / throughput and scheduling owners | [#218](https://github.com/clockgrove/factory/issues/218) is implemented on the merged #219 Objective-authority model: unchanged observations use incremental wake hints, acceptance stays fresh and complete, ordinary throughput is isolated from expiry faults, refill proof is lane-symmetric, requested model policy is explicit, and local progress wakes preserve retry bounds. Main already contains exact non-managed timeline skipping with mixed/ambiguous fallbacks. Next: review and integrate one capability PR, then #215 owns the full gate/install and live qualification. Corrected code does not relabel historical evidence or prove comparative value. Pressure, real lease contention and Priority edits remain required. | Code ready for review; live qualification pending |
...
- [#220](https://github.com/clockgrove/factory/issues/220): bounded retention of released
  shared-capacity identities remains a long-lived maintenance requirement. The journal's
  hard bound fails closed and does not discard active or uncertain resource claims.
