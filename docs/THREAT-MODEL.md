...
| Package or dependency compromise changes install behavior | Lockfile and production audit, no install lifecycle scripts, clean-install verification, official asset checksums and durable provenance. Greenfield pnpm and Bun reject partial authority and exotic sources, require exact tool/dependency pins and registry integrity, scrub package-manager configuration, fetch only from declared registries with scripts disabled, and copy rather than hard-link from verified stores. uv additionally requires exact uv/CPython pins and wheel-backed locked dependencies while disabling source builds, editable installs, alternate indexes, automatic Python downloads and ambient Python/Pip configuration. Runtime archives are path-, type-, size-, digest-, executable- and tree-checked; Bun ZIP parsing does not trust an ambient extractor. Other missing package/language recipes have no future-authority adapter and fail before execution. |
