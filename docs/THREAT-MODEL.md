...
| A worker changes forbidden or sensitive paths | Exact-SHA isolated worktree; manifest-first collection; allowed-path and sensitive-surface checks; fresh-checkout validation before publication. Managed npm, pnpm, Bun, and uv greenfield adapters are dependency-root-only and structurally bind exact manifests, locks, workspace/task closure and finite operations. Workflow additions must also satisfy a parsed, credential-free protected-push-only policy before publication. The exact immutable Git diff is checked again at integration, and every execution-affecting surface stops at human merge. |
...
| Package or dependency compromise changes install behavior | Lockfile and production audit, no install lifecycle scripts, clean-install verification, official asset checksums and durable provenance. Greenfield npm, pnpm, and Bun reject partial authority and exotic sources, require exact tool/dependency pins and registry integrity, scrub package-manager configuration, fetch only from declared registries with scripts disabled, and copy rather than hard-link from verified stores. npm additionally binds the embedded CLI to its exact managed Node interpreter and rejects workspace globs, custom registries, and native/install builds. uv requires exact uv/CPython pins and wheel-backed locked dependencies while disabling source builds, editable installs, alternate indexes, automatic Python downloads and ambient Python/Pip configuration. Runtime archives are path-, type-, size-, digest-, executable- and tree-checked; Bun ZIP parsing does not trust an ambient extractor. Other missing package/language recipes have no future-authority adapter and fail before execution. |
