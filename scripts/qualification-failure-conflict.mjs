...
export function failureFixture(namespace, scenario) {
  ...
  const paths = {
    payload: `${prefix}/value.txt`,
    recipe: `${prefix}/recipe.mjs`,
    test: `${prefix}/value.test.mjs`,
  };
  ...
  return {
    protocol: "clockgrove.factory/failure-conflict-fixture-v1",
    namespace,
    scenario,
    paths,
    files,
...
export function failureObjectiveBody(namespace, scenario) {
  ...
  return (
    `# Explicit installed ${scenario} qualification\n\n${qualificationNamespaceMarker(namespace)}\n\n` +
...
export function rawFailureGit(repository, args, input, extraEnv = {}) {
  const env = Object.fromEntries(
    Object.entries(process.env).filter(
      ([name]) => !name.startsWith("GIT_") && name !== "NODE_OPTIONS",
    ),
  );
  Object.assign(env, {
  ...
  const result = spawnSync("git", [...gitOptions, "-C", repository, ...args], {
    env,
    input,
    encoding: "utf8",
    timeout: 15000,
...
export function assertFailureFixture(repository, baseSha, fixture) {
  ...
  for (const [path, content] of Object.entries(fixture.files)) {
    ...
    assert.equal(
      line,
      `100644 blob ${failureBlob(content)}\t${path}`,
      "committed fixture bytes/mode differ",
    );
  }
  const packageBytes = git(repository, ["cat-file", "blob", `${baseSha}:package.json`]);
  assert.equal(
    JSON.parse(packageBytes).scripts?.test,
    "node --test",
    "fixture requires the existing grounded bare Node test entrypoint",
  );
  return {
    baseSha,
    baseTreeSha: git(repository, ["rev-parse", `${baseSha}^{tree}`]),
    fixtureDigest: failureHash(JSON.stringify(fixture)),
  };
}
...
export function proveFailureContent({ repository, baseSha, artifact, fixture }) {
  ...
  assert.equal(
    artifact.payload,
    undefined,
    "small fixture cannot use a display-only payload marker",
  );
  ...
  try {
    ...
    const env = {
      ...
      GIT_INDEX_FILE: join(root, "private-index"),
    ...
    assert.equal(
      git(
        root,
        ["diff-tree", "--no-commit-id", "--name-only", "-r", baseline.baseTreeSha, workerTree],
        undefined,
        env,
      ),
    ...
    assert.equal(
      git(root, ["ls-tree", workerTree, "--", fixture.paths.payload], undefined, env),
      `100644 blob ${failureBlob(fixture.output)}\t${fixture.paths.payload}`,
    );
    const result = {
      ...baseline,
      artifactDigest: artifact.digest,
      patchDigest: failureHash(artifact.patch),
      path: fixture.paths.payload,
      workerTree,
      baseBlob: failureBlob(fixture.files[fixture.paths.payload]),
      workerBlob: failureBlob(fixture.output),
    };
    if (fixture.scenario === "failed-validation") return result;
    const workerCommit = git(
      root,
      ["commit-tree", workerTree, "-p", baseSha],
      "Actual retained worker tree\n",
      env,
    );
    git(root, ["read-tree", baseSha], undefined, env);
    ...
    git(
      root,
      ["update-index", "--add", "--cacheinfo", `100644,${competingBlob},${fixture.paths.payload}`],
      undefined,
      env,
    );
    const competingTree = git(root, ["write-tree"], undefined, env);
    const competingCommit = git(
      root,
      ["commit-tree", competingTree, "-p", baseSha],
      "Competing qualification tree\n",
      env,
    );
    const merge = rawFailureGit(
      root,
      ["merge-tree", "--write-tree", "--messages", workerCommit, competingCommit],
      undefined,
      env,
    );
    assert.equal(merge.status, 1, "actual retained worker output does not conflict");
    assert.ok(
      merge.stdout.includes(`CONFLICT (content): Merge conflict in ${fixture.paths.payload}`),
      "expected same-file content conflict unavailable",
    );
    ...
    return {
      ...
      conflict: {
        exitCode: 1,
        output: merge.stdout,
        outputDigest: failureHash(merge.stdout),
        boundary: "raw Git three-way content conflict; no filesystem application",
      },
    };
  } finally {
...
export function failureEvents(observation, authority) {
  ...
  const reserved = one(
    ...
    "one original attempt, no replacement",
  );
  const started = one(
    run.filter((event) => event.event === "AttemptStarted"),
    "actual worker execution required",
  );
  ...
  for (const key of [
    ...
    assert.equal(started[key], reserved[key]);
  assert.equal(reserved.policyDigest, start.policyDigest);
  assert.equal(reserved.baseSha, authority.failure.baseSha);
  assert.equal(
    one(observation.children, "exactly one compiled Work Item required").number,
    reserved.workItem,
  );
  const compiled = one(
    run.filter((event) => event.event === "GraphCompiled"),
    "one original compiled graph required",
  );
  const projected = one(
    run.filter((event) => event.event === "GraphProjected"),
    "one original projection required",
  );
  assert.equal(compiled.graphSize, 1);
  assert.equal(projected.graphSize, 1);
  ...
  assert.ok(
    ...
    "negative case reached publication/integration",
  );
...
export function assertFailureAccounting(run) {
  ...
  for (const event of actual) {
    ...
    assert.notEqual(
      ...
      "complete measured accounting required",
    );
    assert.ok(!keys.has(identity(event)), "duplicate actual accounting identity");
    keys.add(identity(event));
  }
  for (const reserved of run.filter(
    (event) => event.event === "BudgetReserved" && !event.modelInvocationId,
  ))
    one(
      actual.filter((event) => identity(event) === identity(reserved)),
      "unreconciled native budget",
    );
  assert.ok(
    model.usage.some((event) => event.phase === "management") &&
      model.usage.some((event) => event.phase === "execution"),
  );
  assert.ok(
    actual.some((event) => event.unit === "local_milliseconds" && event.phase === "execution"),
  );
...
export function assertFailedValidation(observation, authority, fixture) {
  ...
  const validation = one(
    run.filter((event) => event.event === "ValidationRecorded"),
    "actual failed validation receipt required",
  );
  assert.equal(validation.passed, false);
  for (const key of ["workItem", "attempt", "baseSha"])
    assert.equal(validation[key], reserved[key]);
  ...
  const failed = one(
    run.filter((event) => event.event === "AttemptFailed"),
    "failed validation attempt required",
  );
  assert.ok(failed.sequence > validation.sequence);
  const expected = `validation failed (1): ${fixture.validationCommand}`;
  assert.ok(
    failed.reason === expected || failed.reason?.startsWith(`${expected}\nOutput tail:\n`),
    "attempt failed at a different boundary",
  );
  assert.ok(
    failed.reason.includes("factory-qualification-invalid-value"),
    "immutable assertion diagnostic missing",
  );
  one(
    run.filter((event) => event.event === "FactoryRunEscalated"),
    "bounded escalation required",
  );
  assert.ok(
    !run.some(
      (event) => event.phase === "management" && String(event.usageId ?? "").startsWith("review-"),
    ),
    "failed validation reached semantic review",
  );
    ...
    one(
      run.filter(
        (event) =>
          ...
          event.workItem === capacity.workItem &&
          event.attempt === capacity.attempt &&
          event.phase === capacity.phase &&
          event.backend === capacity.backend,
      ),
      "capacity cleanup accounting incomplete",
    );
...
export function assertConflictPreserved(before, after, authority, cancelled = false) {
  const original = failureEvents(before, authority),
    final = failureEvents(after, authority);
  assert.deepEqual(final.start, original.start);
  const kinds = new Set(["attempt", "budget", "validation", "publication", "graph"]);
  assert.deepEqual(
    final.run.filter((event) => kinds.has(event.kind)),
    original.run.filter((event) => kinds.has(event.kind)),
    "conflict continuation repeated or changed work/accounting",
  );
  assert.ok(
    !final.run.some((event) => event.event === "FactoryRunEscalated"),
    "unexpected terminal escalation",
  );
  if (cancelled) {
    const request = one(
      final.run.filter(
        (event) =>
          ...
          event.requestId === `${authority.namespace}-cancel`,
      ),
      "exact cleanup cancellation request required",
    );
    const terminal = one(
      final.run.filter((event) => event.event === "FactoryRunCancelled"),
      "same-run cancellation completion required",
    );
    assert.ok(terminal.sequence > request.sequence);
  } else
    assert.ok(
      !final.run.some((event) => event.event === "FactoryRunCancelled"),
      "terminal runs are not continuation fixtures",
    );
