...
export interface LargeFileRefusalContext {
  authority: {
    repository: string; namespace: string; checkout: string;
    policy: { maxAttemptsPerItem: number; allowedPaidBackends: string[] };
    largeFile: { scenario: string };
  };
  evidence: { objective: { number: number }; base: string;
    actor: { id: number; login: string }; largeFileRefusal?: Record<string, unknown> };
...
export interface LargeFileRefusalFixture {
  namespace: string; baseSha: string;
  paths: { prefix: string; payload: string };
  lfs: Array<{ oid: string }>;
}
export function createLargeFileRefusalPorts(context: LargeFileRefusalContext, fixture: LargeFileRefusalFixture): {
