...
const sameAttempt = (left, right) =>
  ["objective", "workItem", "attempt", "runId"].every((key) => left[key] === right[key]);
...
async function packetProof(context, fixture, read, events, reserved, reservation) {
  const graphEvent = one(
    events.filter((event) => event.runId === reserved.runId && event.event === "GraphCompiled"),
    "compiled refusal graph missing or repeated",
  );
  const graphRef = `refs/clockgrove-factory/graphs/objective-${reserved.objective}/run-${hash(reserved.runId).slice(0, 32)}`;
  assert.ok(
    graphEvent.graphRef === graphRef && graphEvent.baseSha === context.evidence.base,
    "compiled refusal graph identity differs",
  );
  const demand = {
    kind: "checkpoint",
    ref: graphRef,
  ...
  const raw = safe(await read(demand)),
    graph = assertQualificationCheckpoint(raw, demand, [context.evidence.base]);
  assert.ok(
    raw.blobOid === graphEvent.graphBlobSha &&
      hash(canonical(graph)) === graphEvent.graphDigest &&
      raw.content === canonical(graph),
    "compiled refusal graph content differs",
  );
  assert.ok(
    Array.isArray(graph.workItems) && graph.workItems.length > 0 && graph.workItems.length <= 3,
    "refusal graph cardinality unavailable",
  );
  const outside = `${fixture.paths.prefix}/outside-scope.txt`;
  for (const packet of graph.workItems)
    assert.ok(
      Array.isArray(packet.scope) &&
        packet.scope.length === 1 &&
        packet.scope[0] === fixture.paths.payload &&
        !packet.scope.some((path) =>
          path.endsWith("/") ? outside.startsWith(path) : outside === path,
        ),
      "compiled scope admits the refusal recipe or outside path",
    );
  assert.ok(
    reserved.backend === "codex-app-server/local-worktree",
    "refusal packet proof requires the exact installed App Server backend",
  );
  const attemptId = hash(
    JSON.stringify([
      "clockgrove.factory/attempt-v2",
      context.authority.repository,
      reserved.runId,
      reserved.objective,
      reserved.workItem,
      reserved.attempt,
  ...
  const sessionDemand = {
    kind: "checkpoint",
    ref: `refs/clockgrove-factory/sessions/${attemptId}/prepared`,
    path: ".clockgrove-factory/control/app-server-session.json",
    maxBytes: 196608,
  };
  const prepared = safe(await read(sessionDemand)),
    session = assertQualificationCheckpoint(prepared, sessionDemand, [reservation.oid]);
  const binding = session.binding;
  for (const key of identityKeys)
    assert.ok(
      binding[key] === (key === "repository" ? context.authority.repository : reserved[key]),
      "prepared packet attempt identity differs",
    );
  assert.ok(
    session.protocol === "clockgrove.factory/app-server-session-v1" &&
      session.stage === "prepared" &&
      binding.attemptId === attemptId &&
      session.packet.baseSha === reserved.baseSha &&
      hash(canonical(session.packet)) === binding.packetDigest &&
      reserved.localScopeBatch?.identity.invocationDigest === binding.packetDigest,
    "prepared packet digest/base/scope differs",
  );
  assert.deepEqual(
    session.packet.allowedPaths,
    [fixture.paths.payload],
    "prepared packet admits unintended paths",
  );
  return {
    graph: raw,
    prepared,
    allowedPaths: session.packet.allowedPaths,
    outsideScopePath: outside,
  };
}

function treeBytes(entries) {
  assert.ok(
    entries.length <= 10000 && new Set(entries.map((entry) => entry.path)).size === entries.length,
    "raw source tree exceeds bound or duplicates paths",
  );
...
async function symlinkRetention(context, fixture, read, identity, ref, events, reserved) {
  ...
  const load = async (stage, parents) => {
    ...
    assert.ok(
      raw[stage].commit.message.trim() === message,
      "symlink transfer lifecycle binding differs",
    );
    return descriptor;
  };
  ...
  assert.ok(
    artifact.baseSha === identity.baseSha &&
      ...
      artifact.changedPaths.length === 1 &&
      artifact.changedPaths[0] === path &&
      typeof artifact.patch === "string" &&
      Buffer.byteLength(artifact.patch) <= 4096,
    "symlink artifact is not the bounded original inline output",
  );
  const blobOid = gitOid("blob", Buffer.from(target)),
    lines = artifact.patch.split("\n");
  assert.ok(
    lines.length === 9 &&
      lines[0] === `diff --git a/${path} b/${path}` &&
      lines[1] === "new file mode 120000" &&
      /^index 0{7,40}\.\.[a-f0-9]{7,40}$/.test(lines[2]) &&
      blobOid.startsWith(lines[2].split("..")[1]) &&
      lines[3] === "--- /dev/null" &&
      lines[4] === `+++ b/${path}` &&
      lines[5] === "@@ -0,0 +1 @@" &&
      lines[6] === `+${target}` &&
      lines[7] === "\\ No newline at end of file" &&
      lines[8] === "",
  ...
  assert.ok(
    canonical(file) === canonical(expectedFile),
    "symlink manifest misrepresents raw target bytes/mode",
  );
  const base = await read({ kind: "commit", oid: identity.baseSha }),
    trees = [];
  assert.ok(
    base.treeOid === manifest.baseTreeSha,
    "symlink base tree differs from original source",
  );
  const parts = path.split("/");
  assert.ok(parts.length <= 8, "symlink fixture path exceeds bound");
  const graft = async (treeOid, index) => {
    let entries = [];
    if (treeOid) {
      const { data } = await context.request("GET /repos/{owner}/{repo}/git/trees/{tree_sha}", {
        tree_sha: treeOid,
      });
      assert.ok(
        data.sha === treeOid && data.truncated === false && Array.isArray(data.tree),
        "symlink source tree read incomplete",
      );
      entries = data.tree.map(({ path, mode, type, sha }) => ({ path, mode, type, sha }));
      assert.ok(
        gitOid("tree", treeBytes(entries)) === treeOid,
        "symlink source tree bytes differ from Git identity",
      );
      trees.push({ sha: treeOid, entries });
    }
    const name = parts[index],
      prior = entries.find((entry) => entry.path === name);
    if (index === parts.length - 1) {
      assert.ok(!prior, "symlink recipe must create a new path, not replace source content");
      return gitOid(
        "tree",
        treeBytes([...entries, { path: name, mode: "120000", type: "blob", sha: blobOid }]),
      );
    }
    assert.ok(
      !prior || (prior.mode === "040000" && prior.type === "tree"),
      "symlink parent path is not a raw tree",
    );
    const child = await graft(prior?.sha, index + 1);
    return gitOid(
      "tree",
      treeBytes([
        ...entries.filter((entry) => entry.path !== name),
        { path: name, mode: "040000", type: "tree", sha: child },
      ]),
    );
  };
  ...
  const succeeded = one(
    events.filter((event) => event.event === "AttemptSucceeded" && sameAttempt(event, reserved)),
    "symlink raw artifact success receipt missing",
  );
  const failed = one(
    events.filter((event) => event.event === "AttemptFailed" && sameAttempt(event, reserved)),
    "symlink guard failure missing",
  );
  assert.ok(
    succeeded.artifactDigest === digest &&
      succeeded.sequence > reserved.sequence &&
      succeeded.sequence < failed.sequence &&
      succeeded.baseSha === reserved.baseSha &&
      succeeded.backend === reserved.backend &&
      succeeded.directorEpoch === reserved.directorEpoch &&
      succeeded.policyDigest === reserved.policyDigest,
  ...
  return {
    ...
    async artifactRefusal(observation) {
      ...
      for (const name of ["FactoryRunStarted", "AttemptReserved", "AttemptStarted"])
        assert.ok(
          events.filter((event) => event.event === name).length === 1,
          "refusal Objective has another execution identity",
        );
      const start = one(
      ...
      const reserved = one(
        run.filter((event) => event.event === "AttemptReserved"),
        "one actual refusal attempt required",
      );
      const started = one(
        run.filter((event) => event.event === "AttemptStarted"),
        "one actual worker start required",
      );
      assert.ok(
        reserved.attempt === 1 &&
          sameAttempt(reserved, started) &&
          reserved.baseSha === evidence.base &&
          reserved.policyDigest === start.policyDigest &&
          ["baseSha", "backend", "policyDigest", "directorEpoch"].every(
            (key) => reserved[key] === started[key],
          ),
      ...
      assert.ok(
        ...
        "refused artifact reached acceptance/publication",
      );
      one(
        run.filter((event) => event.event === "FactoryRunEscalated"),
        "expected refusal run escalation is unavailable",
      );
      const failed = one(
        run.filter((event) => event.event === "AttemptFailed" && sameAttempt(event, reserved)),
        "one exact failed artifact attempt required",
      );
      assert.ok(
        failed.sequence > started.sequence &&
          typeof failed.reason === "string" &&
          failed.reason.length <= 8000 &&
          ["baseSha", "backend", "policyDigest", "directorEpoch"].every(
            (key) => failed[key] === reserved[key],
          ),
        "failed refusal receipt unavailable",
      );
      const expected =
        scenario === "scope"
      ...
      assert.ok(
        !run.some((event) => event.event === "ValidationRecorded"),
      ...
      const identity = Object.fromEntries(
        identityKeys.map((key) => [
          key,
          key === "repository" ? authority.repository : reserved[key],
        ]),
      );
      const ref = `refs/clockgrove-factory/artifact-transfers/${hash(JSON.stringify(identity))}`;
      const transfer =
        scenario === "symlink"
          ? await symlinkRetention(context, fixture, read, identity, ref, run, reserved)
          : {
      ...
      const result = {
        ...
        uploadEvidence:
          scenario === "symlink"
            ? "exact raw Git-object retention observed; not a filesystem execution grant"
