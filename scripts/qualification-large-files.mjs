...
function outputFiles(namespace) {
  ...
  const metadata = Buffer.from(`${JSON.stringify({ recipe: "factory-large-files-fixture-v1", bytes: audio.length, sha256: hash(audio), format: "PCM-u8-mono-8000Hz" }, null, 2)}\n`);
  const executable = Buffer.from(`#!/usr/bin/env node\nimport { verify } from "../large-files-recipe.mjs";\nverify("metadata");\n`);
  const result = Buffer.from(`${JSON.stringify({ recipe: "factory-large-files-fixture-v1", verified: true, payloadSha256: hash(audio), metadataSha256: hash(metadata), executableSha256: hash(executable) }, null, 2)}\n`);
  return [
    { path: paths.payload, phase: "payload", bytes: audio, mode: "100644", mediaType: "audio/wav", generated: true },
    { path: paths.executable, phase: "metadata", bytes: executable, mode: "100755", mediaType: "unknown", generated: false },
    { path: paths.metadata, phase: "metadata", bytes: metadata, mode: "100644", mediaType: "unknown", generated: true },
    { path: paths.result, phase: "join", bytes: result, mode: "100644", mediaType: "unknown", generated: true },
  ];
}
...
function exclusiveFile(root, path, bytes, mode = "100644") {
  ...
  const fd = fs.openSync(join(root, path), fs.constants.O_WRONLY | fs.constants.O_CREAT | fs.constants.O_EXCL | fs.constants.O_NOFOLLOW, mode === "100755" ? 0o755 : 0o644);
  try { fs.writeFileSync(fd, bytes); fs.fchmodSync(fd, mode === "100755" ? 0o755 : 0o644); }
...
function regularBytes(root, path, maximum) {
  ...
  for (const [index, part] of parts.entries()) {
    current = join(current, part);
    const info = fs.lstatSync(current);
    assert.ok(!info.isSymbolicLink() && (index === parts.length - 1 ? info.isFile() : info.isDirectory()), "fixture content is not regular");
  }
  ...
  try {
    ...
    while (offset < bytes.length) {
      ...
      assert.ok(count > 0, "fixture content truncated"); offset += count;
    }
...
function recipeOutput(root, namespace, scenario, phase) {
  ...
  if (scenario === "accepted") {
    for (const file of phaseFiles(namespace, phase)) exclusiveFile(root, file.path, file.bytes, file.mode);
  } else if (scenario === "scope") {
    exclusiveFile(root, `${paths.prefix}/outside-scope.txt`, Buffer.from("Synthetic out-of-scope refusal fixture.\n"));
...
function baselineFiles(namespace) {
  const paths = largeFilePaths(namespace);
  const lfs = [
    { path: paths.canonical, version: "https://git-lfs.github.com/spec/v1", content: Buffer.from("Factory qualification canonical LFS object; public synthetic bytes.\n") },
    { path: paths.legacy, version: "https://hawser.github.com/spec/v1", content: Buffer.from("Factory qualification legacy LFS object; public synthetic bytes.\n") },
  ];
  for (const asset of lfs) {
    asset.oid = hash(asset.content);
    asset.pointer = Buffer.from(`version ${asset.version}\noid sha256:${asset.oid}\nsize ${asset.content.length}\n`);
  }
  return {
    lfs,
    files: [
      { path: paths.attributes, bytes: Buffer.from("lfs/*.bin filter=lfs diff=lfs merge=lfs -text\n") },
      { path: paths.recipe, bytes: Buffer.from(recipeSource(namespace)) },
      { path: paths.test, bytes: Buffer.from('import {test} from "node:test";\nimport {verify} from "./large-files-recipe.mjs";\ntest("exact large-file bytes and modes",()=>verify());\n') },
...
function git(repository, args, input, maximum = 1024 * 1024, deadline = Date.now() + 120_000) {
  ...
  const result = spawnSync("/usr/bin/git", [
    ...
    "-c", "protocol.allow=never", "-c", "core.attributesFile=/dev/null", ...args,
  ], {
    cwd: repository, input, maxBuffer: maximum, timeout: Math.min(remaining, 30_000),
    env: { PATH: "/usr/bin:/bin", HOME: repository, LANG: "C", LC_ALL: "C", GIT_CONFIG_NOSYSTEM: "1", GIT_CONFIG_SYSTEM: "/dev/null", GIT_CONFIG_GLOBAL: "/dev/null", GIT_ATTR_NOSYSTEM: "1", GIT_NO_LAZY_FETCH: "1", GIT_TERMINAL_PROMPT: "0", GIT_LFS_SKIP_SMUDGE: "1", GIT_AUTHOR_NAME: "Factory qualification", GIT_AUTHOR_EMAIL: "fixture@example.invalid", GIT_COMMITTER_NAME: "Factory qualification", GIT_COMMITTER_EMAIL: "fixture@example.invalid", GIT_AUTHOR_DATE: "2026-01-01T00:00:00Z", GIT_COMMITTER_DATE: "2026-01-01T00:00:00Z" },
  });
...
function listing(repository, treeish, prefix, deadline) {
  ...
  const entries = git(repository, ["ls-tree", "-r", "-t", "-l", "-z", treeish, ...(prefix ? ["--", prefix] : [])], undefined, 4 * 1024 * 1024, deadline).toString("utf8").split("\0").filter(Boolean);
...
function importBase(source, destination, baseSha, deadline) {
  ...
  for (const object of objects) {
    ...
    const bytes = git(source, ["cat-file", object.type, object.oid], undefined, (object.bytes ?? 4 * 1024 * 1024) + 1024, deadline);
    assert.equal(gitOid(object.type, bytes), object.oid);
    assert.equal(gitText(destination, ["hash-object", "-w", "--stdin", "-t", object.type], bytes, 1024, deadline), object.oid);
    copied.add(object.oid);
  }
  for (const file of files) {
    const bytes = git(destination, ["cat-file", "blob", file.oid], undefined, file.bytes + 1024, deadline);
    exclusiveFile(destination, file.path, bytes, file.mode);
  }
  assert.equal(gitText(destination, ["hash-object", "-w", "--stdin", "-t", "commit"], commit, 1024, deadline), baseSha);
  git(destination, ["read-tree", baseSha], undefined, 1024, deadline);
  return tree;
}
function describeFile(file) {
  return { path: file.path, mode: file.mode ?? "100644", bytes: file.bytes.length, digest: hash(file.bytes) };
}

export function createLargeFileFixture({ parent, namespace, sourceRepository, baseSha }) {
  assertNamespace(namespace);
  assert.equal(sourceRepository === undefined, baseSha === undefined, "source repository/base must be paired");
  ...
  git(repository, ["init", "--quiet", "--template=", "--object-format=sha1"], undefined, 1024, deadline);
  const sourceTreeSha = sourceRepository ? importBase(sourceRepository, repository, baseSha, deadline) : undefined;
  ...
  for (const file of baseline.files) {
    ...
    git(repository, ["update-index", "--add", "--cacheinfo", "100644", oid, file.path], undefined, 1024, deadline);
  }
  const baseTreeSha = gitText(repository, ["write-tree"], undefined, 1024, deadline);
  const fixtureBase = gitText(repository, ["commit-tree", baseTreeSha, ...(baseSha ? ["-p", baseSha] : []), "-m", `Large-file qualification baseline ${namespace}`], undefined, 1024, deadline);
  ...
  const lfs = baseline.lfs.map((asset) => {
    ...
    return { path: asset.path, oid: asset.oid, size: asset.content.length, pointerDigest: hash(asset.pointer), pointerBlobOid: gitOid("blob", asset.pointer), mode: "100644", objectPath: object };
  });
  const descriptor = {
    version: LARGE_FILE_RECIPE_VERSION, id, namespace, root, repository, baseSha: fixtureBase, baseTreeSha,
    ...(baseSha ? { sourceBaseSha: baseSha, sourceTreeSha } : {}),
    paths: largeFilePaths(namespace), recipePath: largeFilePaths(namespace).recipe,
    baseline: baseline.files.map(describeFile), lfs,
    expected: outputFiles(namespace).map((file) => ({ ...describeFile(file), phase: file.phase, mediaType: file.mediaType, generated: file.generated })),
  };
...
function ownedFixture(fixture, checkout = fixture.repository) {
  ...
  const saved = JSON.parse(regularBytes(fixture.root, "fixture.json", 64 * 1024).bytes.toString("utf8"));
  ...
  assert.ok(checkout.startsWith(`${fixture.root}${sep}`), "fixture output cannot mutate another checkout");
  return checkout;
}
export function writeLargeFileOutput({ fixture, checkout = fixture.repository, scenario = "accepted", phase = "payload" }) {
...
export function largeFileScenario(id) {
  const scenarios = {
    "lfs-tool-missing": { stage: "source-preflight", reason: /requires git-lfs|Git LFS.*unavailable/i },
    "lfs-object-missing": { stage: "source-preflight", reason: /required LFS object.*missing|LFS object.*cache/i },
...
export function assertLargeFileRefusal(observation) {
  ...
  assert.ok(!observation.reason.includes("Q".repeat(40)), "refusal leaked the synthetic credential canary");
...
export function observeLargeFileTree({ repository, treeish, fixture, baseSha = fixture.baseSha, patch }) {
  assert.match(treeish, /^[a-f0-9]{40}$/); assert.match(baseSha, /^[a-f0-9]{40}$/);
  const deadline = Date.now() + 120_000;
  const treeSha = gitText(repository, ["rev-parse", `${treeish}^{tree}`], undefined, 1024, deadline);
  const baseTreeSha = gitText(repository, ["rev-parse", `${baseSha}^{tree}`], undefined, 1024, deadline);
  const entries = listing(repository, treeSha, largeFilePaths(fixture.namespace).prefix, deadline).filter((entry) => entry.type === "blob");
  assert.ok(entries.length <= 16 && entries.every((entry) => entry.bytes <= LARGE_FILE_AUDIO_BYTES));
  const expected = new Map([...baselineFiles(fixture.namespace).files, ...outputFiles(fixture.namespace)].map((file) => [file.path, file]));
  const files = entries.map((entry) => {
    const spec = expected.get(entry.path);
    assert.ok(spec, "unexpected path in qualification namespace");
    const bytes = git(repository, ["cat-file", "blob", entry.oid], undefined, entry.bytes + 1024, deadline);
    ...
    return { path: entry.path, mode: entry.mode, gitBlobOid: entry.oid, bytes: bytes.length, digest: hash(bytes), mediaType: spec.mediaType ?? "unknown", generated: spec.generated ?? false };
  });
  for (const file of baselineFiles(fixture.namespace).files)
    assert.ok(files.some((observed) => observed.path === file.path), "baseline/LFS pointer missing");
  let patchProof;
  if (patch !== undefined) {
    assert.ok(patch instanceof Uint8Array && patch.byteLength > 0 && patch.byteLength <= MAX_PATCH_BYTES);
    ownedFixture(fixture);
    assert.ok(fs.readdirSync(fixture.root).filter((name) => name.startsWith("patch-proof-")).length < 16, "fixture patch-proof directory bound exceeded");
    ...
    git(proof, ["apply", "--cached", "--binary", "--whitespace=error-all", "-"], patch, 1024, deadline);
    const appliedTreeSha = gitText(proof, ["write-tree"], undefined, 1024, deadline);
    assert.equal(appliedTreeSha, treeSha, "actual patch does not produce the independently observed Git tree");
    patchProof = { bytes: patch.byteLength, digest: hash(patch), appliedTreeSha };
  }
  return { baseSha, baseTreeSha, treeSha, files, ...(patchProof ? { patchProof } : {}), provenance: "independent-local-raw-git-object-read" };
}
...
export function assertLargeFileArtifact({ fixture, artifact, observation, patch, phase }) {
  ...
  assert.equal(observation.provenance, "independent-local-raw-git-object-read");
  assert.ok(Array.isArray(artifact.changedPaths) && artifact.changedPaths.length === expected.length);
  ...
  for (const spec of expected) {
    ...
    assert.deepEqual(file, { path: spec.path, action: "write", mode: spec.mode, bytes: spec.bytes.length, digest: hash(spec.bytes), mediaType: spec.mediaType, generated: spec.generated });
    for (const [key, value] of Object.entries(describeFile(spec))) assert.equal(actual[key], value);
    assert.equal(actual.gitBlobOid, gitOid("blob", spec.bytes));
  }
  assert.ok(patch instanceof Uint8Array && patch.byteLength > 0 && patch.byteLength <= MAX_PATCH_BYTES);
  const bytes = Buffer.from(patch);
  assert.deepEqual(observation.patchProof, { bytes: bytes.length, digest: hash(bytes), appliedTreeSha: observation.treeSha }, "artifact patch lacks independent exact-tree application proof");
  if (phase === "payload") {
    ...
    assert.equal(payload.bytes, bytes.length); assert.equal(payload.digest, hash(bytes));
    ...
    assert.equal(artifact.patch, `# Factory content-addressed Git patch sha256:${hash(bytes)} bytes:${bytes.length}\n`);
  } else {
  ...
  const normalizedManifest = {
    version: 1, baseTreeSha: manifest.baseTreeSha, resultTreeSha: manifest.resultTreeSha,
    files: manifest.files.map((file) => ({ path: file.path, action: file.action, mode: file.mode, bytes: file.bytes, digest: file.digest, mediaType: file.mediaType, generated: file.generated })),
  };
  const normalizedPayload = artifact.payload ? {
    kind: artifact.payload.kind, digest: artifact.payload.digest, bytes: artifact.payload.bytes,
    chunks: artifact.payload.chunks.map((chunk) => ({ digest: chunk.digest, bytes: chunk.bytes })),
  } : undefined;
  const digest = createHash("sha256").update(artifact.baseSha).update("\0")
    .update(artifact.changedPaths.slice().sort().join("\0")).update("\0").update(artifact.patch)
    .update("\0content-v1\0").update(JSON.stringify({ ...(normalizedPayload ? { payload: normalizedPayload } : {}), fileManifest: normalizedManifest })).digest("hex");
  assert.equal(artifact.digest, digest, "artifact content digest differs");
  return { phase, artifactDigest: artifact.digest, resultTreeSha: observation.treeSha, patchBytes: bytes.length, patchSha256: hash(bytes), oversized: bytes.length > INLINE_BYTES };
}
