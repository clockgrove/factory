...
function linkedShape(event, requireMarkers) {
  ...
  assert.ok(["execution", "management"].includes(event.phase), "model invocation has an invalid phase");
  for (const field of ["objective", "workItem", "attempt"])
    if (field === "objective" || event[field] !== undefined)
      assert.ok(Number.isSafeInteger(event[field]) && event[field] > 0, "invalid model invocation tuple");
  ...
  if (requireMarkers || event.policyDigest !== undefined) {
    assert.match(event.policyDigest ?? "", /^[a-f0-9]{64}$/i, "fresh dispatch lacks its original policy binding");
    assert.ok(Number.isSafeInteger(event.directorEpoch) && event.directorEpoch > 0, "fresh dispatch lacks its original epoch");
  }
  assert.ok(typeof event.usageId === "string" && event.usageId.length > 0 && event.usageId.length <= 300);
}

function actualCounters(event) {
  assert.ok(event.usageEvidence === undefined || event.usageEvidence === "as-recorded", "model usage cannot be a conservative native reservation");
  ...
  assert.ok(fields.length > 0 && fields.every((field) => ["inputTokens", "outputTokens", "cachedInputTokens"].includes(field)));
  ...
  if (usage.inputTokens !== undefined && usage.outputTokens !== undefined)
    assert.equal(usage.inputTokens + usage.outputTokens, event.amount, "actual model total contradicts reported counters");
}
...
export function qualificationModelAccounting(events, { requireMarkers = false } = {}) {
  ...
  for (const event of usage) {
    ...
    if (event.modelInvocationId !== undefined) {
      linkedShape(event, requireMarkers);
      assert.ok(!event.usageId.startsWith("invocation-"), "dispatch marker cannot masquerade as actual usage");
