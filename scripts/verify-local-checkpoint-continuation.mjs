...
const one = (values) => {
  assert.equal(values.length, 1, "unique original identity required");
  return values[0];
};
...
export function assertContinuationSeed(original, witness, pause, installed, now = Date.now()) {
  ...
  assert.deepEqual(
    original.actions.map((entry) => entry.action),
    ["start", "create", "activate"],
  ...
  assert.deepEqual(original.runRequest, {
    tool: "factory_activate",
    arguments: {
      owner,
      repo,
      objectiveNumber: original.objective.number,
      requestId: activation.requestId,
      baseSha: original.base,
      policy: original.authority.policy,
    },
  });
  ...
  const controller = one(
    original.controllerReadiness.filter((entry) => entry.identity).map((entry) => entry.identity),
  );
  for (const [field, key] of [
    ["unit", "unit"],
    ["invocationId", "invocationId"],
    ["pid", "producerPid"],
    ["startTicks", "producerStartTicks"],
...
export function continuationEvidencePath(original, originalPath, outputPath) {
  ...
  assert.equal(
    boundedPath(outputPath),
    `${original.authority.evidence}.continuation.json`,
...
export function assertContinuationRetry(previous, input, now = Date.now()) {
  ...
  assert.deepEqual(
    previous.actions,
    [],
    "any attempted or uncertain lifecycle action blocks retry",
  );
  assert.equal(previous.authority.evidence, previousPath);
  const priorAuthority = { ...previous.authority },
    originalAuthority = { ...original.authority };
  delete priorAuthority.evidence;
  delete originalAuthority.evidence;
  ...
  one(
    previous.harnessFiles.filter(
      (file) => file.path === "scripts/verify-local-checkpoint-continuation.mjs",
    ),
  );
  ...
  assert.ok(
    Number.isSafeInteger(depth) && depth >= 0 && depth < 3,
    "bounded read-only retry chain exhausted",
  );
  if (depth === 0) {
  ...
  return {
    outputPath: `${base}.retry-${previousSha256}.json`,
    retry: { previousPath, previousSha256, depth: depth + 1 },
  };
}
...
export async function continuationStage(stage, operation, record) {
  ...
  try {
    ...
    record({
      ...checkpointObservationFailure(error, { phase: "continuation", stage: "extension" }),
      stage,
      state: "refused",
    });
    throw error;
...
export async function continuationPreflight(context, original, stopped, stage, bounded) {
  ...
  await stage("actor", async () => {
    ...
    assert.equal(actor.id, original.actor.id);
    assert.equal(actor.login, original.actor.login);
  });
  const repository = await stage("repository", async () => {
    const value = (await request("GET /repos/{owner}/{repo}")).data;
    assert.ok(value.private && !value.archived && value.permissions?.push);
    return value;
  });
  await stage("objective", async () => {
    const issue = (
      await request("GET /repos/{owner}/{repo}/issues/{issue_number}", {
        issue_number: original.objective.number,
      })
    ).data;
    assert.equal(issue.node_id, original.objective.node_id);
    assert.equal(issue.id, original.objective.id);
    assert.equal(issue.user.id, original.actor.id);
    assert.equal(hash(issue.body), original.objectiveBodyDigest);
  });
  await stage("checkout-clean", () =>
  ...
  await stage("checkout-base", () =>
    assert.equal(command("git", ["rev-parse", "HEAD"], authority.checkout), original.base),
  );
  await stage("remote-base", async () =>
    assert.equal(
  ...
  await stage("other-objectives", async () => {
    for (const issue of await list("GET /repos/{owner}/{repo}/issues", { state: "open" }))
      if (
        issue.number !== original.objective.number &&
...
export function assertContinuationObservation(
  ...
  assert.deepEqual(
    one(events.filter((event) => event.event === "ActivationRequested")),
    original.actions[2].response,
  );
  assert.deepEqual(one(events.filter((event) => event.event === "RunPauseRequested")), pause);
  assert.ok(
    events.every((event) => event.runId === binding.runId || event.event === "ActivationRequested"),
  );
  ...
  assert.ok(
    appServerHoldReady(
      { ...observation, checkpointReached: witness },
      original.authority,
      original.sessionArm,
      pause.requestId,
    ),
  ...
  for (const { event } of original.latest?.receipts ?? [])
    assert.ok(
      events.some((fresh) => canonical(fresh) === canonical(event)),
      "original receipt disappeared",
    );
  ...
  assert.equal(
    model.usage.length,
    2,
    "only original compiler and one worker may have consumed tokens",
  );
...
async function observeGraph(context, observation) {
  ...
  const projectionRequest = {
    kind: "checkpoint",
    ref: projected.projectionRef,
    path: ".clockgrove-factory/control/graph-projection.json",
    maxBytes: 2097152,
  };
  const projectionProof = await read(projectionRequest);
  const projection = assertQualificationCheckpoint(
    projectionProof,
    projectionRequest,
    graphProof.commit.oid,
  );
  ...
  for (const [index, binding] of projection.bindings.entries()) {
    ...
    const metadata = one([
      ...child.body.matchAll(/<!--\s*clockgrove-factory:graph-item\s+([A-Za-z0-9_-]+)\s*-->/g),
    ]);
    ...
    assert.deepEqual(
      dependencies.map((entry) => entry.number).sort((a, b) => a - b),
      packet.dependsOn
        .map(
          (id) => one(projection.bindings.filter((entry) => entry.compilerId === id)).issueNumber,
        )
        .sort((a, b) => a - b),
    );
  }
...
export async function runCheckpointContinuation(port, authority, input) {
  ...
  const seed = await stage("runner-seed", () =>
    assertContinuationSeed(original, witness, pause, input.artifact),
  );
  await port.preflight();
  const observed = await stage("fresh-observation", () => port.observe());
  const facts = await stage("fresh-observation", () =>
    assertContinuationObservation(observed, original, witness, pause),
  );
  ...
  await port.checkpoint({
    checkpoint: held,
    facts: { ...facts, stable: originalEvents },
    original: seed.controller,
    scopes,
    sessionProofs,
    graph,
  });
  if (input.diagnosticsOnly)
    return {
      result: "diagnostic-only",
      scope: "read-only-continuation-preconditions",
      lifecycleActionsAttempted: false,
    };
...
export async function main(env = process.env) {
  ...
  const original = readContinuationInput(
    env.FACTORY_CHECKPOINT_ORIGINAL,
    env.FACTORY_CHECKPOINT_ORIGINAL_SHA256,
  );
  const witness = readContinuationInput(
    env.FACTORY_CHECKPOINT_WITNESS,
    env.FACTORY_CHECKPOINT_WITNESS_SHA256,
    16384,
  );
  const pause = readContinuationInput(
    env.FACTORY_CHECKPOINT_PAUSE,
    env.FACTORY_CHECKPOINT_PAUSE_SHA256,
    16384,
  );
  const diagnosticsOnly = env.FACTORY_CHECKPOINT_DIAGNOSTICS_ONLY === "1";
  assert.equal(
    env.FACTORY_CHECKPOINT_CONTINUATION_ACK,
  ...
  if (diagnosticsOnly) {
    ...
    assert.match(
      output.slice(`${original.authority.evidence}.diagnostic-`.length),
      /^[a-z0-9-]+\.json$/,
    );
  } else if (env.FACTORY_CHECKPOINT_PREVIOUS || env.FACTORY_CHECKPOINT_PREVIOUS_SHA256) {
    assert.equal(env.FACTORY_CHECKPOINT_ORIGINAL, original.authority.evidence);
    let previousPath = env.FACTORY_CHECKPOINT_PREVIOUS,
      previousSha256 = env.FACTORY_CHECKPOINT_PREVIOUS_SHA256;
    for (let depth = 0; depth < 3; depth++) {
      const previous = readContinuationInput(previousPath, previousSha256);
      const proof = assertContinuationRetry(previous, {
        original,
        witness,
        pause,
        originalSha256: env.FACTORY_CHECKPOINT_ORIGINAL_SHA256,
        witnessSha256: env.FACTORY_CHECKPOINT_WITNESS_SHA256,
        pauseSha256: env.FACTORY_CHECKPOINT_PAUSE_SHA256,
        previousPath,
        previousSha256,
      });
      priorRecords.push(previous);
      if (depth === 0) {
        output = proof.outputPath;
        retry = proof.retry;
      }
    ...
    output = continuationEvidencePath(
      original,
      env.FACTORY_CHECKPOINT_ORIGINAL,
      env.FACTORY_CHECKPOINT_EVIDENCE,
    );
  ...
  await checkpointMain(
    env,
    (port, accepted) =>
      runCheckpointContinuation(port, accepted, {
        original,
        witness,
        pause,
    ...
    {
      authority,
      scope: "installed-app-server-observation-continuation",
      ...
      extendPort: async (context) => {
        ...
        const stage = (name, operation) =>
          continuationStage(name, operation, (entry) => {
            (evidence.continuationDiagnostics ??= []).push(entry);
            save();
          });
        const seed = await stage("seed", () =>
          assertContinuationSeed(original, witness, pause, artifact),
        );
        const root = resolve(fileURLToPath(new URL("..", import.meta.url)));
        await stage("original-harness", () => {
          for (const file of original.harnessFiles) {
            assert.match(file.path, /^scripts\/[A-Za-z0-9_.-]+\.mjs$/);
            assert.equal(
              hash(`${command("git", ["show", `${original.sourceCommit}:${file.path}`], root)}\n`),
              file.sha256,
        ...
        await stage("original-harness", () => {
          for (const prior of priorRecords)
            for (const file of prior.harnessFiles) {
              assert.match(file.path, /^scripts\/[A-Za-z0-9_.-]+\.mjs$/);
              assert.equal(
                hash(`${command("git", ["show", `${prior.sourceCommit}:${file.path}`], root)}\n`),
                file.sha256,
        ...
        assert.equal(
          original.harnessSha256,
          one(
            original.harnessFiles.filter(
              (file) => file.path === "scripts/verify-local-checkpoint-restart.mjs",
            ),
          ).sha256,
        );
        for (const key of [
        ...
        evidence.continuation = {
          ...
          witnessSha256: env.FACTORY_CHECKPOINT_WITNESS_SHA256,
          pauseSha256: env.FACTORY_CHECKPOINT_PAUSE_SHA256,
          originalSourceCommit: original.sourceCommit,
          originalResult: original.result,
          runId: seed.runId,
          deadline: new Date(seed.deadline).toISOString(),
          enteredAt: new Date().toISOString(),
          ...(retry ? { retry } : {}),
        };
        save();
        const bounded = () =>
          assert.ok(Date.now() < seed.deadline, "original qualification deadline expired");
        const stopped = async () => {
          const fields = await stage("controller-properties", () =>
            Object.fromEntries(
              command("systemctl", [
                "--user",
                "show",
          ...
          await stage("controller-fragment", () => {
            assert.equal(
              fields.FragmentPath,
              `${realpathSync(homedir())}/.config/systemd/user/${authority.unit}`,
            );
            const meta = lstatSync(fields.FragmentPath);
            assert.ok(meta.isFile() && meta.uid === process.getuid());
          });
          const configDigest = await stage("controller-config", () =>
            assertControllerUnit(context.readBounded(fields.FragmentPath, 16384), {
          ...
          let absent = false;
          await stage("controller-process", () => {
            try {
              lstatSync(`/proc/${seed.controller.pid}`);
            } catch (error) {
              if (error.code !== "ENOENT") throw error;
              absent = true;
            }
          });
          const result = await stage("controller-stopped", () =>
            assertStoppedContinuationController(fields, seed.controller, configDigest, absent),
          );
          evidence.continuation.stopped = result;
          save();
          return result;
        };
        const continuationPort = {
          ...port,
          pauseRequestId: pause.requestId,
          stopped,
          stage,
          ...
          action: async (action) => {
            assert.equal(
              diagnosticsOnly,
              false,
              "observation-only diagnosis cannot perform lifecycle actions",
            );
            assert.ok(
              ["start", "resume", "stop"].includes(action),
              "continuation may not create, activate, rearm or restart work",
            );
            ...
            assert.equal(
              ...
              0,
              "uncertain continuation start must not repeat",
            );
            await continuationPort.preflight();
            const fresh = await port.observe();
            assertContinuationObservation(fresh, original, witness, pause);
            assert.deepEqual(
              fresh.receipts.map(({ event }) => event).filter((event) => event.kind === "graph"),
              evidence.checkpoint.checkpoint.receipts
                .map(({ event }) => event)
                .filter((event) => event.kind === "graph"),
            );
            ...
            save();
            const response = await call("factory_controller_start", {
              repository: authority.checkout,
              requestId: `${authority.namespace}-continuation-start`,
            });
            evidence.actions[0].response = response;
            evidence.actions[0].returnedAt = new Date().toISOString();
            save();
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    process.exitCode = 2;
    console.error("Checkpoint continuation refused; no automatic replay or cleanup.");
  }
}
