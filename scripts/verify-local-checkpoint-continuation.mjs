...
export async function runCheckpointContinuation(port, authority, input) {
  ...
  const seed = assertContinuationSeed(original, witness, pause, input.artifact);
  await port.preflight();
  const observed = await port.observe();
  const facts = assertContinuationObservation(observed, original, witness, pause);
  const held = { ...observed, checkpointReached: witness };
  const graph = await port.graphProof(held);
  const sessionProofs = await port.sessionProof(held, witness);
  assert.equal(sessionProofs.length, 1);
  const scopes = await port.absence(held, [seed.controller], true);
  await port.stopped();
...
export async function main(env = process.env) {
  ...
  const authority = {
    ...original.authority,
    evidence: continuationEvidencePath(
      original,
      env.FACTORY_CHECKPOINT_ORIGINAL,
  ...
  await checkpointMain(
      ...
      }),
    {
      ...
      extendPort: async (context) => {
        ...
        const seed = assertContinuationSeed(original, witness, pause, artifact);
        ...
        const stopped = async () => {
          ...
          const configDigest = assertControllerUnit(
            context.readBounded(fields.FragmentPath, 16384),
            {
          ...
          const result = assertStoppedContinuationController(
            fields,
            seed.controller,
            configDigest,
            absent,
