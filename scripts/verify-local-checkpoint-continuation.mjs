...
export async function main(env = process.env) {
  ...
  const authority = { ...original.authority, evidence: continuationEvidencePath(original,
    env.FACTORY_CHECKPOINT_ORIGINAL, env.FACTORY_CHECKPOINT_EVIDENCE) };
  assert.match(authority.repository, /^[a-z0-9_.-]+\/[a-z0-9_.-]+$/);
  assert.notEqual(authority.repository, "clockgrove/factory");
  await checkpointMain(env, (port, accepted) => runCheckpointContinuation(port, accepted, { original, witness, pause, artifact: original.artifact }), {
    ...
    extendPort: async (context) => {
      ...
      const stopped = async () => {
        const fields = Object.fromEntries(command("systemctl", ["--user", "show", authority.unit,
        ...
        assert.equal(fields.FragmentPath, `${realpathSync(homedir())}/.config/systemd/user/${authority.unit}`);
        const meta = lstatSync(fields.FragmentPath);
        assert.ok(meta.isFile() && meta.uid === process.getuid());
        const configDigest = assertControllerUnit(context.readBounded(fields.FragmentPath, 16384), {
        ...
        let absent = false;
        try { lstatSync(`/proc/${seed.controller.pid}`); } catch (error) { if (error.code !== "ENOENT") throw error; absent = true; }
        const result = assertStoppedContinuationController(fields, seed.controller, configDigest, absent);
        evidence.continuation.stopped = result; save(); return result;
      };
      const continuationPort = {
        ...port, pauseRequestId: pause.requestId, stopped,
        ...
        preflight: async () => {
          ...
          assert.equal((await request("GET /repos/{owner}/{repo}/commits/{ref}", { ref: repository.default_branch })).data.sha, original.base);
