...
export function concurrencyAuthority(
...
export function verifyConcurrencyArtifacts(
  request: (route: string, parameters: Record<string, unknown>, timeoutMs?: number) => Promise<unknown>,
  authority: ConcurrencyAuthority,
  branch: string,
  evidence: unknown[],
  deadline?: number,
...
export function main(
  ...
  run?: (
    env: Record<string, string | undefined>,
    runner: typeof runConcurrencyScenario,
    extension: CheckpointExtension,
  ) => Promise<void>,
): Promise<void>;
export function assertRetiredController(
