...
export interface FailurePort extends CheckpointPort {
  contentProof(observation: unknown, witness?: unknown): Promise<{ summary: unknown; objects: FailureContentProof }>;
...
export function runFailureScenario(port: FailurePort, authority: FailureAuthority): Promise<unknown>;
export function assertRefusalJournal(rows: unknown, authority: { namespace: string }, competing: { branch: string; before: string; after: string; observedAt: string }, producer: { invocationId: string }): Record<string, unknown>;
