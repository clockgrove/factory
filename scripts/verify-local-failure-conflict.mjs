...
export function failureAuthority(env) {
  ...
  return {
    ...
    failure: {
      scenario,
      baseSha: env.FACTORY_FAILURE_BASE_SHA,
      fixtureDigest: env.FACTORY_FAILURE_FIXTURE_SHA256,
    },
  };
}
...
export function assertRefusalJournal(rows, authority, competing, producer) {
  ...
  const matches = rows.filter(
    (row) => row.MESSAGE === expected && row._SYSTEMD_INVOCATION_ID === producer.invocationId,
  );
  ...
  return {
    ...
    atMicroseconds: row.__REALTIME_TIMESTAMP,
    message: expected,
    diagnosticBoundary:
      "exact current-activation base refusal after real worker completion; not internal merge repair",
  };
}
...
export async function runFailureScenario(port, authority) {
  ...
  if (authority.failure.scenario === "failed-validation") {
    ...
    const failed = await port.poll("failed-validation", (value) =>
      value.receipts.some(({ event }) => event.event === "FactoryRunEscalated"),
    );
    const facts = assertFailedValidation(
      failed,
      authority,
      failureFixture(authority.namespace, "failed-validation"),
    );
  ...
  const cancelled = await port.poll("cancelled", (value) =>
    value.receipts.some(({ event }) => event.event === "FactoryRunCancelled"),
  );
  ...
  return {
    result: "passed",
    scenario: "real-conflict",
    boundary:
      "real post-execution content conflict; bounded earlier controller reconciliation refusal, not internal merge repair",
    competing,
    content,
    accounting,
    refusal,
  ...
  return {
    ...
    extendPort: async (context) => {
      ...
      const once = async (name, action) => {
        assert.ok(
          !(evidence.failureActions ?? []).some((entry) => entry.name === name),
          "uncertain qualification mutation cannot repeat",
        );
        const entry = { name, requestedAt: new Date().toISOString() };
        (evidence.failureActions ??= []).push(entry);
        save();
      ...
      const noPublication = async () => {
        assert.equal(
          (await list("GET /repos/{owner}/{repo}/pulls", { state: "all" })).filter(
            (pull) =>
              (pull.head?.ref ?? "").startsWith(
                `factory/objective-${evidence.objective.number}/`,
              ) ||
              new RegExp(
                `(?:close[sd]?|fix(?:es|ed)?|resolve[sd]?)\\s+#${evidence.objective.number}\\b`,
                "i",
              ).test(pull.body ?? ""),
          ).length,
          0,
          "negative case has a publication",
        );
      ...
      const currentRef = async () => {
        ...
        const data = (
          await request("GET /repos/{owner}/{repo}/git/ref/{ref}", { ref: ref.slice(5) })
        ).data;
      ...
      return {
        ...
        stopOriginal: async (original) => {
          await port.controller("active", original);
          await once("stop-original", () =>
            call("factory_controller_stop", {
              repository: authority.checkout,
              requestId: `${authority.namespace}-stop-original`,
            }),
          );
        ...
        compete: async (held, content) => {
          ...
          assert.equal(
            await currentRef(),
            evidence.base,
            "competing writer changed expected trunk before qualification CAS",
          );
          const { objects } = content;
          assert.equal(objects.baseSha, evidence.base);
          const blob = await once(
            "competing-blob",
            async () =>
              (
                await request("POST /repos/{owner}/{repo}/git/blobs", {
                  encoding: "utf-8",
                  content: fixture.competing,
                })
              ).data,
          );
            ...
            async () =>
              (
                await request("POST /repos/{owner}/{repo}/git/trees", {
                  base_tree: objects.baseTreeSha,
                  tree: [
                    { path: fixture.paths.payload, mode: "100644", type: "blob", sha: blob.sha },
                  ],
                })
              ).data,
          );
          assert.equal(
            tree.sha,
            objects.competingTree,
            "remote competing tree differs from proven conflict",
          );
            ...
            async () =>
              (
                await request("POST /repos/{owner}/{repo}/git/commits", {
                  message: `Explicit qualification competing content [${authority.namespace}]`,
                  tree: tree.sha,
                  parents: [evidence.base],
                })
              ).data,
          );
          ...
          const result = {
            ...
            after: commit.sha,
            tree: tree.sha,
            branch: evidence.failureRepository.branch,
            requestedAt: new Date().toISOString(),
          };
          evidence.competing = result;
          save();
          await once("cas-fixture-trunk", async () => {
            const response = await request("POST /graphql", {
              query: updateRefs,
              variables: {
                repositoryId: evidence.failureRepository.nodeId,
                name: `refs/heads/${result.branch}`,
                beforeOid: result.before,
                afterOid: result.after,
              },
            });
            assert.equal(
              response.data.errors,
              undefined,
              "conditional trunk mutation refused; retain uncertain evidence",
            );
          ...
          assert.equal(
            await currentRef(),
            result.after,
            "exact conditional trunk mutation not observed",
          );
        ...
        refusal: async (producer, competing) => {
          ...
          for (let count = 0; count < 36; count++) {
            ...
            const expected = `[factory-controller] preflight blocked: activation ${authority.namespace}-activate is stale: ${competing.branch} advanced from ${competing.before} to ${competing.after}; reactivate against the new head`;
            if (rows.some((row) => row.MESSAGE === expected))
              return assertRefusalJournal(rows, authority, competing, producer);
            await sleep(5000);
          }
          throw Error(
            "bounded post-conflict controller refusal unavailable; do not repeat execution",
          );
        },
      };
    },
...
export async function main(env = process.env) {
  ...
  if (!authority) {
    console.log("Not exercised: explicit failure/conflict authority required.");
    return;
  }
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  if (process.argv[2] === "--print-fixture") {
    const fixture = failureFixture(
      process.env.FACTORY_FAILURE_NAMESPACE,
      process.env.FACTORY_FAILURE_CASE,
    );
    console.log(
      JSON.stringify(
        {
          fixture,
          sha256: failureHash(JSON.stringify(fixture)),
          preparation:
            'Commit these exact files in the authorized disposable repository; retain its existing package.json test="node --test". This command writes nothing and does not activate Factory.',
        },
        null,
        2,
      ),
