...
description: Compiles a human-authored Objective (a GitHub Issue labelled `factory:objective`) into a validated Work Item graph conforming to schemas/objective.schema.json. Use when Director reads an Objective that has no Work Item graph yet, or when replanning needs to add or revise Work Items for an Objective already in flight.
---

# Objective compilation

This is Factory's "high-level compiler": the one place judgment about
...
skill is the cheapest point at which to prevent that. All `§` references are to `docs/DESIGN.md`.
...
JSON object. This becomes the input to `graph.ts`'s apply step: every
...
2. **Read the repository before you name a single path.** Call `read_repository_layout` on the
   ...
   A `scope` inferred from conventional structure in the Objective's prose can be wrong without
   failing here. It fails several steps later as an `untouched` verdict, after an agent run has been
   spent, and it reads like the agent ignored its brief rather than like the brief named a path that
   never existed. Checking is one cheap call; not checking is a defect that surfaces somewhere it
   cannot be diagnosed.
...
3. **Find independent seams.** Default to maximum independence. For a small decomposable Objective,
   2–3 *independent* Work Items is the target shape — bias toward splitting work so items do not depend on each other,
...
4. **Draft each Work Item's fields** (`schemas/work-item.schema.json`):
   ...
   - `acceptance` — criteria checkable **from the diff alone**. Outcome-evaluation (§5.2) verifies
     these against the PR, never against the agent's self-report (§11.7) — if a criterion can only
     ...
     agent's own habits. The executing coding agent may colocate a test file under `src/` instead
     of a described `test/` path. Prefer explicit, negatively-stated phrasing when the path is load-bearing:
     ...
     anywhere else"). Use stronger phrasing — an acceptance bullet reading `REQUIRED: a test file exists at exactly the
     ...
     test/, so a colocated test silently never runs"). State the constraint, its exact expected
     value, and why it matters; do not merely describe the desired end state. If the exact path
     genuinely does not matter (many test runners discover tests anywhere), do not assert one — a
     criterion the compiler doesn't actually need enforced is scope creep waiting to become a false
     escalation.
   - `scope` — concrete file or directory paths (trailing `/` for a directory), never glob
     patterns. A one-line scope constraint holds reliably in practice (`docs/PLATFORM-BEHAVIOR.md`);
     this field is
...
6. **Self-check before emitting.** `objective.schema.json` validates shape only; these graph-level
   ...
   - No two Work Items' `scope` entries overlap unless one explicitly `dependsOn` the other. An
     ...
     at merge" failure mode (§6, `docs/PLATFORM-BEHAVIOR.md`) — catching it at compile time is
     cheaper than discovering it as a merge conflict.

     This cost is not free even though §6 recovers from it. If three items overlap
     because each has to create the same `src/index.ts`, every one that loses the race has its pull
     request **closed and re-dispatched**, and each of those re-dispatches **consumes one of the
     item's three attempts** — the last item can burn two attempts before it ever gets a clean base.
     Four overlapping items can exhaust it and escalate, with correct-but-wasted work thrown away
     each time. So the rule is not a stylistic preference: an overlap you leave in is a budget you
     are spending on the item that merges last.
...
Example objective: *"add three pure functions, each with tests."*
...
All three items are independent (disjoint `scope`, empty `dependsOn`) — the intended shape for a
small decomposable Objective.
