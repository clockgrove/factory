...
import { encodeEventComment, latestSupportedRun, nextEventSequence } from "../control/receipts.js";

export const APPLICATION_OPERATIONS = [
  "doctor", "plan", "status", "explain", "activate", "pause", "resume", "drain",
  "cloud-pause", "retry", "priority", "replay", "cancel",
...
export interface ControllerInput { repository: string; checkout: string; requestId: string }

export interface ControllerLifecycleReceipt {
  operation: "start" | "stop" | "install" | "uninstall";
  repository: string;
  checkout: string;
  requestId: string;
  accepted: false;
...
export class PendingControllerLifecycle implements ControllerLifecycle {
  private receipt(operation: ControllerLifecycleReceipt["operation"], input: ControllerInput): Promise<ControllerLifecycleReceipt> {
    return Promise.resolve({ operation, ...input, accepted: false, status: "controller-implementation-pending" });
  }
  start(input: ControllerInput) { return this.receipt("start", input); }
  stop(input: ControllerInput) { return this.receipt("stop", input); }
  install(input: ControllerInput) { return this.receipt("install", input); }
  uninstall(input: ControllerInput) { return this.receipt("uninstall", input); }
}
...
export type CommandOperation = "pause" | "resume" | "drain" | "cloud-pause" | "retry" | "priority" | "replay" | "cancel";
...
export class FactoryApplicationService {
  ...
  async inspect(operation: ReadOperation, objective: number, workItem?: number): Promise<unknown> {
    ...
    return { operation, repository: `${this.context.owner}/${this.context.repo}`, objective: snapshot, ...(workItem ? { workItem } : {}) };
  }

  doctor(objective: number) { return this.inspect("doctor", objective); }
  plan(objective: number) { return this.inspect("plan", objective); }
  status(objective: number) { return this.inspect("status", objective); }
  explain(objective: number, workItem?: number) { return this.inspect("explain", objective, workItem); }
  replay(input: { objective: number; requestId: string; reason?: string }) {
    return this.command("replay", input);
  }

  async activate(input: { objective: number; requestId: string; baseSha?: string; policy?: unknown }): Promise<FactoryEvent> {
    ...
    return this.append(snapshot, input.requestId, {
      event: "ActivationRequested", repository: `${this.context.owner}/${this.context.repo}`,
      baseSha, policy, policyDigest: policyDigest(policy), controllerProtocolMin: PROTOCOL_V2,
      controllerProtocolMax: PROTOCOL_V2,
    });
  }

  async command(operation: CommandOperation, input: { objective: number; requestId: string; reason?: string; workItem?: number; priorityRank?: number }): Promise<FactoryEvent> {
    const snapshot = await this.context.reader.readObjective(input.objective);
    ...
    const event = operation === "pause" ? "RunPauseRequested"
      : operation === "resume" ? "RunResumeRequested"
      : operation === "drain" ? "RunDrainRequested"
      : operation === "cloud-pause" ? "CloudPauseRequested"
      : operation === "retry" ? "WorkItemRetryRequested"
      : operation === "priority" ? "WorkItemPriorityChanged"
      ...
      : operation === "replay" ? "RunResumeRequested"
      : "FactoryRunCancellationRequested";
    const activeRun = latestSupportedRun(snapshot.factoryEvents ?? []);
    if (operation === "cancel" && !activeRun) throw new Error(`Objective #${snapshot.number} has no Factory v2 run`);
    return this.append(snapshot, input.requestId, {
      event, ...(input.reason ? { reason: input.reason } : {}),
      ...
      ...(operation === "cancel" && activeRun ? { runId: activeRun.runId } : {}),
    });
  }

  controller(operation: "start" | "stop" | "install" | "uninstall", input: ControllerInput): Promise<unknown> {
    ...
    const fingerprint = JSON.stringify({ operation, repository: input.repository, checkout: input.checkout });
    const prior = FactoryApplicationService.controllerRequests.get(key);
    if (prior) {
      if (prior.fingerprint !== fingerprint) throw new Error(`idempotency key ${input.requestId} was already used for a different request`);
      return prior.receipt;
    }
    const receipt = lifecycle[operation](input);
    FactoryApplicationService.controllerRequests.set(key, { fingerprint, receipt });
    receipt.catch(() => {
  ...
  private allEvents(snapshot: ApplicationSnapshot): FactoryEvent[] {
    return [...(snapshot.factoryEvents ?? []), ...snapshot.workItems.flatMap(item => item.factoryEvents ?? [])];
  }

  private async append(snapshot: ApplicationSnapshot, requestId: string, fields: Record<string, unknown>): Promise<FactoryEvent> {
  ...
  private async appendLocked(snapshot: ApplicationSnapshot, requestId: string, fields: Record<string, unknown>): Promise<FactoryEvent> {
    const store = this.context.store;
    if (!store) throw new Error("this application service is read-only");
    const existing = this.allEvents(snapshot).find(event => "requestId" in event && event.requestId === requestId);
    if (existing) {
      const comparableKeys = ["event", "operation", "repository", "baseSha", "policyDigest", "workItem", "priorityRank", "reason"];
      const conflict = comparableKeys.some(key => (existing as unknown as Record<string, unknown>)[key] !== fields[key]);
      if (conflict) throw new Error(`idempotency key ${requestId} was already used for a different request`);
      return existing;
    }
    ...
    if (fields.event === "FactoryRunCancellationRequested") {
      ...
      if (
        ...
        throw new Error(`only activating actor ${activeStart.actor} may cancel this run`);
      }
    }
    ...
    const event = parseFactoryEvent({
      protocol: PROTOCOL_V2, kind: "run", event: fields.event, objective: snapshot.number,
      runId: typeof fields.runId === "string" ? fields.runId : run?.runId ?? requestId,
      sequence: nextEventSequence(this.allEvents(snapshot)),
      at: now.toISOString(), requestedBy: actor, requestId, ...fields,
    });
    await store.addIssueComment(snapshot.id, encodeEventComment(`Factory accepted ${String(fields.event)} from ${actor}.`, event));
    return event;
  }

  private static readonly queues = new Map<string, Promise<void>>();
  private static readonly controllerRequests = new Map<string, { fingerprint: string; receipt: Promise<unknown> }>();
  ...
  private async serialize<T>(objective: number, action: () => Promise<T>): Promise<T> {
    ...
    try {
      ...
      release();
      if (FactoryApplicationService.queues.get(key) === current) FactoryApplicationService.queues.delete(key);
    }
  }
}
