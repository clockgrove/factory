...
function usable(candidate: BackendCandidate, item: EconomicItem, policy: RunPolicy): boolean {
  return (
    ...
    candidate.id === candidate.capabilities.id &&
    candidate.permanentReasons.length === 0 &&
    candidate.transientReasons.length === 0 &&
    candidate.probe?.available === true &&
    candidate.probe.authenticated &&
...
function cloudEligibility(
  item: EconomicItem,
  evidence?: DecompositionEvidence,
): CloudEligibilityAssessment {
  const result = (
    status: CloudEligibilityAssessment["status"],
    reason: string,
    backendIds: string[] = [],
  ): CloudEligibilityAssessment => ({ workItem: item.id, status, reason, backendIds });
  ...
  if (!candidates)
    return result("unknown", "Per-item registry capability observations unavailable.");
  const eligible = candidates.filter(
    (candidate) =>
      allowed.includes(candidate.id) &&
      candidate.paid &&
      candidate.capabilities?.requiresPaidRuntime &&
      usable(candidate, item, policy),
  );
  if (eligible.length)
    return result(
      "eligible-in-principle",
      "Supplied execution policy and capability checks fit; validation, delivery, budget, burst and current authority still require admission.",
      eligible.map((candidate) => candidate.id).sort(),
    );
  const incomplete =
    allowed.some((id) => !candidates.some((candidate) => candidate.id === id)) ||
    candidates.some(
      (candidate) =>
        allowed.includes(candidate.id) &&
        candidate.permanentReasons.length === 0 &&
        (!candidate.probe?.available ||
          !candidate.probe.authenticated ||
          candidate.transientReasons.length > 0),
    );
  return incomplete
    ? result("unknown", "Paid capability observations are incomplete or temporarily unavailable.")
    : result("ineligible", "Supplied paid execution capabilities do not satisfy this packet.");
}

function localFit(
  waves: readonly EconomicItem[][],
  evidence?: DecompositionEvidence,
): LocalFitAssessment {
  const unavailable = (reason: string): LocalFitAssessment => ({
    availability: "unavailable",
    likelySlots: null,
    waveFits: waves.map(() => null),
    measuredAt: evidence?.resource?.measuredAt ?? null,
    reasons: [reason],
  });
  if (
    !evidence?.resource ||
    !evidence.capacity ||
    !evidence.repositoryLimits ||
    !evidence.deliveryMode ||
    !evidence.candidates
  )
    return unavailable(
      "Resource sample, capacity ledger, controller limits, delivery mode or per-item capabilities unavailable.",
    );
  ...
  if (
    ...
    ![
      resource.effectiveCpu,
      resource.totalMemoryMb,
      resource.availableMemoryMb,
      resource.loadRatio,
      resource.memoryUsageRatio,
    ].every((n) => Number.isFinite(n) && n >= 0) ||
    resource.effectiveCpu <= 0 ||
    resource.availableMemoryMb > resource.totalMemoryMb ||
    resource.memoryUsageRatio > 1
  )
  ...
  if (
    (evidence.nowMs !== undefined && !Number.isFinite(evidence.nowMs)) ||
  ...
  if ((evidence.cooldownUntilMs ?? 0) > (evidence.nowMs ?? 0))
    reasons.push("Observed local admission cooldown is active.");
  if (evidence.deliveryMode === "escalate")
    reasons.push("Selected delivery mode cannot execute work.");
  ...
  const waveFits = waves.map((wave): number | null => {
    ...
    if (
      wave.some(
        (item) =>
          !evidence.candidates!.has(item.id) ||
          policy.backendOrder.some(
            (id) => !evidence.candidates!.get(item.id)!.some((candidate) => candidate.id === id),
          ),
      )
    )
      return null;
    ...
    for (const item of [...wave].sort((a, b) => a.id.localeCompare(b.id))) {
      ...
      const candidates = evidence
        ...
        .filter(
          (candidate) =>
            policy.backendOrder.includes(candidate.id) &&
            candidate.local &&
      ...
      if (!Number.isFinite(cpu) || cpu <= 0 || !Number.isFinite(memoryMb) || memoryMb <= 0)
        return null;
      for (const candidate of candidates) {
        ...
        const identity = {
          ...
          workItem: itemNumbers.get(item.id)!,
  ...
  return {
    availability: waveFits.includes(null) ? "unavailable" : "estimated",
    likelySlots: waveFits.includes(null) ? null : Math.max(...(waveFits as number[])),
    waveFits,
    measuredAt: resource.measuredAt,
    reasons: [
      ...reasons,
      ...(waveFits.includes(null)
        ? [
            "At least one wave lacks complete per-item registry observations or valid resource requirements.",
          ]
        : []),
...
export function assessDecomposition(
  ...
  if (!redundantItemPairs.length)
    feedback.push(
      "Retain distinct acceptance and execution boundaries where they provide independently reviewable behavior; structural counts alone do not prove combining them is cheaper or safe.",
    );
  return {
