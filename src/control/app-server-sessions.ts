...
import { canonicalSessionJson, parseAppServerSessionCheckpoint, type AppServerSessionCheckpoint, type AppServerSessionStage } from "../execution/app-server-session.js";
...
export function appServerSessionRef(repository: string, reservation: AttemptReservation, stage: AppServerSessionStage): string {
  return `refs/clockgrove-factory/sessions/${durableAttemptId({ repository, ...reservation })}/${stage}`;
}
function assertReservation(repository: string, reservation: AttemptReservation, checkpoint: AppServerSessionCheckpoint): void {
  const binding = checkpoint.binding;
  if (reservation.backend !== "codex-app-server/local-worktree" || binding.repository !== repository.toLowerCase() ||
    binding.attemptId !== durableAttemptId({ repository, ...reservation }) || binding.baseSha !== reservation.baseSha ||
    binding.policyDigest !== reservation.policyDigest ||
    canonicalSessionJson(binding.localScopeBatch) !== canonicalSessionJson(reservation.localScopeBatch)) throw new Error("durable session differs from its immutable reservation");
}
...
export class AppServerSessionManager {
  constructor(private readonly store: CompiledGraphStore, private readonly leases: LeaseManager) {}
  async load(repository: string, reservation: AttemptReservation, stage: AppServerSessionStage): Promise<AppServerSessionCheckpoint | null> {
    ...
    if (!prepared || canonicalSessionJson(prepared.binding) !== canonicalSessionJson(checkpoint.binding))
    ...
    if (stage === "terminal") {
      ...
      if (!turn || turn.turnId !== checkpoint.turnId || canonicalSessionJson(turn.binding) !== canonicalSessionJson(checkpoint.binding))
        throw new Error("durable terminal checkpoint lacks its exact dispatch");
    }
    return checkpoint;
  }
  private async readStage(repository: string, reservation: AttemptReservation, stage: AppServerSessionStage): Promise<AppServerSessionCheckpoint | null> {
    ...
    if (commit.oid !== oid || commit.parentOids.length !== 1 || commit.parentOids[0] !== reservation.oid)
    ...
    if (bytes.length > MAX_BYTES || createHash("sha1").update(`blob ${bytes.length}\0`).update(bytes).digest("hex") !== blob)
  ...
  async persist(args: { repository: string; reservation: AttemptReservation; lease: LeaseState; checkpoint: AppServerSessionCheckpoint }): Promise<void> {
    ...
    if (args.lease.objective !== args.reservation.objective || args.lease.runId !== args.reservation.runId ||
      args.lease.policyDigest !== args.reservation.policyDigest || args.lease.epoch < args.reservation.directorEpoch)
    ...
    if (bytes.length > MAX_BYTES) throw new Error("durable session checkpoint exceeds its byte bound");
    const existing = await this.load(args.repository, args.reservation, checkpoint.stage);
    if (existing) {
      if (canonicalSessionJson(existing) !== bytes.toString()) throw new Error("durable session already has a conflicting immutable checkpoint");
      return;
    }
    ...
    await this.leases.assertCurrent(args.lease);
    const treeOid = await this.store.createTree({ entries: [{ path: PATH, mode: "100644", type: "blob", sha: blob }] });
    await this.leases.assertCurrent(args.lease);
    const commit = await this.store.createCommit({ treeOid, parentOids: [args.reservation.oid],
      message: `Factory App Server ${checkpoint.stage}\n\nFactory-Session-Digest: ${createHash("sha256").update(bytes).digest("hex")}` });
    await this.leases.assertCurrent(args.lease);
    try {
      if (await this.store.createRef(appServerSessionRef(args.repository, args.reservation, checkpoint.stage), commit)) return;
    } catch (error) {
    ...
    if (!winner || canonicalSessionJson(winner) !== bytes.toString()) throw new Error("durable session checkpoint creation conflicted");
  }
}
