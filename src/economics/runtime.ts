...
export interface RuntimeEconomics {
  ...
  consumptionByDeliveryOutcome: EvidenceMetric<
    Array<{
  ...
  interventions: EvidenceMetric<{
    ...
    entries: Array<{
      ...
      workItem?: number;
      reason:
        | { availability: "observed"; code: string | null; sha256: string }
...
const unavailable = (reason: string): EvidenceMetric<never> => ({
  availability: "unavailable",
  reason,
});
...
const capacityKey = (event: Capacity) =>
  JSON.stringify([
    key(event),
    event.phase,
    event.backend,
    event.sourceRunId ?? null,
    event.targetBaseSha ?? null,
  ]);
function location(backend: string, admissionClass?: Attempt["admissionClass"]): Location {
  ...
  const known =
    localBackends.has(backend) || /^factory\/integration-validation-[a-f0-9]{64}$/.test(backend)
      ? "local"
      : cloudBackends.has(backend) || /^factory\/integration-sandbox-[a-f0-9]{64}$/.test(backend)
        ? "cloud"
        : "unclassified";
  if (
    admissionClass !== undefined &&
    known !== "unclassified" &&
    (admissionClass === "local") !== (known === "local")
  )
    return "unclassified";
  return known;
}
function counts(values: readonly Location[]): EvidenceMetric<Locations> {
  if (!values.length)
    return unavailable(
      "no matching invocation receipts are present; missing evidence is not zero activity",
    );
...
function uniqueStart(
  values: Attempt[],
  event: "AttemptReserved" | "AttemptStarted",
): Attempt | undefined {
  ...
  if (
    ...
    selected.some(
      (value) =>
        !sameAttempt(first, value) ||
        value.providerResourceId !== first.providerResourceId ||
        value.resourceHostIdentity !== first.resourceHostIdentity ||
        value.environmentIdentity !== first.environmentIdentity ||
        value.admissionClass !== first.admissionClass ||
        value.requestedCpu !== first.requestedCpu ||
        value.requestedMemoryMb !== first.requestedMemoryMb ||
        JSON.stringify(value.localScopeBatch) !== JSON.stringify(first.localScopeBatch),
    )
  )
    return undefined;
  return first;
...
export function summarizeRuntimeEconomics(
  ...
  const events = deduplicateFactoryEvents([...input])
    .filter((event) => event.runId === runId)
    .sort((a, b) => a.sequence - b.sequence);
  const startMs = Date.parse(startedAt),
    endMs = finishedAt === undefined ? undefined : Date.parse(finishedAt);
  const interval = (
    start: FactoryEvent,
    end: FactoryEvent,
    where: Location,
    ...
    if (
      ...
      (endMs !== undefined && b > endMs)
    )
      return undefined;
    return { start: a, end: b, location: where };
  };
  const attempts = new Map<string, Attempt[]>();
  const conservativelySettled = new Set(
    events
      .filter(
        (event) => event.kind === "budget" && event.usageEvidence === "conservative-reservation",
      )
      .map(key),
  );
  for (const event of events)
    if (event.kind === "attempt") {
      ...
      group.push(event);
      attempts.set(key(event), group);
    }
  const starts: Attempt[] = [],
    executionIntervals: Interval[] = [],
    capacityIntervals: Interval[] = [];
  let unresolvedExecution = 0,
    unresolvedCapacity = 0,
    executionReservations = 0;
  const validationReservations = events.filter(
    (event): event is Capacity =>
      event.kind === "capacity" &&
  ...
  for (const group of attempts.values()) {
    ...
    const terminal =
      started &&
      group.find(
        (event) => event.sequence > started.sequence && executionTerminals.has(event.event),
      );
    const active =
      started &&
      terminal &&
      !conservativelySettled.has(key(started)) &&
      sameAttempt(started, terminal) &&
      !(
    ...
    if (active) executionIntervals.push(active);
      ...
      group.some((event) => event.event === "AttemptStarted" || executionTerminals.has(event.event))
    )
      unresolvedExecution++;
    if (!reserved) {
      unresolvedCapacity++;
    ...
    const releases: FactoryEvent[] = [
      ...group.filter(
        (event) => capacityTerminals.has(event.event) && sameAttempt(reserved, event),
      ),
      ...validationReservations.filter(
        (event) =>
          key(event) === key(reserved) &&
          event.sourceRunId === undefined &&
          event.directorEpoch === reserved.directorEpoch &&
          event.policyDigest === reserved.policyDigest,
      ),
    ]
      .filter((event) => event.sequence > reserved.sequence)
      .sort((a, b) => a.sequence - b.sequence);
    const occupied =
      !conservativelySettled.has(key(reserved)) &&
      releases[0] &&
      interval(reserved, releases[0], location(reserved.backend, reserved.admissionClass));
    if (occupied) capacityIntervals.push(occupied);
    else unresolvedCapacity++;
  }
  ...
  let validationCompleted = 0,
    validationUnresolved = 0;
  for (const event of events) {
    if (event.kind !== "capacity" || event.phase !== "validation") continue;
    const identity = capacityKey(event),
      prior = open.get(identity);
    if (event.event === "CapacityReserved") {
      if (prior) {
        ...
        if (
          ...
          JSON.stringify(event.localScopeBatch) !== JSON.stringify(prior.receipt.localScopeBatch) ||
          event.directorEpoch !== prior.receipt.directorEpoch ||
          event.policyDigest !== prior.receipt.policyDigest ||
          event.recoveryEpoch !== prior.receipt.recoveryEpoch ||
          event.requestedCpu !== prior.receipt.requestedCpu ||
          event.requestedMemoryMb !== prior.receipt.requestedMemoryMb
        )
    ...
    if (occupied) {
      capacityIntervals.push(occupied);
      validationCompleted++;
    } else validationUnresolved++;
  ...
  const unsupportedExecutionCapacityReceipts = events.filter(
    (event) => event.kind === "capacity" && event.phase === "execution",
  ).length;
  const validations = events.filter(
    (event): event is Extract<FactoryEvent, { kind: "validation" }> => event.kind === "validation",
  );
  const missingValidationAdmissions = validations.filter(
    (event) =>
      !validationReservations.some(
        (reserved) => key(reserved) === key(event) && reserved.sequence < event.sequence,
      ),
  ).length;
  const localIntervals = capacityIntervals.filter((value) => value.location === "local");
  const localTime = sweep(localIntervals),
    activeTime = sweep(executionIntervals);
  const unknownLocations = capacityIntervals.some((value) => value.location === "unclassified");
  const finiteWindow =
    endMs !== undefined && Number.isFinite(startMs) && Number.isFinite(endMs) && endMs > startMs;
  ...
  for (const started of starts) {
    const earlier = starts.filter(
      (prior) =>
        prior.workItem === started.workItem &&
        prior.attempt < started.attempt &&
        prior.sequence < started.sequence,
    );
    if (earlier.length) {
      const last = earlier.at(-1)!;
      if (
        (attempts.get(key(last)) ?? []).some(
          (event) =>
            sameAttempt(last, event) &&
  ...
  const deferred = [...attempts.values()].filter((group) =>
  ...
  for (const value of validations) {
    ...
    const artifacts = new Set(
      (attempts.get(key(value)) ?? [])
        .filter(
          (event) =>
            event.event === "AttemptCollected" &&
            event.sequence < value.sequence &&
            event.baseSha === value.baseSha &&
            event.artifactDigest,
        )
        .map((event) => event.artifactDigest!),
    );
    if (artifacts.size === 1) rejected.add(JSON.stringify([key(value), [...artifacts][0]]));
    else unboundFailures++;
  }
  ...
  for (const value of usage.values()) {
    ...
    const row = consumption.get(consumptionKey) ?? {
      unit: value.unit,
      usageEvidence,
      integrated: 0,
      unintegratedAtTerminalRun: 0,
    ...
    if (
      ...
      const candidates = validationReservations.filter(
        (reserved) =>
          reserved.workItem === value.workItem &&
          reserved.sourceRunId !== undefined &&
          reserved.backend === `factory/${value.usageId}` &&
          reserved.sequence < value.sequence,
      );
      const identities = new Set(
        candidates.map((reserved) =>
          JSON.stringify([reserved.sourceRunId, reserved.attempt, reserved.targetBaseSha]),
        ),
      );
      const source = candidates[0];
      if (identities.size === 1 && source) {
        bound = true;
        integrated = events.some(
          (outcome) =>
            outcome.event === "RecoverySourceIntegrated" &&
            outcome.workItem === value.workItem &&
            outcome.sourceRunId === source.sourceRunId &&
            outcome.sourceAttempt === source.attempt &&
            outcome.mergeCandidateIdentityDigest ===
              value.usageId!.slice("integration-validation-".length),
        );
      }
    }
    const category = !bound
      ? "unattributed"
      : integrated
        ? "integrated"
        : finishedAt
          ? "unintegratedAtTerminalRun"
          : "unresolvedDelivery";
    row[category] += value.amount;
    row.usageIdentities++;
    consumption.set(consumptionKey, row);
  }
  const withModel = starts.filter((started) =>
    [...usage.values()].some(
      (value) =>
  ...
  const finiteConsumption = [...consumption.values()].every((row) =>
    [row.integrated, row.unintegratedAtTerminalRun, row.unresolvedDelivery, row.unattributed].every(
      Number.isFinite,
    ),
  );
  const interventionEvents = events.filter(
    (event) => operatorEvents.has(event.event) || event.event === "FactoryRunEscalated",
  );
    ...
    reason:
      "reason" in event && typeof event.reason === "string"
        ? {
            ...
            code: queuedReasonCode(event.reason),
  ...
  return {
    ...
    execution: {
      ...
      completeIntervals: executionIntervals.length,
      unresolvedOrConflictingAttempts: unresolvedExecution,
      unresolvedCapacityIntervals: unresolvedCapacity,
      unsupportedExecutionCapacityReceipts,
      closedIntervalTime: executionIntervals.length
        ? observed(
            {
              summedWorkerMilliseconds: activeTime.sum,
              wallMillisecondsWithAnyWorker: activeTime.union,
              method:
                "AttemptStarted-to-first-execution-terminal; closed intervals only; GitHub observation time, not CPU time",
            },
            executionIntervals.length * 2,
          )
        : unavailable("no complete bound execution intervals"),
    },
    validation: {
      admittedInvocations: counts(validationLocations),
      completedCapacityIntervals: validationCompleted,
      unresolvedOrConflictingInvocations: validationUnresolved,
      validationResultsWithoutAdmissionEvidence: missingValidationAdmissions,
    },
    admittedLocalConcurrency:
      finiteWindow &&
      executionReservations + validationLocations.length > 0 &&
      unresolvedCapacity === 0 &&
      unsupportedExecutionCapacityReceipts === 0 &&
      validationUnresolved === 0 &&
      missingValidationAdmissions === 0 &&
      !unknownLocations
        ? observed(
            {
              peak: localTime.peak,
              average: localTime.sum / (endMs! - startMs),
              localReservationMilliseconds: localTime.sum,
              denominatorMilliseconds: endMs! - startMs,
              windowStart: startedAt,
              windowEnd: finishedAt!,
              intervals: localIntervals.length,
              method:
                "time-weighted execution-and-validation reservations over the terminal run window; idle time included",
            },
            capacityIntervals.length * 2,
          )
        : unavailable(
            "terminal run window and complete classified execution/validation capacity intervals are required; missing or conflicting receipts are not zero occupancy",
          ),
    retries: starts.length
      ? observed(
          {
            observedStartsAfterFailedAttempt: retries,
            otherAdditionalStarts: additionalStarts,
            startsWithMissingEarlierAttemptEvidence: missingEarlier,
          },
          starts.length,
        )
      : unavailable("no actual worker starts are recorded"),
    deferredAttempts: attempts.size
      ? observed(deferred.length, attempts.size)
      : unavailable("no attempt evidence"),
    rejectedValidationArtifacts: validations.length
      ? observed(
          {
            distinctArtifacts: rejected.size,
            failedResults,
            failedResultsWithoutArtifactBinding: unboundFailures,
          },
          seenValidation.size,
        )
      : unavailable("no independent validation results; rejection is not physical disposal"),
    consumptionByDeliveryOutcome:
      usage.size && finiteConsumption
        ? observed(
            [...consumption.values()].sort((a, b) => a.unit.localeCompare(b.unit)),
            usage.size,
          )
        : unavailable(
            "finite execution/validation usage receipts required; no zero consumption inferred",
          ),
    executionModelUsageCoverage: {
    ...
    interventions: interventionEvents.length
      ? observed(
          {
            operatorRequests: interventionEvents.filter((event) => operatorEvents.has(event.event))
              .length,
            escalationBoundaries: interventionEvents.filter(
              (event) => event.event === "FactoryRunEscalated",
            ).length,
            entries,
            omittedEntries: interventionEvents.length - entries.length,
          },
          interventionEvents.length,
        )
      : unavailable(
          "no operator-request or escalation receipts; actual human effort is not measured",
        ),
    actualArtifactDisposal: unavailable(
      "no durable physical-disposal receipt; rejected or unintegrated artifacts may remain reusable",
    ),
    modelAuthoredYield: unavailable(
      "no durable authored/generated byte or line attribution; generated output cannot be counted as authored yield",
    ),
    measuredSavings: unavailable(
      "no controlled comparison or counterfactual runtime measurement; overlap is not demonstrated savings",
    ),
  };
}
