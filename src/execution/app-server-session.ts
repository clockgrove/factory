...
const Usage = z
  ...
  .strict();
export const AppServerRawTokensSchema = z
  .object({
    totalTokens: counter,
    inputTokens: counter,
    outputTokens: counter,
    cachedInputTokens: counter,
    cacheWriteInputTokens: counter,
    reasoningOutputTokens: counter,
  })
  .strict();
export const AppServerResponseUsageSchema = z
  .object({ responseId: text, usage: AppServerRawTokensSchema.nullable() })
  .strict();
export const AppServerSessionBindingSchema = z
  .object({
  ...
  .strict();
const SessionCheckpointSchema = z
  .object({
    protocol: z.literal(APP_SERVER_SESSION_PROTOCOL),
    stage: z.enum(APP_SERVER_SESSION_STAGES),
    ...
    rawTokenUsage: z
      .object({ total: AppServerRawTokensSchema, last: AppServerRawTokensSchema })
      .strict()
      .optional(),
...
export function canonicalSessionJson(value: unknown): string {
  ...
  if (value !== null && typeof value === "object")
    return `{${Object.entries(value)
      ...
      .map(([key, item]) => `${JSON.stringify(key)}:${canonicalSessionJson(item)}`)
      .join(",")}}`;
  ...
  return createHash("sha256")
    .update(
      canonicalSessionJson({
        ...
        policyNetworkDestinations: context.policyNetworkDestinations ?? null,
...
export function parseAppServerSessionCheckpoint(input: unknown): AppServerSessionCheckpoint {
  ...
  if (
    workerPacketDigest(packet) !== binding.packetDigest ||
    packet.baseSha !== binding.baseSha ||
    binding.attemptId !== durableAttemptId(binding) ||
    binding.repository !== binding.repository.toLowerCase() ||
    (value.stage !== "prepared" && !value.turnId) ||
    (value.stage === "prepared" && value.turnId) ||
    (value.stage === "terminal") !== Boolean(value.state) ||
    (value.stage === "terminal") !== Boolean(value.providerStatus) ||
    (value.stage !== "terminal" &&
      (value.usage ||
        value.rawTokenUsage ||
        value.responseUsage ||
        value.final ||
        value.usageStreamComplete !== undefined)) ||
    ...
    throw new Error("durable App Server checkpoint binding is invalid");
  const scope = binding.localScopeBatch;
  if (
    scope.commandCount !== 1 ||
    scope.identity.phase !== "execution" ||
    scope.identity.commandIndex !== 0 ||
    scope.identity.repository !== binding.repository ||
    scope.identity.runId !== binding.runId ||
    scope.identity.objective !== binding.objective ||
    scope.identity.workItem !== binding.workItem ||
    scope.identity.attempt !== binding.attempt ||
    scope.identity.directorEpoch !== binding.directorEpoch ||
    scope.identity.policyDigest !== binding.policyDigest ||
    scope.identity.hostIdentity !== binding.hostIdentity ||
    scope.identity.invocationDigest !== binding.packetDigest ||
    scope.deadline !== binding.deadline
  )
    throw new Error("durable App Server scope does not bind its exact worker invocation");
  if (
    value.usage &&
    canonicalSessionJson(value.usage) !==
      canonicalSessionJson(
        completedAppServerUsage({
          completed: value.providerStatus === "completed",
          baseline: binding.usageBaseline,
          total: value.rawTokenUsage?.total,
          responses: value.responseUsage ?? [],
          streamComplete: value.usageStreamComplete === true,
        }),
      )
  )
...
export function completedAppServerUsage(args: {
  completed: boolean;
  baseline: AppServerRawTokens;
  total: AppServerRawTokens | undefined;
  responses: AppServerResponseUsage[];
  ...
  if (
    ...
    !args.streamComplete ||
    !args.total ||
    !args.responses.length ||
    args.responses.length > 1000
  )
    return undefined;
  ...
  for (const name of Object.keys(sums) as Array<keyof AppServerRawTokens>) {
    const baseline = args.baseline[name],
      total = args.total[name];
    if (
      baseline === null ||
      total === null ||
      ...
      total < baseline ||
      total - baseline !== sums[name]
    )
      return undefined;
  }
  if (
    !Number.isSafeInteger(sums.inputTokens! + sums.outputTokens!) ||
    sums.cachedInputTokens! > sums.inputTokens!
  )
    return undefined;
  return {
...
export function assertAppServerSessionContext(
  context: AttemptContext,
  binding: AppServerSessionBinding,
): void {
  if (
    binding.attemptId !== durableAttemptId(context) ||
    binding.policyDigest !== context.policyDigest ||
    binding.baseSha !== context.packet.baseSha ||
    binding.packetDigest !== workerPacketDigest(context.packet) ||
    binding.boundaryDigest !== appServerBoundaryDigest(context) ||
    binding.workspace !== context.workspace ||
    binding.deadline !== context.deadline.toISOString()
  )
    throw new Error("durable App Server session does not match the authorized attempt");
}
