...
import { retainArtifactContent } from "./artifact-content.js";
import type { NormalizedArtifact } from "./artifacts.js";

const owners = new AsyncLocalStorage<Map<string, () => Promise<void>>>();
...
export function retainScopedArtifact(artifact: NormalizedArtifact): NormalizedArtifact {
  ...
  if (!scope.has(artifact.payload.digest))
    scope.set(artifact.payload.digest, retainArtifactContent(artifact.payload));
  return artifact;
}
...
export async function withArtifactContentScope<T>(operation: () => Promise<T>): Promise<T> {
  return owners.run(new Map(), async () => {
    ...
    const settled = await Promise.allSettled([...owners.getStore()!.values()].map((release) => release()));
    const failures = settled.flatMap((entry) => entry.status === "rejected" ? [entry.reason] : []);
    if (failures.length)
      throw new AggregateError([...(result.ok ? [] : [result.error]), ...failures], "artifact content owner cleanup failed");
