...
export const ArtifactPathSchema = z.string().min(1).max(500).refine((path) =>
  !path.startsWith("/") && !path.includes("\\") && !/[\u0000-\u001f\u007f]/.test(path) &&
  path.split("/").every((part) => part !== "." && part !== ".." && part !== "" && part.toLowerCase() !== ".git"),
"unsafe artifact path");
export const ContentChunkSchema = z.object({ digest: sha256Digest, bytes: z.number().int().positive().max(CONTENT_CHUNK_BYTES) }).strict();
export const ArtifactPayloadSchema = z.object({ kind: z.literal("git-patch-chunks-v1"),
  digest: sha256Digest, bytes: z.number().int().positive().max(MAX_CONTENT_BYTES),
  chunks: z.array(ContentChunkSchema).min(1).max(MAX_CONTENT_BYTES / CONTENT_CHUNK_BYTES),
}).strict().superRefine((value, context) => {
  if (value.chunks.reduce((sum, chunk) => sum + chunk.bytes, 0) !== value.bytes ||
      value.chunks.slice(0, -1).some((chunk) => chunk.bytes !== CONTENT_CHUNK_BYTES))
    context.addIssue({ code: z.ZodIssueCode.custom, message: "payload chunk sizes do not match total" });
});
export type ArtifactPayload = z.infer<typeof ArtifactPayloadSchema>;
const MediaSchema = z.enum(["image/png", "image/jpeg", "image/gif", "image/webp", "application/pdf", "application/wasm", "application/zip", "audio/wav", "video/mp4", "unknown"]);
export const ArtifactFileSchema = z.object({ path: ArtifactPathSchema,
  action: z.enum(["write", "delete"]), mode: z.enum(["100644", "100755"]),
  bytes: z.number().int().nonnegative().max(MAX_CONTENT_FILE_BYTES), digest: sha256Digest,
  mediaType: MediaSchema, generated: z.boolean(),
}).strict().superRefine((value, context) => {
  if (value.action === "delete" && (value.bytes !== 0 || value.digest !== sha256(Buffer.alloc(0)) || value.mediaType !== "unknown"))
    context.addIssue({ code: z.ZodIssueCode.custom, message: "deleted file must have canonical empty content identity" });
});
export const ArtifactFileManifestSchema = z.object({ version: z.literal(1), baseTreeSha: gitSha,
  resultTreeSha: gitSha, files: z.array(ArtifactFileSchema).max(MAX_CONTENT_FILES),
}).strict().superRefine((value, context) => {
  if (new Set(value.files.map((file) => file.path)).size !== value.files.length ||
    ...
    context.addIssue({ code: z.ZodIssueCode.custom, message: "duplicate paths or excessive total file bytes" });
});
...
export function knownMediaType(prefix: Buffer): z.infer<typeof MediaSchema> {
  ...
  if (prefix.subarray(0,8).equals(Buffer.from([0,97,115,109,1,0,0,0]))) return "application/wasm";
...
export async function regularContentPath(root: string, relative: string): Promise<string> {
  ...
  if (!rootInfo.isDirectory() || rootInfo.isSymbolicLink()) throw new Error("content root is not a real directory");
...
export async function inspectContentFile(path: string, maxBytes = MAX_CONTENT_FILE_BYTES) {
  ...
  try {
    ...
    if (!before.isFile() || before.size > maxBytes) throw new Error("content file exceeds byte limit or is not regular");
    ...
    return { bytes, digest: hash.digest("hex"), mediaType: knownMediaType(prefix), mode: (before.mode & 0o111 ? "100755" : "100644") as "100644" | "100755" };
...
export async function cachePayload(path: string): Promise<ArtifactPayload> {
  ...
  try {
    ...
    const payload = ArtifactPayloadSchema.parse({ kind: "git-patch-chunks-v1", digest: identity.digest, bytes: identity.bytes, chunks });
...
export async function readContentChunk(chunk: z.infer<typeof ContentChunkSchema>): Promise<Buffer> {
  ...
  if (!path) throw new Error(`artifact content unavailable locally; recover immutable transfer ${chunk.digest}`);
  const info = await inspectContentFile(path, CONTENT_CHUNK_BYTES);
  if (info.digest !== chunk.digest || info.bytes !== chunk.bytes) throw new Error("cached artifact chunk identity mismatch");
  const bytes = await readFile(path);
  if (bytes.length !== chunk.bytes || sha256(bytes) !== chunk.digest) throw new Error("cached artifact chunk changed");
  return bytes;
}
...
export async function materializePayload(payload: ArtifactPayload, destination: string): Promise<void> {
  ...
  try {
    ...
    if (bytes !== payload.bytes || hash.digest("hex") !== payload.digest) throw new Error("reassembled artifact digest mismatch");
  } catch (error) {
    await file.close();
...
export async function verifyPayload(payload: ArtifactPayload): Promise<void> {
  ...
  if (bytes !== payload.bytes || hash.digest("hex") !== payload.digest) throw new Error("payload content identity mismatch");
}

export async function cleanupContentRoot(root: string): Promise<void> {
  if (!ownedRoots.has(root) || !resolve(root).startsWith(`${resolve(tmpdir())}${sep}factory-content-`))
...
export async function releasePayload(payload: ArtifactPayload): Promise<void> {
  const roots = new Set(payload.chunks.map((chunk) => cachedChunks.get(chunk.digest)).filter((path): path is string => Boolean(path)).map(dirname));
  for (const root of roots) await cleanupContentRoot(root);
}

/** Revalidate full bytes after application, not only a worker/provider-supplied manifest. */
export async function verifyMaterializedFiles(root: string, manifest: ArtifactFileManifest): Promise<void> {
  ArtifactFileManifestSchema.parse(manifest);
  for (const file of manifest.files) {
    if (file.action === "delete") {
      try { await lstat(join(root, file.path)); } catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") continue; throw error; }
    ...
    if (observed.bytes !== file.bytes || observed.digest !== file.digest || observed.mode !== file.mode || observed.mediaType !== file.mediaType)
...
export async function copyBoundedContent(source: string, destination: string, maximum = MAX_CONTENT_BYTES): Promise<void> {
  const info = await inspectContentFile(source, maximum);
  await mkdir(dirname(destination), { recursive: true });
  let bytes = 0;
  await pipeline(createReadStream(source, { flags: constants.O_RDONLY | constants.O_NOFOLLOW }), new Transform({ transform(chunk, _encoding, callback) {
    bytes += chunk.length;
  ...
  if (copied.digest !== info.digest || copied.bytes !== info.bytes) throw new Error("stream copy identity mismatch");
}
