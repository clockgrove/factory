...
function assertNoContentSecrets(text: string): void {
  assertNoSecretMaterial(text, "artifact content");
  // Binary/generated bytes have no reliable word delimiters. Keep the structured
  // record scanner and additionally recognize credential signatures embedded in
  // adjacent bytes, including signatures split across the streaming read boundary.
  if (
    /(?:gh[opurs]_[A-Za-z0-9_]{20,}|github_pat_[A-Za-z0-9_]{20,}|sk-[A-Za-z0-9_-]{20,}|(?:AKIA|ASIA)[A-Z0-9]{16})/.test(
      text,
    )
  )
    throw new Error("artifact content contains suspected credential bytes");
}
