...
export function renderCompilerProposalPrompt(
  ...
  return [
    ...
    "Every Work Item acceptance criterion must be decidable from that pinned candidate artifact, its allowed paths, and its declared validation evidence before publication. Represent root, middle, top, dependency count, sibling or join absence, delivery-group position or continuity, and all other graph or delivery topology only with dependsOn plus the advertised exact-graph-admission capability, never in acceptance criteria. Split a mixed artifact-and-topology clause: retain only the artifact-decidable behavior as a criterion, express topology through dependsOn, and bind the topology obligation to exact-graph-admission.",
...
export class CodexCliManagementBackend implements ManagementBackend {
  ...
  async judgePlan(
    ...
    const prompt = [
      ...
      "Require every Work Item acceptance criterion to be decidable from that pinned candidate artifact, its allowed paths, and its declared validation evidence before publication. A criterion must not ask artifact review to prove root, middle, top, dependency count, sibling or join absence, delivery-group position or continuity, or any other graph or delivery topology. Require topology to be represented by the exact dependsOn graph and the advertised exact-graph-admission capability. Return a blocking acceptance-quality repair when a criterion contains topology; when artifact behavior and topology share one clause, require the repair to split them without dropping either obligation.",
