...
import { assertArtifactScope, materializeArtifactPatch, verifyArtifact } from "../execution/artifacts.js";
import { assertFilesystemArtifactManifest, verifyMaterializedFiles } from "../execution/artifact-content.js";
...
export async function withVerifiedReviewCheckout<T>(
  ...
  if (input.requiresIsolation || input.packet.requirements.trust !== "trusted_local")
    throw new Error("semantic review requires isolated management execution; the local read-only management backend cannot provide that boundary");
  const artifact = verifyArtifact(input.artifact);
  verifyValidationEvidence(input.evidence);
  if (!input.evidence.passed || artifact.outcome !== "succeeded" ||
  ...
  assertNoSecretMaterial({ patch: artifact.patch, logs: artifact.logs }, "semantic review artifact");
  const worktree = await materializePinnedCompilationTree(input.repository, artifact.baseSha, { purpose: "worktree" });
  const deadline = Date.now() + 120_000;
  const git = async (args: string[]) => {
    const remaining = deadline - Date.now();
    if (remaining <= 0) throw new Error("semantic review materialization deadline expired");
    const result = await runContainedProcess({
      command: "git", args: ["--no-optional-locks", "--no-replace-objects", "-c", "core.hooksPath=/dev/null", "-c", "core.fsmonitor=false", ...args],
      cwd: worktree.path, env: pinnedGitEnvironment(sanitizedWorkerEnvironment(process.env)),
      timeoutMs: remaining, maxOutputBytes: 256 * 1024,
    });
  ...
  try {
    ...
    const manifest = await inspectPatchManifest(worktree.path, artifact.baseSha, patchPath, artifact.changedPaths);
    ...
    const changed = (await git(["diff", "--cached", "--name-only", "-z"])).split("\0").filter(Boolean).sort();
    ...
    if (tree !== input.evidence.outputTreeSha || manifest.resultTreeSha !== tree ||
