...
export function assertPeerActivation(
  ...
  const withdrawn = activationCancellation(events, {
...
async function assertAdoption(
  ...
  const request = one(
    events.filter(
      (event) =>
        event.event === "RecoveryRequested" && event.requestId === planRecord.plan.requestId,
    ),
  );
  const predecessor = one(
    events.filter(
      (event) =>
        event.event === "FactoryRunStarted" && event.runId === planRecord.plan.predecessor.runId,
    ),
  );
  requirePeer(
...
export async function verifyRecoveryPeerTrunkIntegration(input: {
  ...
  if (!receiverStart.recoveryPlanDigest) {
    ...
    const compiled = one(
      receiverEvents.filter(
        (event) => event.event === "GraphCompiled" && event.runId === receiverStart.runId,
      ),
    );
    requirePeer(
      compiled.event === "GraphCompiled" &&
        compiled.sequence > receiverStart.sequence &&
        compiled.baseSha === receiverStart.baseSha &&
        compiled.graphRef === graph.ref &&
        compiled.graphBlobSha === graph.blobOid &&
        compiled.graphDigest === graph.graphDigest &&
        compiled.graphSize === graph.graphSize &&
        graph.objective.workItems.every((item) => item.baseSha === receiverStart.baseSha),
    );
    const graphCommit = await store.readCommit(graph.commitOid);
    requirePeer(
      graphCommit.parentOids.length === 1 && graphCommit.parentOids[0] === receiverStart.baseSha,
    );
    ...
    assertSnapshotMatchesCompiledGraph(
      graph.objective,
      {
        workItems: receiver.workItems.map((item) => {
  ...
  let result:
    | { parent: string; requiresIsolation: boolean; executionRequiresIsolation: boolean }
    | undefined;
  for (const number of numbers.filter((number) => number !== receiver.number)) {
    ...
    const integrations = events.filter(
      (event) =>
        ...
        (event.event === "RecoverySourceIntegrated" &&
          event.mergeCommitSha === input.targetBaseSha),
    );
    ...
    requirePeer(
      start.event === "FactoryRunStarted" &&
        ...
        start.baseBranch === receiverStart.baseBranch &&
        start.fork === receiverStart.fork &&
        integrated.sequence > start.sequence &&
        time(integrated) >= time(start) &&
        time(integrated) <= beforeAt,
    );
    ...
    const runtime = start.recoveryPlanDigest
      ? await loadRecoveryRuntime({
          ...
          runId: start.runId,
          store,
          readSnapshot: async () => ({ snapshot: peer, historyComplete: true }),
        })
      : undefined;
    ...
    const graph =
      runtime?.status === "verified"
        ? runtime.graph
        : await loadCompiledGraph(store, number, start.runId);
    requirePeer(graph);
    const projection =
      runtime?.status === "verified"
        ? runtime.projection
    ...
    if (!runtime) {
      const compiled = one(
        events.filter((event) => event.event === "GraphCompiled" && event.runId === start.runId),
      );
      requirePeer(
        compiled.event === "GraphCompiled" &&
          compiled.sequence > start.sequence &&
          compiled.sequence < integrated.sequence &&
          compiled.baseSha === start.baseSha &&
          compiled.graphRef === graph.ref &&
          compiled.graphBlobSha === graph.blobOid &&
          compiled.graphDigest === graph.graphDigest &&
          compiled.graphSize === graph.graphSize &&
      ...
      const graphCommit = await store.readCommit(graph.commitOid);
      requirePeer(
        graphCommit.parentOids.length === 1 && graphCommit.parentOids[0] === start.baseSha,
      );
    ...
    let executionRequiresIsolation =
      policy.trust === "sandbox_untrusted" || packet.requirements.trust !== "trusted_local";
    ...
    if (integrated.event === "RecoverySourceIntegrated") {
      ...
      requirePeer(
        proof.targetBaseSha === merge.parentOids[0] && proof.outputTreeSha === merge.treeOid,
      );
      const original = one(
        events.filter(
          (event) => event.event === "FactoryRunStarted" && event.runId === integrated.sourceRunId,
        ),
      );
      requirePeer(original.event === "FactoryRunStarted");
      executionRequiresIsolation ||= original.policy.trust === "sandbox_untrusted";
      requiresIsolation ||=
        executionRequiresIsolation || Boolean(proof.candidate?.isolatedResource);
    } else {
      requirePeer(
        integrated.event === "AttemptIntegrated" && integrated.policyDigest === start.policyDigest,
      );
      ...
      requirePeer(
        reserved.event === "AttemptReserved" && reserved.policyDigest === start.policyDigest,
      );
      requirePeer(
        events
          .filter(
            (event) =>
              event.runId === start.runId &&
              (event.event === "GraphCompiled" || event.event === "GraphProjected"),
          )
          .every((event) => event.sequence < reserved.sequence && time(event) <= time(reserved)),
      );
      requirePeer(
        group.every(
          (event) =>
      ...
      if (publication.mode === "regular-prs")
        requirePeer(
          !isManagedAgentBackendId(reserved.backend) &&
            publication.branch ===
              publicationBranch(number, integrated.workItem, integrated.attempt),
        );
      const selections = events.filter(
        (event) => event.event === "DeliverySelected" && event.runId === start.runId,
      );
      requirePeer(
        selections.length === 1 &&
          selections[0]?.event === "DeliverySelected" &&
          selections[0].sequence > start.sequence &&
          selections[0].sequence < reserved.sequence &&
      ...
      const published = one(
        group.filter(
          (event) => event.event === "AttemptPublished" && event.headSha === publication.headSha,
        ),
      );
      requirePeer(
        validation.event === "ValidationRecorded" &&
          published.event === "AttemptPublished" &&
          published.artifactDigest,
      );
      const accepted = one(
        group.filter(
          (event) =>
            event.event === "AttemptValidated" && event.artifactDigest === published.artifactDigest,
        ),
      );
      requirePeer(
        accepted.sequence > validation.sequence &&
          accepted.sequence < published.sequence &&
          published.sequence < publication.sequence &&
          publication.itemId ===
            one(
              projection.bindings.filter((binding) => binding.issueNumber === integrated.workItem),
            ).compilerId &&
          publication.baseSha === validation.baseSha &&
          publication.baseBranch === receiver.defaultBranch,
      );
      ...
      const usage = group.filter(
        (event) =>
          ...
          event.usageId ===
            `${reviewKind === "artifact" ? "review" : "rebase-review"}-${review.identityDigest}`,
      );
      requirePeer(
        usage.length > 0 &&
          usage.every(
      ...
      requirePeer(
        head.oid === publication.headSha &&
          ...
          head.treeOid === validation.outputTreeSha,
      );
      if (!isManagedAgentBackendId(reserved.backend))
        requirePeer(
      ...
      const linked = one(
        work.linkedPullRequests.filter((pull) => pull.number === publication.pullRequest),
      );
      requirePeer(
        pull.merged &&
          pull.mergeCommitSha === input.targetBaseSha &&
          pull.nodeId === linked.id &&
          pull.number === publication.pullRequest &&
          linked.state === "MERGED" &&
          linked.headSha === pull.headSha &&
      ...
      if (pull.headSha !== publication.headSha) {
      ...
      } else if (merge.parentOids[0] === validation.baseSha)
        requirePeer(merge.treeOid === validation.outputTreeSha);
      else {
        const identity = {
          runId: start.runId,
          objective: number,
        ...
        const acceptedCandidate = await loadReviewCheckpoint(store, {
        ...
        requirePeer(
          acceptedCandidate?.review.accepted && !acceptedCandidate.review.unmetCriteria.length,
        );
        for (const [phase, unit, usageId, amount] of [
          ...
          [
            "management",
            "model_tokens",
            `integration-review-${acceptedCandidate.identityDigest}`,
            acceptedCandidate.usage.inputTokens + acceptedCandidate.usage.outputTokens,
          ],
        ] as const) {
          const amounts = group.filter(
            (event) =>
              event.kind === "budget" &&
              event.event === "BudgetReconciled" &&
              event.phase === phase &&
          ...
          requirePeer(
            amounts.length > 0 &&
              amounts.every((event) => event.kind === "budget" && event.amount === amount),
          );
        }
      }
    }
