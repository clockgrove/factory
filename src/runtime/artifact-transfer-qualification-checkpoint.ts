...
import { z } from "zod";
import { artifactTransferRef, type ArtifactTransferIntentCheckpoint } from "../control/artifact-transfers.js";
...
export const ArtifactTransferQualificationArmSchema = z.object({
  ...
  minPayloadBytes: z.number().int().min(MAX_ARTIFACT_PATCH_BYTES + 1).max(256 * 1024 * 1024),
  expiresAt: z.string().datetime(),
}).strict();
export type ArtifactTransferQualificationArm = z.infer<typeof ArtifactTransferQualificationArmSchema>;
export const ArtifactTransferQualificationTerminalSchema = z.object({
  ...
  session: z.object({ threadId: z.string().min(1).max(160), turnId: z.string().min(1).max(160),
    checkpointDigest: sha256Digest }).strict().optional(),
}).strict();
export type ArtifactTransferQualificationTerminal = z.infer<typeof ArtifactTransferQualificationTerminalSchema>;
...
export function proveArtifactTransferQualificationReceipts(args: {
  ...
  const events = deduplicateFactoryEvents([...args.events]).filter((event) =>
    event.runId === id.runId && event.objective === id.objective);
  const starts = events.filter((event) => event.kind === "run" && event.event === "FactoryRunStarted");
  if (starts.length !== 1 || starts[0]!.kind !== "run" || starts[0]!.event !== "FactoryRunStarted" ||
    starts[0]!.activationRequestId !== args.activationRequestId || !args.activationRequestId ||
    starts[0]!.policyDigest !== id.policyDigest || starts[0]!.repository.toLowerCase() !== id.repository)
    throw new Error("transfer qualification lacks exact activation start");
  const attempts = events.filter((event) => event.kind === "attempt" &&
    event.workItem === id.workItem && event.attempt === id.attempt);
  ...
  if (reserved.length !== 1 || started.length !== 1 || attempts.length !== 2 ||
    reserved[0]!.sequence !== args.reservationSequence ||
    reserved[0]!.sequence >= started[0]!.sequence ||
    attempts.some((event) => event.kind !== "attempt" || event.backend !== args.backend ||
      event.baseSha !== id.baseSha || event.policyDigest !== id.policyDigest ||
      event.directorEpoch !== id.directorEpoch) ||
    reserved[0]!.kind !== "attempt" || started[0]!.kind !== "attempt" ||
  ...
  const model = events.filter((event) => event.kind === "budget" &&
    event.workItem === id.workItem && event.attempt === id.attempt &&
    event.event === "BudgetReconciled" && event.phase === "execution" && event.unit === "model_tokens");
  const usageId = `worker-${id.workItem}-${id.attempt}`;
  if (model.length !== 1 || model[0]!.kind !== "budget" || model[0]!.usageId !== usageId ||
    !Number.isSafeInteger(args.modelTokens) || args.modelTokens! < 0 ||
    model[0]!.amount !== args.modelTokens || model[0]!.sequence <= started[0]!.sequence ||
  ...
  return { reservationReceiptDigest: recoveryEventDigest(reserved[0]!),
    startedReceiptDigest: recoveryEventDigest(started[0]!), modelReceiptDigest: recoveryEventDigest(model[0]!),
    modelTokens: args.modelTokens!, usageId };
}

export class ArtifactTransferQualificationHeldError extends Error {
  constructor(cause?: unknown) {
    super("one-shot artifact transfer qualification held; exact original transfer recovery required", { cause });
    this.name = "ArtifactTransferQualificationHeldError";
  }
}
const hash = (bytes: string | Buffer) => createHash("sha256").update(bytes).digest("hex");
const directoryPath = () => join(tmpdir(), `factory-artifact-transfer-checkpoints-${process.getuid?.()}`);
export function artifactTransferQualificationCheckpointPath(unit: string, invocationId: string): string {
...
async function privateArm(path: string) {
  ...
  try {
    ...
    if (!before.isFile() || before.uid !== process.getuid?.() || (before.mode & 0o777) !== 0o600 ||
      before.nlink !== 1 || before.size > 16_384)
      throw new Error("artifact transfer arm is not bounded private owner data");
    const bytes = Buffer.alloc(before.size + 1);
    const result = await file.read(bytes, 0, bytes.length, 0), after = await file.stat();
    if (result.bytesRead !== before.size || after.size !== before.size ||
...
export async function holdArtifactTransferQualificationCheckpoint(args: {
  ...
  let directory;
  try {
    directory = await open(directoryPath(), constants.O_RDONLY | constants.O_NOFOLLOW | constants.O_DIRECTORY);
  } catch (error) {
  ...
  try {
    ...
    const arm = ArtifactTransferQualificationArmSchema.parse(JSON.parse(original.bytes.toString("utf8")));
    const identity = checkpoint.identity;
    for (const key of ["repository", "objective", "policyDigest", "baseSha"] as const)
      if (arm[key] !== identity[key]) throw new Error("artifact transfer arm differs from current activation");
    if (arm.activationRequestId !== args.activationRequestId ||
      arm.unit !== id.producerUnit || arm.invocationId !== id.producerInvocationId ||
      arm.hostIdentity !== id.hostIdentity || arm.producerPid !== batch.producerPid ||
      arm.producerStartTicks !== batch.producerStartTicks || batch.producerPid !== process.pid ||
      identity.attempt !== 1 || id.runId !== identity.runId || id.workItem !== identity.workItem ||
      id.attempt !== identity.attempt || id.directorEpoch !== identity.directorEpoch ||
      id.policyDigest !== identity.policyDigest || id.repository !== identity.repository ||
      id.objective !== identity.objective || id.phase !== "execution" ||
      batch.commandCount !== 1 || id.commandIndex !== 0)
    ...
    if (checkpoint.intentRef !== `${artifactTransferRef(identity)}/intent` ||
      !Number.isSafeInteger(checkpoint.payloadBytes) || checkpoint.payloadBytes > 256 * 1024 * 1024 ||
      !Number.isSafeInteger(checkpoint.payloadChunks) || checkpoint.payloadChunks < 2 || checkpoint.payloadChunks > 64 ||
      Date.parse(arm.expiresAt) <= Date.now() || Date.parse(arm.expiresAt) > Date.now() + 600_000)
    ...
    if (!current || current.hostIdentity !== arm.hostIdentity || current.producerPid !== arm.producerPid ||
      current.producerStartTicks !== arm.producerStartTicks || current.producerUnit !== arm.unit ||
    ...
    await unchangedArm();
    await args.assertCurrent();
    if (Date.now() >= Date.parse(arm.expiresAt)) throw new Error("artifact transfer checkpoint expired before reaching");
    const witness = {
      protocol: "clockgrove.factory/artifact-transfer-checkpoint-reached-v1",
      armDigest: hash(original.bytes), activationRequestId: arm.activationRequestId,
      ...identity,
      ...
      reachedAt: new Date().toISOString(), expiresAt: arm.expiresAt,
    };
    assertNoSecretMaterial(witness, "artifact transfer qualification witness");
    const bytes = Buffer.from(`${JSON.stringify(witness)}\n`);
    if (bytes.length > 16_384) throw new Error("artifact transfer qualification witness exceeds bound");
    ...
    while (!args.signal?.aborted && Date.now() < Date.parse(arm.expiresAt)) {
      await unchangedArm();
      await sleep(Math.max(1, Math.min(500, Date.parse(arm.expiresAt) - Date.now())), undefined,
