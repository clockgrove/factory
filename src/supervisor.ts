...
import { discoverLocalScopeHost, observeLocalScope, stopLocalScope } from "./runtime/local-scope.js";
...
export class FactorySupervisor {
  ...
  async #cancelActivatedRun(
    ...
    const binding: ActivationBinding = {
      objective: run.objective, requestId: run.activationRequestId, requestedBy: run.actor,
      repository: run.repository, baseSha: run.baseSha, policyDigest: run.policyDigest,
    };
    const readCancellation = async () => {
      const request = await this.#reader.readRunCancellationRequest(run.objective, run.runId, run.actor, binding);
      if (!request) return null;
      if (request.objective !== run.objective || request.requestedBy.toLowerCase() !== run.actor.toLowerCase() ||
        (request.event === "FactoryRunCancellationRequested" && request.runId !== run.runId) ||
        (request.event === "ActivationCancellationRequested" && !activationCancellation([request], binding)))
    ...
    const assertActivation = (events: readonly FactoryEvent[]) => {
      const activations = events.filter((event) => event.event === "ActivationRequested" && event.requestId === binding.requestId);
      const activation = activations[0];
      if (activations.length !== 1 || activation?.event !== "ActivationRequested" || activation.objective !== run.objective ||
        activation.runId !== binding.requestId || activation.repository.toLowerCase() !== binding.repository.toLowerCase() ||
        activation.requestedBy.toLowerCase() !== binding.requestedBy.toLowerCase() ||
        activation.baseSha !== binding.baseSha || activation.policyDigest !== binding.policyDigest ||
    ...
    const heartbeat = setInterval(() => {
      void this.#lease.renewIfNeeded().catch((error) => { heartbeatError = error; this.#lease.fail(error); });
    }, 30_000);
    heartbeat.unref();
    const assertCurrent = async () => {
      ...
      if (!observed || current.number !== run.objective || current.id !== snapshot.id || current.repositoryId !== snapshot.repositoryId ||
        current.authorLogin !== snapshot.authorLogin || current.defaultBranch !== snapshot.defaultBranch ||
        ["runId", "actor", "policyDigest", "activationRequestId", "baseSha", "repository", "baseBranch", "fork"]
    ...
    try {
      ...
      if (graph) {
        ...
        this.#compiledProjection = projection;
        this.#fenceSnapshot(snapshot);
      } else if (snapshot.workItems.length) throw new Error("cancellation graph ownership is unavailable");
      ...
      for (const item of items) {
        const reservations = (await this.#attempts.list(run.objective, item.number))
          .filter((reservation) => reservation.runId === run.runId);
        if ((item.factoryEvents ?? []).some((event) => event.kind === "attempt" && event.runId === run.runId &&
        ...
        for (const reservation of reservations) {
          if (reservation.objective !== run.objective || reservation.workItem !== item.number ||
            reservation.policyDigest !== run.policyDigest || reservation.directorEpoch > acquired.epoch)
            throw new Error("cancellation reservation differs from original ownership");
          snapshot = await assertCurrent();
          const events = snapshotEvents(snapshot).filter((event) => event.runId === run.runId &&
            "workItem" in event && event.workItem === item.number && "attempt" in event && event.attempt === reservation.attempt);
          const backend = this.#registry.get(reservation.backend);
          if (!backend?.reconcileStale) throw new Error(`cancellation cleanup unavailable for ${reservation.backend}`);
          ...
          const resourceIds = new Set(events.flatMap((event) => event.kind === "attempt" && event.providerResourceId ? [event.providerResourceId] : []));
          if (resourceIds.size > 1) throw new Error("cancellation resource identity conflicts");
          const executionBudget = unreconciledBudgetReservations(events).find((event) => event.phase === "execution" && event.unit === "sandbox_milliseconds");
          await backend.reconcileStale({ repository: run.repository, objective: run.objective,
            workItem: item.number, attempt: reservation.attempt, runId: run.runId,
            directorEpoch: reservation.directorEpoch, policyDigest: reservation.policyDigest,
            phase: "execution", ...(reservation.localScopeBatch ? { localScopeBatch: reservation.localScopeBatch } : {}),
            ...(resourceIds.size ? { providerResourceId: [...resourceIds][0]! } : {}),
            ...(!resourceIds.size && executionBudget ? { noHandleReplacementNotBefore:
              new Date(Date.parse(executionBudget.at) + executionBudget.amount + 60_000).toISOString() } : {}),
          });
          await assertCurrent();
          ...
          const capacities = events.filter((event) => event.kind === "capacity" && event.event === "CapacityReserved");
          for (const capacity of capacities) {
            if (capacity.kind !== "capacity") continue;
            if (!capacity.localScopeBatch)
              throw new Error("cancellation validation resource lacks supported exact local scope ownership");
            const batch = LocalScopeBatchSchema.parse(capacity.localScopeBatch);
            if (batch.identity.repository !== run.repository || batch.identity.runId !== run.runId ||
              batch.identity.objective !== run.objective || batch.identity.workItem !== item.number ||
              batch.identity.attempt !== reservation.attempt || batch.identity.policyDigest !== run.policyDigest ||
              batch.identity.phase !== capacity.phase || batch.identity.directorEpoch !== (capacity.recoveryEpoch ?? capacity.directorEpoch))
          ...
          if (reservation.backend === "codex-app-server/local-worktree")
            ...
            const terminal = events.filter((event) => event.kind === "attempt" && event.reportedModelTokens !== undefined &&
              ["AttemptSucceeded", "AttemptFailed", "AttemptTimedOut", "AttemptCancelled"].includes(event.event));
            const known = terminal[0];
            if (known?.kind === "attempt") {
              if (terminal.some((event) => event.kind !== "attempt" || event.reportedModelTokens !== known.reportedModelTokens))
                throw new Error("cancellation terminal model usage conflicts");
              const actual = events.filter((event) => event.kind === "budget" && event.event === "BudgetReconciled" &&
                event.phase === "execution" && event.unit === "model_tokens");
              const usageId = `worker-${item.number}-${reservation.attempt}`;
              if (actual.some((event) => event.kind !== "budget" || event.amount !== known.reportedModelTokens || event.usageId !== usageId))
              ...
              if (!this.#hasModelUsageLink(actual, link)) {
                await assertCurrent();
                await this.#lease.use(async (lease) => { this.#budgetEvents.push(await this.#recorder.budget({
                  lease, workItemNodeId: item.id, reservation, sequence: this.#sequences.take(), event: "BudgetReconciled",
                  phase: "execution", unit: "model_tokens", amount: known.reportedModelTokens!, usageId, ...link,
                  ...(known.reportedModelUsage ? { reportedModelUsage: known.reportedModelUsage } : {}),
                })); });
              }
            }
          }
          const validation = events.find((event) => event.kind === "validation" && event.passed);
          const collected = events.find((event) => event.kind === "attempt" && event.event === "AttemptCollected");
          if (validation?.kind === "validation" && collected?.kind === "attempt" && collected.artifactDigest) {
            const review = await this.#reviews.load({ kind: "artifact", runId: run.runId, objective: run.objective,
              workItem: item.number, attempt: reservation.attempt, artifactDigest: collected.artifactDigest,
              baseSha: validation.baseSha, outputTreeSha: validation.outputTreeSha, evidenceDigest: validation.evidenceDigest });
          ...
          for (const capacity of unreconciledCapacityReservations(events)) {
            ...
            await this.#lease.use(async (lease) => {
              if (capacity.directorEpoch > lease.epoch || capacity.policyDigest !== lease.policyDigest)
                throw new Error("cancellation capacity is fenced from the current lease");
              await this.#store.addIssueComment(item.id, encodeEventComment("Factory reconciled cancelled capacity.", parseFactoryEvent({
                ...capacity, localScopeBatch: undefined, event: "CapacityReconciled", sequence: this.#sequences.take(),
                at: (await this.#store.serverTime()).toISOString(), recoveryEpoch: lease.epoch,
          ...
          for (const budget of unreconciledBudgetReservations(events)) {
            ...
            if (budget.phase === "management" || (budget.phase === "validation" && !capacities.length))
              throw new Error("cancellation budget lacks exact resource retirement evidence");
            if (!["local_milliseconds", "sandbox_milliseconds", "validation_milliseconds"].includes(budget.unit))
              throw new Error("cancellation native accounting cannot infer this reserved unit");
            await assertCurrent();
            await this.#lease.use(async (lease) => { this.#budgetEvents.push(await this.#recorder.budget({
              lease, workItemNodeId: item.id, reservation, sequence: this.#sequences.take(), event: "BudgetReconciled",
              phase: budget.phase, unit: budget.unit, amount: budget.amount, ...(budget.usageId ? { usageId: budget.usageId } : {}),
              usageEvidence: "conservative-reservation", reason: "exact resources absent after cancellation; reserved bound charged, elapsed usage unavailable",
            })); });
          }
          if (!events.some((event) => event.kind === "attempt" &&
            ["AttemptSucceeded", "AttemptFailed", "AttemptTimedOut", "AttemptCancelled", "AttemptDeferred", "AttemptIntegrated"].includes(event.event))) {
            await assertCurrent();
            await this.#lease.use((lease) => this.#attempts.record({ lease, workItemNodeId: item.id,
              reservation, sequence: this.#sequences.take(), event: "AttemptCancelled", allowRecovery: true,
              reason: "operator cancelled original attempt after exact resource cleanup; unknown model usage is not zero" }));
          }
      ...
      if (unreconciledCapacityReservations(remaining).length ||
      ...
      for (const marker of unresolvedModelInvocations(remaining)) {
        ...
        if (!reservation || marker.policyDigest !== reservation.policyDigest || marker.directorEpoch !== reservation.directorEpoch)
          throw new Error("cancellation unknown execution invocation lacks exact resource retirement");
      }
      return await this.#terminal(manager, snapshot, "FactoryRunCancelled", "operator requested cleanup-only cancellation; retained work was not resumed");
    } catch (error) {
