...
const hard = { ...legacy, economics: { ...legacy.economics, modelTokenBudgetMode: "hard" as const } };

function fixture() {
  const snapshot: ApplicationSnapshot = {
    id: "objective", number: 7, title: "Objective", defaultBranch: "main",
    workItems: [], factoryEvents: [],
  };
  ...
  const compile = vi.fn(async () => { throw new Error("unexpected model invocation"); });
  const checkout = vi.fn(async () => { throw new Error("unexpected checkout preparation"); });
...
describe("public fresh model-token policy admission", () => {
  it.each([legacy, hard])("rejects ambiguous or unsupported activation before writes", async (policy) => {
    const f = fixture();
    await expect(f.service.activate({ objective: 7, requestId: "new-budget", baseSha, policy }))
  ...
  it("records explicit observed intent with its own immutable digest and exact replay", async () => {
    ...
    expect(await f.service.activate({ objective: 7, requestId: request.requestId })).toEqual(accepted);
    expect(f.writes).toHaveBeenCalledTimes(1);
    await expect(f.service.activate({ ...request, policy: legacy })).rejects.toThrow(/requires explicit/);
    expect(f.writes).toHaveBeenCalledTimes(1);
  });
  ...
  it("returns only an exact historical activation receipt, never copies its implicit intent to a new request", async () => {
    ...
    const original = parseFactoryEvent({
      protocol: "clockgrove.factory/v2", kind: "run", event: "ActivationRequested",
      objective: 7, runId: "historical", sequence: 1, at: "2026-01-01T00:00:00.000Z",
      requestedBy: "actor", requestId: "historical", repository: "o/r", baseSha,
      policy: legacy, policyDigest: policyDigest(legacy),
      controllerProtocolMin: "clockgrove.factory/v2", controllerProtocolMax: "clockgrove.factory/v2",
    });
    f.snapshot.factoryEvents = [original];
    expect(await f.service.activate({ objective: 7, requestId: "historical" })).toEqual(original);
    expect(await f.service.activate({ objective: 7, requestId: "historical", policy: legacy })).toEqual(original);
    await expect(f.service.activate({ objective: 7, requestId: "fresh-copy", baseSha, policy: legacy }))
  ...
  it.each([legacy, hard, { ...observed, economics: { ...observed.economics, maxModelTokens: 0 } }])(
    ...
    async (policy) => {
      ...
      expect(report.diagnostics).toEqual(expect.arrayContaining([
        expect.objectContaining({ status: "fail", summary: expect.stringMatching(/requires explicit|hard is unsupported|threshold exhausted/) }),
      ]));
      expect(f.compile).not.toHaveBeenCalled();
