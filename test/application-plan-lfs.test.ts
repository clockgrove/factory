...
async function fixture() {
  ...
  const git = (...args: string[]) =>
    execFileSync("/usr/bin/git", ["-c", "core.hooksPath=/dev/null", "-C", repository, ...args], {
      encoding: "utf8",
      timeout: 5_000,
      env: {
        PATH: "/usr/bin:/bin",
  ...
  const planning = {
    ...
    readRepositoryLayout: (max: number, base?: string) =>
      readPlanningRepositoryLayout(repository, max, base),
  };
    ...
    workItems: [
      {
        ...
        scope: ["sample.test.mjs"],
        preconditions: [],
        outOfScope: [],
        conventions: [],
        dependsOn: [],
        baseSha: context.baseSha,
        validationCommands: ["node --test sample.test.mjs"],
        requirements: {
          os: ["linux"],
          architecture: [],
          tools: ["node"],
...
describe("explicit plan pinned LFS preflight", () => {
  it.each([false, true])(
    ...
    async (hydrated) => {
        ...
        async (context: CompilationContext, checkpoint: CompilationCheckpoint) => {
          ...
          for (const asset of f.assets) {
            ...
            const pointer = execFileSync(
              "/usr/bin/git",
              ["-C", context.repository, "show", ":" + asset.path],
              { encoding: "utf8" },
            );
      ...
      const report = await buildPlanReport({
        repository: "o/r",
        request: { objective: 7, compile: true, baseSha: f.baseSha.toUpperCase() },
        snapshot,
        planning: {
          ...f.planning,
          management: backend(compile),
          readRepositoryLayout: async () => ({ files: ["invented.ts"], truncated: false }),
        },
      });
      ...
      for (const asset of f.assets)
        expect(await readFile(join(f.repository, asset.path))).toEqual(
          hydrated ? asset.bytes : Buffer.from(asset.pointer),
        );
  ...
  it.each([
    ...
    "symlink",
    "mode",
  ] as const)("refuses %s before the model and preserves unavailable usage", async (fault) => {
    const f = await fixture(),
      asset = f.assets[0]!;
    if (fault === "tool") await rm(join(f.bin, "git-lfs"));
    if (fault === "missing-object") await rm(asset.cache);
    if (fault === "corrupt-object")
      await writeFile(asset.cache, Buffer.alloc(asset.bytes.length, 1));
    if (fault === "changed-source")
      await writeFile(join(f.repository, asset.path), Buffer.alloc(asset.bytes.length, 2));
    if (fault === "symlink") {
      await rm(join(f.repository, asset.path));
      await symlink(asset.cache, join(f.repository, asset.path));
    }
    ...
    if (fault === "tool")
      expect(report.diagnostics[0]!.summary).toContain("pinned repository requires git-lfs");
    if (fault === "missing-object")
      expect(report.diagnostics[0]!.summary).toContain("required LFS object");
    if (fault === "corrupt-object")
      expect(report.diagnostics[0]!.summary).toContain("LFS cache object digest mismatch");
    await expect(access(f.toolMarker)).rejects.toThrow();
  });
  ...
  it("does not accept forged hydrated facts or staged binary replacements as clean pointer state", async () => {
    ...
    forged.assets[0]!.oid = "0".repeat(64);
    await expect(assertCleanPlanningFiles(f.repository, forged)).rejects.toThrow(
      /actual Git pointer/,
    );
    const oid = execFileSync("/usr/bin/git", ["-C", f.repository, "hash-object", "-w", "--stdin"], {
      input: f.assets[0]!.bytes,
      encoding: "utf8",
    }).trim();
    f.git("update-index", "--cacheinfo", "100644", oid, f.assets[0]!.path);
    await expect(validatePlanningCheckout(f.repository, f.baseSha, "o/r")).rejects.toThrow(
      /index differs/,
    );
  });
  ...
  it("preserves the paid successful result and usage when exact tree cleanup fails", async () => {
    ...
    expect(report.proposedGraph?.workItems).toHaveLength(1);
    expect(report.usage).toEqual(usage);
    expect(report.diagnostics).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          status: "warning",
          summary: expect.stringContaining("compilation tree cleanup needs attention"),
        }),
      ]),
    );
    expect(compile).toHaveBeenCalledTimes(1);
  ...
  it("preserves known failed-call usage when cleanup also fails", async () => {
    ...
    const compile = vi.fn(async () => {
      throw new ManagementOutputError(new Error("invalid management output"), usage);
    });
    const report = await plan(f, compile);
    expect(report.compilation).toMatchObject({
      result: "failed",
  ...
  it("disposes a prepared tree after hydration failure without invoking management", async () => {
    ...
    vi.spyOn(pinned, "materializePinnedCompilationTree").mockImplementation(async (...args) => {
      const tree = await prepare(...args);
      preparedPath = tree.path;
      return tree;
    });
    vi.spyOn(lfs, "materializeLocalLfsAssets").mockRejectedValue(
      new Error("LFS cache object changed during hydration"),
    );
    const compile = vi.fn();
    const report = await plan(f, compile);
  ...
  it("keeps postcompile source checking and its observed usage for later hydrated edits", async () => {
    ...
    const compile = vi.fn(
      async (context: CompilationContext, checkpoint: CompilationCheckpoint) => {
        ...
        await checkpoint(result);
        await writeFile(
          join(f.repository, f.assets[0]!.path),
          "changed after observed paid result\n",
        );
        return result;
      },
    );
    ...
    expect(report.compilation.result).toBe("failed");
    expect(report.usage).toEqual(usage);
    expect(report.diagnostics[0]!.summary).toContain("tracked changes");
    expect(compile).toHaveBeenCalledTimes(1);
  });
});
