...
function readFixture(deadline = instant + 10000) {
  ...
  return {
    records, wait,
    context: {
      phase: "terminal-artifact-hold", stage: "objective", deadline,
      now: () => clock, wait,
      record: (diagnostic: Recorded) => { records.push(diagnostic); },
    },
  };
}
...
describe("checkpoint observation diagnostics", () => {
  it("records fixed phase, stage, time and status without disclosing arbitrary error content", () => {
    ...
    const diagnostic = checkpointObservationFailure({
      status: 503, message: secret, body: secret,
    ...
    expect(diagnostic).toEqual({ boundary: "observation", phase: "terminal-artifact-hold",
      stage: "comments", failedAt: "2026-09-07T05:00:28.000Z", category: "http",
      code: "UNAVAILABLE", httpStatus: 503 });
    expect(JSON.stringify(diagnostic)).not.toContain(secret);
    expect(checkpointObservationFailure(Error(secret), {
      phase: secret, stage: secret, now: instant,
  ...
  it("records transient failures then completes the same read without resetting its deadline", async () => {
    const f = readFixture();
    const operation = vi.fn().mockRejectedValueOnce({ status: 503 })
      .mockRejectedValueOnce({ code: "ECONNRESET" }).mockResolvedValue("observed");
    ...
    expect(f.records.map(({ attempt, retry }) => ({ attempt, retry }))).toEqual([
      { attempt: 1, retry: true }, { attempt: 2, retry: true },
    ]);
  ...
  it.each([
    ...
    new SyntaxError("private"), { code: "ERR_ASSERTION" },
    { name: "McpError", code: -32001 }, Error("unclassified operator refusal"),
  ...
  it.each(["receipts", "witness", "extension", "accept", "poll"])("does not replay nontransport stage %s", async (stage) => {
    const f = readFixture();
    const failure = { code: "ECONNRESET" };
    const operation = vi.fn().mockRejectedValue(failure);
    await expect(checkpointObservationRead(operation, { ...f.context, stage })).rejects.toBe(failure);
    expect(operation).toHaveBeenCalledTimes(1);
  });
  ...
  it("does not admit mutation stages into the read retry port", async () => {
    const f = readFixture();
    const action = vi.fn();
    await expect(checkpointObservationRead(action, { ...f.context, stage: "activate" })).rejects.toThrow("unsupported observation read stage");
  ...
  it("does not begin an expired read or grant time for another retry", async () => {
    ...
    await expect(checkpointObservationRead(operation, expired.context)).rejects.toMatchObject({ code: "CHECKPOINT_DEADLINE" });
