...
function candidate(paid = false): BackendCandidate {
  ...
  return {
    ...
    capabilities: {
      id,
      agentKind: "codex",
      runtimeKind: paid ? "daytona" : "local-worktree",
      hostExecution: !paid,
      isolation: paid ? "container" : "process",
      supportedOs: ["linux"],
...
function evidence(overrides: Partial<DecompositionEvidence> = {}): DecompositionEvidence {
  return {
    ...
    repositoryLimits: { maxLocalWorkers: 8, maxPaidWorkers: 2 },
    deliveryMode: "native-stacks",
...
describe("grounded advisory compiler economics", () => {
  it("separates dependency width, resource fit and configured savings", () => {
    ...
    expect(assessment).toMatchObject({
      dependencyWaveWidth: 3,
      configuredWorkMinutes: 30,
      configuredCriticalPathMinutes: 10,
      idealConcurrencyTimeSavedMinutes: 20,
      localFit: { availability: "estimated", likelySlots: 2, waveFits: [2], measuredAt: stamp },
    });
    expect(
      assessment.cloudEligibility.every((value) => value.status === "eligible-in-principle"),
    ).toBe(true);
    expect(inputs.capacity!.reservations).toEqual([]);
    expect(graph(undefined, inputs).workItems[0]!.economicReview.rationale).toContain(
      "Resource-constrained local wave fit: 2",
    );
    expect(graph(undefined, inputs).workItems[0]!.economicReview.rationale).toContain(
      "no admission or paid execution authority",
    );
  });
  ...
  it("respects controller, backend, delivery, pressure and cooldown restrictions", () => {
    for (const inputs of [
      ...
      evidence({
        policy: {
          ...policy,
          capacity: { ...policy.capacity!, backendMaxParallel: { [localId]: 1 } },
        },
      }),
    ])
      expect(assessDecomposition(graph().workItems, inputs).localFit.likelySlots).toBe(1);
    expect(
      assessDecomposition(graph().workItems, evidence({ deliveryMode: "regular-prs" })).localFit
        .likelySlots,
    ).toBe(2);
    for (const inputs of [
      evidence({ cooldownUntilMs: Date.parse(stamp) + 1000 }),
      evidence({ deliveryMode: "escalate" }),
    ])
      expect(assessDecomposition(graph().workItems, inputs).localFit.likelySlots).toBe(0);
  ...
  it("retains reservation resource and path conflicts without mutating the supplied ledger", () => {
    ...
    const capacity = ledger.reconcile(1, [
      {
        ...identity,
    ...
    expect(
      assessDecomposition(graph().workItems, evidence({ capacity })).localFit.likelySlots,
    ).toBe(1);
  ...
  it("requires explicit paid authority and complete compatible per-item capability observations", () => {
    const compiled = graph().workItems;
    expect(
      assessDecomposition(compiled, evidence({ policy: { ...policy, allowedPaidBackends: [] } }))
        .cloudEligibility[0]!.status,
    ).toBe("ineligible");
  ...
  it("applies effective trust without altering the packet or granting host execution", () => {
    ...
    expect(assessDecomposition(compiled, evidence({ policy: isolated })).localFit.likelySlots).toBe(
      0,
    );
  });
  it("preserves distinct execution and convention contracts instead of inventing redundancy", () => {
    ...
    expect(() =>
      graph([first, { ...first, id: "b", conventions: ["Use the compatibility implementation"] }]),
    ).not.toThrow();
  });
