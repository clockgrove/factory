...
import { collectCompilationEvidence, type CompilationEvidenceSource } from "../src/compiler/runtime-evidence.js";
...
const requirements: ExecutionRequirements = {
  os: ["linux"], architecture: ["x64"], tools: [], services: [],
  networkDestinations: [], permittedSecretNames: [], trust: "trusted_local",
};
const items = [{ id: "a", requirements }, { id: "b", requirements }];
function source(): CompilationEvidenceSource {
  return {
    objective: 1, policy: parseRunPolicy(DEFAULT_RUN_POLICY),
    repositoryLimits: { maxLocalWorkers: 4, maxPaidWorkers: 0 },
    deliveryMode: "regular-prs", nowMs: 1000, cooldownUntilMs: 0,
    capacity: new CapacityLedger().snapshot(),
    sampleResource: vi.fn(async () => ({
      measuredAt: new Date(1000).toISOString(), logicalCpu: 8, effectiveCpu: 4,
      loadRatio: 0, totalMemoryMb: 8192, availableMemoryMb: 4096,
      memoryUsageRatio: 0.5, source: "host" as const,
    })),
...
describe("new-graph runtime economic observations", () => {
  ...
  it("keeps failed observations unknown without echoing provider diagnostics", async () => {
    const input = source();
    input.sampleResource = vi.fn(async () => { throw Error("private sampling details"); });
    input.evaluate = vi.fn(async () => { throw Error("private provider details"); });
    const result = await collectCompilationEvidence(items, input);
  ...
  it("applies isolated trust before probing and never mutates compiler requirements", async () => {
    ...
    expect(input.evaluate).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it("does not invent resource observations for fixed policy", async () => {
    const input = source();
    input.policy = parseRunPolicy({ ...input.policy, capacity: { ...input.policy.capacity, mode: "fixed" } });
    const result = await collectCompilationEvidence(items, input);
  ...
  it("bounds observations before I/O and rejects duplicate graph identities", async () => {
    ...
    await expect(collectCompilationEvidence([items[0]!, items[0]!], input)).rejects.toThrow(/bounded unique graph/);
    await expect(collectCompilationEvidence(Array.from({ length: 101 }, (_, id) => ({ id: String(id), requirements })), input)).rejects.toThrow(/bounded unique graph/);
    expect(input.sampleResource).not.toHaveBeenCalled();
