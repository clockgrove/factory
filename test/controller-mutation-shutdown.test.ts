...
import { ContentCreationPacer, MutationAdmissionStoppedError, MutationScheduler } from "../src/platform.js";
...
function setup(input: { paced?: boolean; fetch?: typeof globalThis.fetch } = {}) {
  ...
  const request = vi.fn(input.fetch ?? (async () => new Response(JSON.stringify({ sha: "d".repeat(40) }), {
    status: 201, headers: { "content-type": "application/json" },
  })));
  const store = new GitHubControlStore({
    token: "fixture-only", owner: "fixture", repo: "fixture", requestFetch: request,
    mutationScheduler: resources.mutationScheduler, circuitBreaker: resources.circuitBreaker,
    concurrency: resources.concurrency,
  });
  vi.spyOn(GitHubControlStore.prototype, "getRepositoryFacts").mockResolvedValue({ defaultBranch: "main" } as never);
  vi.spyOn(GitHubControlStore.prototype, "getBranchHead").mockResolvedValue({ oid: "a".repeat(40), treeOid: "b".repeat(40) } as never);
  vi.spyOn(GitHubControlStore.prototype, "discoverObjectiveActivations").mockResolvedValue([{
    objective: 1, activatedAt: new Date().toISOString(), requestId: "approved",
    policy: {}, policyDigest: "c".repeat(64), baseSha: "a".repeat(40), requestedBy: "operator",
  }]);
  const acquire = vi.spyOn(RepositoryLeaseManager.prototype, "acquire").mockImplementation(async (identity) => ({
    ...identity, ref: "refs/clockgrove-factory/leases/repository-controller", oid: "a".repeat(40),
    treeOid: "b".repeat(40), epoch: 1, sequence: 1, expiresAt: new Date(Date.now() + 600_000),
  }));
  const release = vi.spyOn(RepositoryLeaseManager.prototype, "release").mockImplementation(async (lease) => {
    expect(lease.epoch).toBe(1);
    expect(lease.controllerId).toBe(acquire.mock.calls[0]![0].controllerId);
    await store.createCommit({ treeOid: lease.treeOid, parentOids: [lease.oid], message: "Factory repository-controller lease release" });
  });
  const run = (operation: () => Promise<void>) => runGitHubRepositoryController({
    token: "fixture-only", owner: "fixture", repo: "fixture", repository: "/fixture",
    signal: abort.signal, resources, pollIntervalMs: 1_000,
    supervisorFactory: () => ({ run: operation }),
  });
  const normal = () => store.stackRequest("POST /repos/{owner}/{repo}/issues/{issue_number}/comments", {
    owner: "fixture", repo: "fixture", issue_number: 1, body: "queued normal observation",
  }, true);
...
it.each([false, true])("waits for admitted transport after stop and preserves its actual outcome (refused: %s)", async (refused) => {
  const response = deferred();
  let entered = false;
  const f = setup({ fetch: async (url) => {
    if (String(url).includes("/comments")) {
      entered = true;
      await response.promise;
      return new Response(JSON.stringify(refused ? { message: "fixture unavailable" } : { id: 1 }), {
        status: refused ? 503 : 201, headers: { "content-type": "application/json" },
      });
    }
    return new Response(JSON.stringify({ sha: "d".repeat(40) }), { status: 201, headers: { "content-type": "application/json" } });
  }});
  let settled = false;
  const task = f.run(async () => { await f.normal(); });
  const outcome = task.then(() => { settled = true; return null; }, (error: unknown) => { settled = true; return error; });
  await vi.advanceTimersByTimeAsync(1);
  ...
  if (refused) expect(failure).toMatchObject({ message: expect.stringContaining("unresolved platform failure") });
...
it("does not hide unresolved cleanup behind a pre-dispatch cancellation", async () => {
  ...
  const task = f.run(async () => {
    try { await f.normal(); }
      ...
      if (error instanceof MutationAdmissionStoppedError) throw Error("resource cleanup remains unknown");
      throw error;
    }
  });
  ...
  await vi.advanceTimersByTimeAsync(1);
  f.abort.abort();
  expect(await outcome).toMatchObject({ message: expect.stringContaining("non-retryable failure") });
...
it("rechecks the permit after an awaited mutation fence and releases it for lease traffic", async () => {
  ...
  const request = vi.fn(async () => new Response(JSON.stringify({ sha: "d".repeat(40) }), { status: 201 }));
  const store = new GitHubControlStore({ token: "fixture-only", owner: "fixture", repo: "fixture",
    mutationScheduler: f.resources.mutationScheduler, requestFetch: request,
