...
describe("conservative economic feedback", () => {
  ...
  it("does not turn dispatch intent into zero usage or an available remaining token allowance", () => {
    ...
    const marker = event({
      kind: "budget", event: "BudgetReserved", sequence: 2,
      phase: "management", unit: "model_tokens", amount: 0,
      modelInvocationId: "next-call", usageId: "invocation-next-call",
    });
    ...
    expect(pending.nativeUnits.find((ledger) => ledger.unit === "model_tokens"))
      .toMatchObject({ reserved: 0, reservations: 0, reconciled: 40, reconciliations: 1 });
    const completed = summarizeEconomics({
      events: [known, marker, modelReceipt(3, { amount: 90, modelInvocationId: "next-call" })], policy,
    });
    ...
    expect(completed.budgets.modelTokens).toMatchObject({
      availability: "observed", value: { configured: 100, committed: 130, remaining: 0 },
    });
  });
