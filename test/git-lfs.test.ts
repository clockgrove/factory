...
describe("pinned LFS detection and local hydration", () => {
  it("accepts canonical and legacy pointers and rejects malformed or extension transforms", () => {
    ...
    for (const invalid of [
      ...
      pointer.replace("oid sha256:", "ext-0-test value\noid sha256:"),
  ...
  it("reads exact commit objects despite modified working-tree attributes and pointer", async () => {
    ...
    expect(facts).toMatchObject({
      ...
      assets: [{ path: "asset.bin", oid, size: bytes.length, mode: "100644" }],
    });
    expect(facts.assets[0]!.pointerBlobOid).toBe(
      git(f.repository, ["rev-parse", `${f.baseSha}:asset.bin`]),
    );
  ...
  it("verifies full cached SHA256 before hydrating and never changes source pointers or index", async () => {
    ...
    await expect(assertLocalLfsAvailable(f.repository, f.baseSha)).resolves.toMatchObject({
      requiredTools: ["git-lfs"],
    });
  ...
  it("does not hydrate missing, corrupted or symlinked cache objects", async () => {
    ...
    await expect(materializeLocalLfsAssets(f.repository, f.destination, f.baseSha)).rejects.toThrow(
      /digest/,
    );
    expect(await readFile(join(f.destination, "asset.bin"), "utf8")).toBe(pointer);
    await rm(f.object);
    await expect(assertLocalLfsAvailable(f.repository, f.baseSha)).rejects.toThrow(
      /missing or unsafe/,
    );
  ...
  it("rejects forged facts, traversal, duplicate paths and aggregate overflow", async () => {
    ...
    expect(() =>
      normalizePinnedLfsFacts({ ...facts, assets: [facts.assets[0]!, facts.assets[0]!] }),
    ).toThrow();
    expect(() =>
      normalizePinnedLfsFacts({ ...facts, assets: [{ ...facts.assets[0]!, path: "../escape" }] }),
    ).toThrow();
    expect(() =>
      normalizePinnedLfsFacts({
        ...facts,
        assets: ["a", "b", "c"].map((path) => ({
          ...facts.assets[0]!,
          path,
          size: 100 * 1024 * 1024,
        })),
