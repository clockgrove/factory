...
import { adaptFixtureCompiler, type LegacyFixtureCompiler } from "./compiler-proposal.js";
...
export function registerAdoptedValidationTests() {
  describe("Supervisor adopted isolated candidate validation / core", () => {
    it.each([false, true])(
      ...
      async (nativeSource) => {
        ...
        expect(input).toMatchObject({
          ...
          policyDigest: f.pd,
          packet: {
            baseSha: f.planRecord.plan.expectedBaseSha,
    ...
    it("resumes the non-dispatching artifact consumer after its success response is lost", async () => {
      ...
      expect(
        interrupted.filter(
          (event) => event.kind === "attempt" && event.event === "AttemptSucceeded",
        ),
      ).toHaveLength(1);
      expect(
        interrupted.some(
...
export function registerAdoptedRecoveryTests() {
  describe("Supervisor adopted isolated candidate validation / recovery", () => {
      ...
      async (stage) => {
        ...
        expect(
          (
            ...
            (entry) => entry.runId === "successor" && entry.reservation.attempt === 2,
          ),
        ).toMatchObject({ disposition: "released", dispatchPossible: false });
      },
      60_000,
    ...
    it.each([false, true])(
      ...
      async (unavailableTarget) => {
        ...
        if (unavailableTarget) {
          ...
          vi.mocked(GitHubControlStore.prototype.getBranchHeadOid).mockResolvedValue(
            unavailableHead,
          );
        }
...
export function registerSuccessorRefreshTests() {
  describe("Supervisor authenticated successor execution / refresh", () => {
    ...
    it("reconciles a lost ordinary refresh CAS response with the same intent and historical accounting", async () => {
      ...
      expect(await f.run(), JSON.stringify(f.messages)).toMatchObject({ status: "completed" });
      expect(f.refresh).toHaveBeenCalledOnce();
      expect([...f.refs].filter(([ref]) => ref.includes("/sibling-refreshes/"))).toEqual(
        refreshRefs,
      );
...
export function registerSuccessorRefreshFenceTests() {
  describe("Supervisor authenticated successor execution / refresh fences", () => {
...
export function registerSuccessorStackIntegrityTests() {
  describe("Supervisor authenticated successor execution / stack integrity", () => {
    it.each([false, true])(
      ...
      async (premerged) => {
        ...
        try {
          ...
          expect(receipts?.terminal).toMatchObject({
          ...
          expect.arrayContaining([
            expect.objectContaining({
              ...
              outcome: expect.objectContaining({
                workItem: 8,
                mergeCommitSha: f.mergeShas.get(18),
              }),
            }),
          ]),
        );
    ...
    it.each([
      ...
      async (options) => {
        ...
        expect(
          evaluate.mock.calls.every(([input]) => input.requirements.trust === "isolated"),
        ).toBe(true);
        expect(f.launch).not.toHaveBeenCalled();
        expect(f.merge).toHaveBeenCalledTimes(merges);
        expect(
...
export function registerSuccessorStackRecoveryTests() {
  describe("Supervisor authenticated successor execution / stack recovery", () => {
    it("binds inherited intermediate isolation into the reserved and launched child packet", async () => {
      ...
      f.launch.mockImplementation(async (context) => {
        ...
        expect(context.packet.baseSha).toBe(
          f.snapshot.workItems[1]!.linkedPullRequests[0]!.headSha,
        );
        throw stop;
      });
      await expect(f.run()).rejects.toThrow(
...
export function registerSuccessorRecompilationTests() {
  describe("Supervisor authenticated successor execution / recompilation", () => {
    it("recompiles once in a second successor after an accounted graph-bootstrap terminal", async () => {
      ...
      expect(
        all.filter(
          (event) => event.event === "GraphCompiled" && event.runId === "second-successor",
        ),
      ).toHaveLength(1);
      expect(
        all.filter(
...
export function registerSuccessorPublicationTests() {
  describe("Supervisor authenticated successor execution / publication", () => {
