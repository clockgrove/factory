...
import { providerSupervisorFixture } from "./provider-supervisor.js";
...
export function registerCapabilityAdmissionTests() {
  describe("Supervisor repository-capability admission / providers", () => {
...
export function registerCapabilityFenceTests() {
  describe("Supervisor repository-capability admission / fences", () => {
...
export function registerCapabilityRuntimeTests() {
  describe("Supervisor repository-capability admission / runtime lineage", () => {
...
export function registerWorkflowPublicationTests() {
  describe("Supervisor workflow publication boundary / publication", () => {
    it.each([false, true])(
    ...
    it("publishes a protected-push-only workflow but leaves its merge human-authorized", async () => {
      ...
      try {
        ...
        expect(fixture.mergePull).not.toHaveBeenCalledWith(
          expect.objectContaining({ number: 108 }),
        );
      } finally {
        scoped.mockRestore();
      }
...
export function registerWorkflowRecoveryTests() {
  describe("Supervisor workflow publication boundary / recovery", () => {
    it.each([
