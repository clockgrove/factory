...
function fixture(input: { count?: number; incomplete?: boolean } = {}) {
  const calls: string[] = [];
  const store = new GitHubControlStore({ token: "fixture-only", owner: "o", repo: "r",
    requestFetch: async (url, init) => {
      ...
      if (request.method === "GET" && target.pathname.endsWith("/pulls")) {
        ...
        return Response.json(Array.from({ length: input.count ?? 1 }, (_, index) => ({
          number: index + 10, node_id: `PR_${index}`, head: { ref: "factory/objective-7/work-item-8/attempt-1" },
        })));
      }
      if (request.method === "POST" && target.pathname === "/graphql") {
        ...
        return Response.json({ data: { nodes: [{ closingIssuesReferences: {
...
describe("exact-commit Objective discovery hints", () => {
  ...
  it("rejects malformed identities before I/O and refuses truncated association pages", async () => {
    ...
    await expect(full.store.readCommitObjectiveCandidates("a".repeat(40))).rejects.toThrow("bounded");
    expect(full.calls).toHaveLength(1);
    const incomplete = fixture({ incomplete: true });
    await expect(incomplete.store.readCommitObjectiveCandidates("a".repeat(40))).rejects.toThrow("bounded");
