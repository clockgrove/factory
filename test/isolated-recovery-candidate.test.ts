...
} from "../src/recovery/isolated-candidate.js";
...
function fixture() {
  ...
  const validation = createValidationEvidence({
    ...
    baseSha: sha("d"),
    outputTreeSha: sha("e"),
    commands: [{ command: "node --test", exitCode: 0, durationMs: 10 }],
    passed: true,
    startedAt: "2026-09-06T08:00:00.000Z",
    completedAt: "2026-09-06T08:00:00.010Z",
  });
  const identity = {
    runId: "successor",
    objective: 7,
  ...
  const candidate: MergeCandidateCheckpointRecord = {
    ref: "refs/fixture/candidate",
    commitOid: sha("1"),
    blobOid: sha("2"),
    identity,
    source,
    validation,
    evidence: bindMergeCandidateValidation({ source, validation }),
    isolatedResource: {
      backend: "codex-cli/daytona",
      invocationOwnershipDigest: owner,
  ...
  const reserved = event({
    ...
    policyDigest: accepted,
    phase: "validation",
    backend: `factory/integration-sandbox-${candidateDigest}`,
    requestedCpu: 1,
    ...
    isolatedValidation: {
      ...
      artifactDigest: validation.artifactDigest,
      invocationOwnershipDigest: owner,
      deadline: "2026-09-06T00:01:00.000Z",
...
describe("isolated adopted candidate proof", () => {
  it("binds completed native usage to the recorded successor epoch and keeps clocks distinct", () => {
    ...
    expect(
      Date.parse(f.candidate.validation.completedAt) - Date.parse(f.candidate.validation.startedAt),
    ).toBe(10);
  });
  ...
  it("treats pending reservation as a liability, never completed or accounted work", () => {
    ...
    expect(() =>
      assertIsolatedCandidateReservation({
        ...f,
        identity: f.candidate.identity,
        reservation: f.reserved,
        events: f.events.slice(0, 2),
      }),
    ).not.toThrow();
    expect(() => assertIsolatedCandidateProof({ ...f, events: f.events.slice(0, 3) })).toThrow(
      /proof unavailable/,
    );
    expect(() =>
      assertIsolatedCandidateProof({
  ...
  it.each([
    ...
    if (changed === "allocation")
      f.events = f.events.filter((entry) => entry.event !== "BudgetReserved");
    if (changed === "reconciliation")
      f.events = f.events.filter((entry) => entry.event !== "CapacityReconciled");
  ...
  it("rejects a second invocation or a later accounting receipt borrowed across the outcome boundary", () => {
    ...
    expect(() => assertIsolatedCandidateProof(f)).toThrow(/proof unavailable/);
    f.events.pop();
    expect(() => assertIsolatedCandidateProof({ ...f, beforeSequence: 5 })).toThrow(
      /proof unavailable/,
    );
  });
  ...
  it("proves failed cleanup separately, repairs missing usage, and never calls it successful", () => {
    ...
    receipt.isolatedFailure = {
      ...
      completedAt: f.candidate.isolatedResource!.completedAt,
      sandboxMilliseconds: 1000,
    };
    ...
    expect(() =>
      assertIsolatedCandidateFailureProof({
        ...proof,
        events: f.events.filter((event) => event.event !== "BudgetReconciled"),
        requireAccounting: false,
      }),
    ).not.toThrow();
    expect(() =>
      assertIsolatedCandidateFailureProof({ ...proof, requireAccounting: false }),
    ).not.toThrow();
    ...
    f.events.push(
      parseFactoryEvent({
        ...
        workItem: 8,
        sequence: 6,
        at: "2026-09-06T00:00:04.000Z",
        phase: "validation",
        unit: "validation_milliseconds",
        amount: 10,
        usageId: `integration-validation-${mergeCandidateIdentityDigest(f.candidate.identity)}`,
      }),
    );
  ...
  it("refuses unknown or conflicting failure completion instead of manufacturing zero usage", () => {
    ...
    receipt.isolatedFailure = {
      validationDigest: digest("b"),
      validationStartedAt: "2026-09-06T00:00:03Z",
      validationCompletedAt: "2026-09-06T00:00:02Z",
      startedAt: "2026-09-06T00:00:02Z",
