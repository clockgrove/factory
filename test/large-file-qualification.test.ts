...
function commitPhase(fixture: LargeFileFixture, phase: LargeFilePhase, baseSha: string) {
  ...
  const patch = git(fixture.repository, [
    "diff",
    "--cached",
    "--binary",
    "--no-ext-diff",
  ...
  const head = git(fixture.repository, ["commit-tree", tree, "-p", baseSha, "-m", phase])
    .toString()
    .trim();
...
describe("installed large-file qualifier fixture and proof contracts", () => {
  ...
  it("rejects unsafe namespace/source modes and does not overwrite an existing namespace", () => {
    ...
    expect(() =>
      createLargeFileFixture({
        ...
        sourceRepository: prepared.repository,
      }),
    ).toThrow();
    expect(() =>
      createLargeFileFixture({
    ...
    const linkOid = git(prepared.repository, ["hash-object", "-w", "--stdin"], "target")
      .toString()
      .trim();
    ...
    const head = git(prepared.repository, [
      "commit-tree",
      tree,
      "-p",
      prepared.baseSha,
  ...
  it("runs the small recipe in three ordered phases and proves exact oversized patch, manifests and final blobs", async () => {
    ...
    for (const phase of ["payload", "metadata", "join"] as const) {
      ...
      execFileSync(process.execPath, [join(prepared.repository, prepared.recipePath), phase], {
      ...
      const artifact = await artifactFromPatchFile({
        ...
        patchPath,
        baseSha: base,
        changedPaths: prepared.expected
      ...
      expect(
        assertLargeFileArtifact({ fixture: prepared, artifact, observation, patch, phase })
          .oversized,
      ).toBe(phase === "payload");
      if (phase === "payload") {
        ...
        const mutations = [
          ...
          },
          {
            ...
            fileManifest: {
              ...
              files: artifact.fileManifest!.files.map((file) => ({ ...file, mode: "100755" })),
            },
          },
        ];
        ...
        const damaged = Buffer.from(patch);
        damaged[damaged.length - 20] = damaged[damaged.length - 20]! ^ 1;
        expect(() =>
          assertLargeFileArtifact({
            fixture: prepared,
            artifact,
    ...
    expect(assertLargeFileFinalTree({ fixture: prepared, observation }).lfsPointersPreserved).toBe(
      true,
    );
    expect(observation.files.find((file) => file.path === prepared.paths.executable)?.mode).toBe(
      "100755",
    );
  ...
  it("does not write through symlink parents, outside its owned fixture, or over existing output", () => {
    ...
    expect(() => writeLargeFileOutput({ fixture: prepared, checkout: elsewhere })).toThrow(
      /another checkout/,
    );
  ...
  it.each(["scope", "secret"] as const)(
    "produces local-only %s collection refusal bytes",
    async (scenario) => {
      const prepared = fixture();
      execFileSync(process.execPath, [join(prepared.repository, prepared.recipePath), scenario], {
        cwd: prepared.repository,
        timeout: 15_000,
        env: { PATH: "/usr/bin:/bin", HOME: prepared.root },
      });
      ...
      expect(assertLargeFileRefusal(observation)).toMatchObject({
        refused: true,
        uploadAbsence: "unavailable",
      });
      expect(() => assertLargeFileRefusal({ ...observation, contentUploads: 0 })).toThrow(
        /provenance/,
      );
  ...
  it("allows exact raw symlink Git content but refuses filesystem materialization before executable validation", async () => {
    const prepared = fixture();
    execFileSync(process.execPath, [join(prepared.repository, prepared.recipePath), "symlink"], {
      cwd: prepared.repository,
      timeout: 15_000,
      env: { PATH: "/usr/bin:/bin", HOME: prepared.root },
    });
    ...
    const observation = {
      scenario: "symlink" as const,
      outcome: "refused" as const,
      stage: "filesystem-materialization" as const,
      reason,
      artifactPublished: false,
    };
    ...
    ).toMatchObject({ uploadAbsence: "not-absent" });
  ...
  it.each([
    ...
    "requires explicit no-model/no-content-upload observation for %s",
    (scenario, reason) => {
      const observation = {
        scenario: scenario as LargeFileRefusalScenario,
        outcome: "refused" as const,
        stage: "source-preflight" as const,
        reason,
        contentUploads: 0,
        contentUploadEvidence: "instrumented-content-write-count" as const,
        artifactPublished: false,
        modelCalls: 0,
      };
      ...
      for (const changed of [
        { ...observation, outcome: "accepted" as const },
        { ...observation, reason: "unrelated platform outage" },
        { ...observation, modelCalls: 1 },
        { ...observation, contentUploads: 1 },
        { ...observation, artifactPublished: true },
      ])
