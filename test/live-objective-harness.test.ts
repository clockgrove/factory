...
function fallbackTransport(status = 404, override: Record<string, unknown> = {}) {
  ...
  return vi.fn(async (route: string, parameters: Record<string, unknown>) => {
    ...
    const response = {
      ...
      data: stacks ? (status === 404 ? { message: "Not Found" } : []) :
        route.endsWith("/pulls") ? [] : {
          id: 99, node_id: "R_fixture", full_name: repository,
          private: true, archived: false, permissions: { push: true },
        },
      ...(stacks ? override : {}),
    };
    if (stacks && override.headers) response.headers = {
      "x-github-api-version-selected": "2026-03-10",
      "x-github-request-id": "ABC:123", "x-ratelimit-remaining": "2000",
      date: "Sun, 06 Sep 2026 00:00:00 GMT", ...override.headers as object,
    };
...
async function fallbackEvidence() {
  ...
  for (const event of value.events) {
    ...
    if (event.event === "DeliverySelected") Object.assign(event, {
      requested: "stacked-prs", selected: "regular-prs", capabilityVersion: "2026-03-10",
    ...
    if (event.event === "AttemptStarted") {
      const id = createHash("sha256").update(JSON.stringify([
        "clockgrove.factory/attempt-v2", value.repository, "fixture", 1, event.workItem, 1, 1,
      ])).digest("hex");
      event.providerResourceId = `sdk-${id.slice(0, 24)}`;
      event.resourceHostIdentity = host;
    }
    if (event.event === "ValidationRecorded") events.push(
      { ...event, kind: "capacity", event: "CapacityReserved", phase: "validation", backend: "codex-sdk/local-worktree" },
    );
    events.push(event);
    if (event.event === "ValidationRecorded") events.push(
      { ...event, kind: "capacity", event: "CapacityReconciled", phase: "validation", backend: "codex-sdk/local-worktree" },
    );
  }
  value.events = events.map((event, index) => {
    ...
    if (event.event === "AttemptReserved" || (event.event === "CapacityReserved" && event.phase === "validation")) {
      const phase = event.event === "AttemptReserved" ? "execution" : "validation";
      event.localScopeBatch = {
        identity: {
          protocol: "clockgrove.factory/local-scope-v1", repository: value.repository,
          runId: "fixture", objective: 1, workItem: event.workItem, attempt: 1, directorEpoch: 1,
          policyDigest: digest, phase, commandIndex: 0, invocationDigest: "a".repeat(64), hostIdentity: host,
        },
        commandCount: phase === "execution" ? 1 : 2, producerPid: 123, producerStartTicks: "12345",
  ...
  observeNativeScopes(result, (unit) =>
    `Id=${unit}\nLoadState=not-found\nActiveState=inactive\nSubState=dead\nControlGroup=\nJob=\nInvocationID=\nKillMode=control-group`, host);
  return result;
}

describe("actual native-unavailability regular fallback", () => {
  const env = {
    FACTORY_LIVE_NATIVE_FALLBACK_OBJECTIVE: "1", FACTORY_LIVE_OBJECTIVE_PREFLIGHT: "1",
    FACTORY_LIVE_OBJECTIVE_MAX_MODEL_TOKENS: "250000",
  };
  const identity = { repository: "example/factory-qualification", actor: { id: 42, login: "operator" } };
  ...
  it("accepts only real 404 bracketed by accessible same-repository and PR reads", async () => {
    ...
    expect(request.mock.calls.map(([route]) => route)).toEqual([
      "GET /repos/{owner}/{repo}", "GET /repos/{owner}/{repo}/pulls",
      "GET /repos/{owner}/{repo}/stacks", "GET /repos/{owner}/{repo}",
    ]);
    expect(() => assertNativeFallbackCapability(proof, { ...identity, actor: { id: 43, login: "operator" } })).toThrow();
  });
  it.each([200, 401, 403, 422, 429, 500])("does not turn HTTP %s into unsupported capability", async (status) => {
  ...
  it.each([
    ...
    const proof = await observeNativeFallbackCapability({ ...identity, request: fallbackTransport(404, override) });
    expect(proof.result).toBe("blocked");
  });
  it("does not fabricate unsupported evidence from missing permissions or transport failure", async () => {
    const unreachable = vi.fn(async () => { throw new Error("transport unavailable"); });
    expect((await observeNativeFallbackCapability({ ...identity, request: unreachable })).result).toBe("blocked");
    ...
    const denied = vi.fn(async (route: string, parameters: Record<string, unknown>) => {
      if (route.endsWith("/pulls")) throw Object.assign(new Error("denied"), { response: { status: 404 } });
      return real(route, parameters);
    });
    expect((await observeNativeFallbackCapability({ ...identity, request: denied })).result).toBe("blocked");
  ...
  it("reobserves before creation and denies capability changes without a write or retry", async () => {
    const preflight = await observeNativeFallbackCapability({ ...identity, request: fallbackTransport() });
    const qualification = nativeFallbackQualification(env)!;
    const hook = qualification.beforeRun as (input: Record<string, unknown>) => Promise<void>;
    const request = fallbackTransport(200);
    const save = vi.fn();
    await expect(hook({ evidence: { ...identity, preflight: { scenario: preflight } }, request, save })).rejects.toThrow();
  ...
  it("preserves non-fallback escalation rather than upgrading terminal history to a pass", async () => {
    ...
    value.events.find((event) => event.event === "FactoryRunCompleted")!.event = "FactoryRunEscalated";
    expect(assessNativeFallbackCompletion(value).result).toBe("failed");
  });
  it.each(["reason", "version", "requested", "selected", "authorization", "accounting", "scope", "serialization", "merge"])("rejects false fallback proof: %s", async (mutation) => {
    const value = await fallbackEvidence();
    const selected = value.events.find((event) => event.event === "DeliverySelected")!;
    if (mutation === "reason") selected.reason = "GitHub denied stack capability inspection for this repository";
    ...
    if (mutation === "accounting") value.events = value.events.filter((event) => event.phase !== "execution" || event.unit !== "model_tokens");
    if (mutation === "scope") delete (value as unknown as Record<string, unknown>).nativeScopeObservations;
    if (mutation === "serialization") value.events.find((event) => event.workItem === 3 && event.event === "AttemptReserved")!.sequence = selected.sequence + 1;
