...
describe("bounded original checkpoint observation continuation", () => {
  ...
  it("permits only a pinned no-action refusal chain and leaves the original records unchanged", () => {
    const f = fixture(),
      first = `${f.original.authority.evidence}.continuation.json`;
    const input = {
      ...f,
      originalSha256: "1".repeat(64),
      witnessSha256: "2".repeat(64),
      pauseSha256: "3".repeat(64),
      previousPath: first,
      previousSha256: "4".repeat(64),
    };
    const previous = {
      ...structuredClone(f.original),
      authority: { ...f.original.authority, evidence: first },
      actions: [],
      harnessFiles: [
        { path: "scripts/verify-local-checkpoint-continuation.mjs", sha256: "5".repeat(64) },
      ],
      continuation: {
        originalSha256: input.originalSha256,
        witnessSha256: input.witnessSha256,
        pauseSha256: input.pauseSha256,
        originalSourceCommit: f.original.sourceCommit,
        runId: f.witness.runId,
        deadline: "2026-09-07T05:43:07.000Z",
      },
    };
    const before = JSON.stringify({ original: f.original, previous });
    const proof = assertContinuationRetry(previous, input, f.now);
    expect(proof).toEqual({
      outputPath: `${first}.retry-${input.previousSha256}.json`,
      retry: { previousPath: first, previousSha256: input.previousSha256, depth: 1 },
    });
    expect(JSON.stringify({ original: f.original, previous })).toBe(before);
    for (const action of [
      { action: "start", requestedAt: "2026-09-07T05:20:00Z" },
      { action: "resume", returnedAt: "2026-09-07T05:21:00Z" },
    ])
      expect(() =>
        assertContinuationRetry({ ...previous, actions: [action] }, input, f.now),
      ).toThrow(/attempted or uncertain/);
    expect(() =>
      assertContinuationRetry(previous, input, Date.parse("2026-09-07T05:43:07.000Z")),
    ).toThrow(/deadline/);
    expect(() =>
      assertContinuationRetry(
        { ...previous, artifact: { inventorySha256: "changed" } },
        input,
        f.now,
      ),
    ).toThrow();
    expect(() =>
      assertContinuationRetry(
        { ...previous, continuation: { ...previous.continuation, originalSha256: "0".repeat(64) } },
        input,
        f.now,
      ),
    ).toThrow();
  ...
  it("observation-only mode cannot enter the lifecycle continuation tail", async () => {
    ...
    try {
      const port = {
        action,
        preflight: async () => {},
        observe: async () => f.observation,
        graphProof: async () => ({}),
        sessionProof: async () => [{ workItem: 8 }],
        absence: async () => [{ status: "absent" }],
        stopped: async () => ({}),
        checkpoint: async () => {},
      };
      await expect(
        runCheckpointContinuation(port, f.original.authority, {
          ...f,
          artifact: f.original.artifact,
          diagnosticsOnly: true,
        }),
      ).resolves.toMatchObject({ result: "diagnostic-only", lifecycleActionsAttempted: false });
