...
describe("prospective concurrent qualification attempts", () => {
  ...
  it.each([250000, 400000, 500000])("binds both prospective activations to the explicit %i threshold", async (limit) => {
    const selectedEnv = {
      ...env,
    ...
    for (const [index, namespace] of selected.namespaces.entries())
      expect(call).toHaveBeenNthCalledWith(index + 1, "factory_activate", {
        owner: "example", repo: "disposable", objectiveNumber: 10 + index,
        requestId: `${namespace}-activate`, baseSha: evidence.base, policy: selected.policy,
      });
  ...
  it.each([
    [undefined, undefined], [undefined, "1000000"], ["400000", undefined],
    ["400000", "500000"], ["400000", "800001"], ["400000", "0800000"],
    ["249999", "499998"], ["500001", "1000002"], ["0", "0"],
    ["250000.5", "500001"], ["2.5e5", "500000"], [" 250000", "500000"],
    ["0250000", "500000"], ["NaN", "NaN"], ["", "500000"],
    ["9007199254740992", "18014398509481984"],
  ])("refuses invalid per-Objective %s / aggregate %s before any runner action", async (perObjective, aggregate) => {
