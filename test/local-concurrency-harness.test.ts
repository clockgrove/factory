...
const observation = (events: Record<string, unknown>[]) => ({ receipts: events.map((event) => ({ event })) });
function pair(closed = true) {
  return [
    observation([event(1, 1, "ControllerObserved", 0), event(1, 2, "AttemptStarted", 1),
      ...(closed ? [event(1, 3, "AttemptSucceeded", 20), event(1, 4, "AttemptStarted", 22, 11), event(1, 5, "AttemptSucceeded", 25, 11)] : [])]),
    observation([event(2, 1, "ControllerObserved", 0), event(2, 2, "AttemptStarted", 2), event(2, 3, "AttemptSucceeded", 4),
      event(2, 4, "AttemptStarted", 6, 21), event(2, 5, "AttemptSucceeded", 9, 21), event(2, 6, "RunPauseRequested", 10)]),
  ];
}
...
describe("installed two-Objective qualification authority", () => {
  it("preserves controller8 and derives scenario2 from two explicit one-worker policies", () => {
    expect(authority).toMatchObject({ controllerLocalCeiling: 8, authorizedScenarioWorkerMaximum: 2,
      aggregateObservedThreshold: 500000, namespaces: ["concurrency-fixture-a", "concurrency-fixture-b"],
      policy: { maxParallel: 1, capacity: { local: { maxWorkers: 1, reserveCpu: 0.5, reserveMemoryMb: 1024 } },
        allowedPaidBackends: [], economics: { maxModelTokens: 250000, modelTokenBudgetMode: "observed-stop" } } });
  });
  it("requires new explicit two-Objective exercise authority, default-home auth and bounded namespace", () => {
    ...
    expect(() => concurrencyAuthority({ ...env, GH_TOKEN: "synthetic-not-a-credential" })).toThrow();
    expect(() => concurrencyAuthority({ ...env, FACTORY_CONCURRENCY_MAX_MODEL_TOKENS: "1000000" })).toThrow();
    expect(() => concurrencyAuthority({ ...env, FACTORY_CONCURRENCY_NAMESPACE: "a".repeat(48) })).toThrow();
    expect(concurrencyAuthority({ ...env, FACTORY_CONCURRENCY_PHASE: "preflight", FACTORY_CONCURRENCY_ACK: undefined })?.phase).toBe("preflight");
  });
...
describe("independent authenticated timing assertions", () => {
  it("requires a completed B worker and distinct B refill during an A execution lifetime", () => {
    expect(concurrencyRefill(pair())).toMatchObject({ boundary: "authenticated-worker-lifetimes", simultaneousCpu: "not-measured",
      slow: { workItem: 10 }, released: { workItem: 20 }, refill: { workItem: 21 } });
  ...
  it("does not manufacture refill from overlap alone or equal server timestamps", () => {
    const overlap = pair(); overlap[1]!.receipts.splice(3);
    expect(concurrencyRefill(overlap)).toBeNull();
    const tied = pair(); tied[1]!.receipts[3]!.event.at = at(4);
    expect(concurrencyRefill(tied)).toBeNull();
    const late = pair(); late[1]!.receipts[3]!.event.at = at(21);
  ...
  it("rejects two concurrently started workers in the same one-worker Objective", () => {
    const invalid = pair(); invalid[1]!.receipts[2]!.event.sequence = 5;
  ...
  it("rejects conflicting terminal events instead of guessing the shorter lifetime", () => {
    const invalid = pair(); invalid[1]!.receipts.push({ event: event(2, 7, "AttemptFailed", 5) });
...
describe("actual inner Director lease proof", () => {
  ...
  const before = { oid: "b".repeat(40), parents: ["a".repeat(40)], event: { ...start, protocol: "clockgrove.factory/v2", kind: "lease", event: "LeaseRenewed", holder: "old", epoch: 1, sequence: 5, at: at(10) } };
  const after = { oid: "c".repeat(40), parents: [before.oid], event: { ...before.event, holder: "new", epoch: 2, sequence: 6, previousOid: before.oid, event: "LeaseAcquired", at: at(15) } };
  it("proves real inner serial takeover without claiming a simultaneous race", () => {
    expect(assertInnerTakeover(before, after, [before], start)).toMatchObject({ boundary: "inner-Director-serial-takeover", simultaneousRace: "not-exercised", originalEpoch: 1, replacementEpoch: 2 });
  });
  it("refuses outer-lease substitution, detached ancestry, same epoch and foreign policy", () => {
    expect(() => assertInnerTakeover(before, { ...after, event: { ...after.event, kind: "repository-lease" } }, [before], start)).toThrow();
    expect(() => assertInnerTakeover(before, { ...after, parents: ["d".repeat(40)] }, [before], start)).toThrow();
    expect(() => assertInnerTakeover(before, { ...after, event: { ...after.event, epoch: 1 } }, [before], start)).toThrow();
    expect(() => assertInnerTakeover(before, after, [before], { ...start, policyDigest: "f".repeat(64) })).toThrow();
  });
});
...
describe("bounded existing installed-controller composition", () => {
  it("does no exercise action in preflight", async () => {
    ...
    expect(await runConcurrencyScenario(f.port, { ...authority, phase: "preflight" })).toMatchObject({ result: "preflight-only" });
  ...
  it("uses committed checkpoint main extension and inventories all new evidence dependencies", async () => {
    ...
    await main(env, run);
    expect(run).toHaveBeenCalledWith(env, runConcurrencyScenario, expect.objectContaining({
      authority, scope: "installed-two-objective-concurrency", harnessPaths: expect.arrayContaining(["scripts/verify-local-concurrency.mjs", "scripts/qualification-sibling-refresh-proof.mjs", "scripts/qualification-model-accounting.mjs"]),
    }));
  });
});

describe("retained artifact proof refuses unbound Git inputs before local evaluation", () => {
  const finalSha = "a".repeat(40), treeSha = "b".repeat(40), blobSha = "c".repeat(40);
  ...
  it("refuses an unrelated default-branch tip", async () => {
    ...
    await expect(verifyConcurrencyArtifacts(request, authority, "main", finalEvidence)).rejects.toThrow(/outside exact proved integrations/);
    expect(request).toHaveBeenCalledTimes(1);
  });
  it.each(["120000", "100755"])("refuses unexpected file mode %s rather than materializing it", async (mode) => {
    const request = vi.fn(async (route: string) => route.endsWith("commits/{ref}")
      ? { data: { sha: finalSha, commit: { tree: { sha: treeSha } } } }
      : { data: { truncated: false, tree: [{ path: qualificationPaths(authority.namespaces[0]!).files[0], mode, type: "blob", sha: blobSha, size: 3 }] } });
    await expect(verifyConcurrencyArtifacts(request, authority, "main", finalEvidence)).rejects.toThrow();
  ...
  it("refuses truncated tree evidence and oversized blobs without downloading their contents", async () => {
    ...
    const request = vi.fn(async (route: string) => route.endsWith("commits/{ref}")
      ? { data: { sha: finalSha, commit: { tree: { sha: treeSha } } } } : { data: tree });
    await expect(verifyConcurrencyArtifacts(request, authority, "main", finalEvidence)).rejects.toThrow();
    tree.truncated = false;
    tree.tree = [{ path: qualificationPaths(authority.namespaces[0]!).files[0], mode: "100644", type: "blob", sha: blobSha, size: 65537 }];
    await expect(verifyConcurrencyArtifacts(request, authority, "main", finalEvidence)).rejects.toThrow();
