...
const env = (scenario: "failed-validation" | "real-conflict" = "failed-validation") => ({
  ...
  FACTORY_FAILURE_CASE: scenario,
  FACTORY_FAILURE_REPOSITORY: repository,
  FACTORY_FAILURE_CHECKOUT: checkout,
  FACTORY_FAILURE_CONTROLLER_UNIT: unit,
  FACTORY_FAILURE_NAMESPACE: namespace,
  FACTORY_FAILURE_PHASE: "exercise",
  FACTORY_FAILURE_EVIDENCE: "/tmp/private/failure.json",
...
function observation(failed = true) {
  ...
  const events: Event[] = [
    ...
    },
    {
      ...
      phase: "execution",
      unit: "model_tokens",
      modelInvocationId: "worker-7-1",
      usageId: "invocation-worker-7-1",
      amount: 0,
    },
  ...
  if (failed)
    events.push(
      ...
      },
      {
        ...
        kind: "attempt",
        event: "AttemptFailed",
        reason: `validation failed (1): ${fixture.validationCommand}\nOutput tail:\nfactory-qualification-invalid-value`,
      },
      { kind: "run", event: "FactoryRunEscalated" },
    );
  else
    events.push({ kind: "command", event: "RunPauseRequested", requestId: `${namespace}-pause` });
  ...
  return {
    ...
    value: {
      receipts,
      status: { run: { runId: "original" } },
      children: [{ number: 7, state: "open" }],
    },
  };
}
...
describe("installed failed-validation/conflict authority and evidence", () => {
  it("has no default action and binds distinct lifecycle/CAS/cancel authority", async () => {
    ...
    try {
      await main({});
      expect(failureAuthority({})).toBeNull();
    } finally {
      log.mockRestore();
    }
    expect(() =>
      failureAuthority({ ...env("real-conflict"), FACTORY_FAILURE_ACK: env().FACTORY_FAILURE_ACK }),
    ).toThrow();
  ...
  it("requires actual immutable-command failure, complete accounting and no publication", () => {
    ...
    expect(
      assertFailedValidation(value, authority, failureFixture(namespace, "failed-validation")),
    ).toMatchObject({ workItem: 7, accounting: { modelTokens: 30 } });
    for (const mutate of [
      (events: Event[]) => {
        events.find((event) => event.event === "ValidationRecorded")!.passed = true;
      },
      (events: Event[]) => {
        events.find((event) => event.event === "AttemptFailed")!.reason =
          "model-token budget exhausted";
      },
      (events: Event[]) => {
        events.find(
          (event) => event.unit === "local_milliseconds" && event.event === "BudgetReconciled",
        )!.usageEvidence = "conservative-reservation";
      },
      (events: Event[]) => {
        events.find(
          (event) => event.modelInvocationId === "worker-7-1" && event.event === "BudgetReconciled",
        )!.modelInvocationId = "wrong-call";
      },
      (events: Event[]) => {
        events.find((event) => event.event === "CapacityReconciled")!.event = "CapacityReserved";
      },
      (events: Event[]) => {
        events.push({
          ...events.at(-1),
      ...
      (events: Event[]) => {
        events.find((event) => event.event === "GraphProjected")!.graphSize = 2;
      },
    ]) {
      const changed = structuredClone(value);
      const raw = value.receipts.map(({ event }) => structuredClone(event));
      mutate(raw);
      changed.receipts = raw.map((event, index) => ({ event, commentId: index + 100, actorId: 2 }));
      expect(() =>
        assertFailedValidation(changed, authority, failureFixture(namespace, "failed-validation")),
      ).toThrow();
    }
  });
  ...
  it("keeps zero dispatch markers unknown and refuses prior or wrong-binding closure", () => {
    ...
    const actual = events.find(
      (event) => event.event === "BudgetReconciled" && event.modelInvocationId === "worker-7-1",
    )!;
    expect(() => assertFailureAccounting(events.filter((event) => event !== actual))).toThrow(
      /unknown model/,
    );
    expect(() =>
      assertFailureAccounting(
        events.map((event) => (event === actual ? { ...event, sequence: 1 } : event)),
      ),
    ).toThrow();
    expect(() =>
      assertFailureAccounting(
        events.map((event) => (event === actual ? { ...event, directorEpoch: 2 } : event)),
      ),
    ).toThrow();
    expect(() =>
      assertFailureAccounting(
        events.map((event) =>
          event.event === "BudgetReserved" && event.modelInvocationId
            ? { ...event, amount: 1 }
            : event,
        ),
      ),
    ...
    expect(() =>
      assertFailureAccounting(
        events.map((event) =>
          event === actual ? { ...event, modelInvocationId: undefined } : event,
        ),
      ),
    ).toThrow(/linkage/);
  });
  ...
  it("refuses repeated work after conflict and requires explicit same-run cancellation closeout", () => {
    ...
    cancelled.receipts.push({
      actorId: 2,
      commentId: 502,
      event: { ...value.receipts[8]!.event, attempt: 2, sequence: 52 },
    });
  ...
  it("binds exact refusal notification to current invocation, activation, two heads and time", () => {
    ...
    const row = {
      MESSAGE,
      _SYSTEMD_INVOCATION_ID: producer.invocationId,
      __REALTIME_TIMESTAMP: String(Date.parse("2026-09-06T10:00:01Z") * 1000),
    };
    expect(assertRefusalJournal([row], { namespace }, competing, producer)).toMatchObject({
      message: MESSAGE,
    });
    for (const changed of [
      { ...row, MESSAGE: "controller-invariant-failure" },
      { ...row, MESSAGE: MESSAGE.replace(namespace, "other-one") },
      { ...row, _SYSTEMD_INVOCATION_ID: "b".repeat(32) },
      { ...row, __REALTIME_TIMESTAMP: "1000000000000000" },
    ])
  ...
  it("executes only the failed-validation scenario ports in order and never retries uncertain activation", async () => {
    ...
    const port: FailurePort = {
      ...
      controller: async () => ({ invocationId: "owned" }),
      observe: async () => value,
      poll: async (_phase, accept) => {
        expect(accept(value)).toBe(true);
        return value;
      },
      absence: async () => ["absent"],
      checkpoint: async () => {},
      takeover: async () => {},
      finalProof: async () => {},
      contentProof: async () => ({
      ...
      noPublication: async () => {},
      stopOriginal: async () => {
        throw Error("wrong scenario");
      },
      compete: async () => {
        throw Error("wrong scenario");
      },
      assertCompeting: async () => {},
      refusal: async () => {
        throw Error("wrong scenario");
      },
    };
    expect(await runFailureScenario(port, authority)).toMatchObject({
      result: "passed",
      scenario: "failed-validation",
    });
    expect(actions).toEqual(["start", "create", "activate", "stop"]);
    actions.length = 0;
    port.action = async (action) => {
      actions.push(action);
      if (action === "activate") throw Error("uncertain response");
    };
  ...
  it("orders real-conflict injection after complete output/absence and requires cancelled closeout", async () => {
    ...
    const port: FailurePort = {
      ...
      controller: async (state) =>
        state === "inactive" ? { state } : restarted ? replacement : original,
      observe: async () => (cancelled ? terminal() : held),
      poll: async (_phase, accept) => {
        const current = cancelled ? terminal() : held;
        expect(accept(current)).toBe(true);
        return current;
      },
      armSession: async () => {
        actions.push("arm");
      ...
      contentProof: async () => ({
        ...
        objects: {
          ...
          conflict: {
            exitCode: 1,
            output: "captured raw Git conflict fixture",
            outputDigest: digest,
            boundary: "raw Git",
          },
        },
      ...
      noPublication: async () => {},
      stopOriginal: async () => {
        actions.push("stop-original");
      },
      compete: async () => {
        expect(actions).toContain("absence");
        expect(actions.at(-1)).toBe("stop-original");
        actions.push("cas");
        return {};
      },
      assertCompeting: async () => {},
    ...
    expect(await runFailureScenario(port, authority)).toMatchObject({
      result: "passed",
      originalRunCancelled: true,
      trunkRestored: false,
    });
    expect(actions.filter((action) => action !== "absence")).toEqual([
      "start",
      "create",
      "arm",
      "activate",
    ...
    restarted = false;
    port.compete = async () => {
      actions.push("cas-uncertain");
      throw Error("lost conditional response");
    };
    ...
    actions.length = 0;
    port.absence = async () => {
      throw Error("resource absence unknown");
    };
...
describe("independent real Git content proof (not live worker evidence)", () => {
  it("proves the retained patch conflicts and never checks out or invokes repository hooks", () => {
    ...
    const git = (args: string[], input?: string) => {
      const result = rawFailureGit(root, args, input, {
    ...
    try {
      ...
      const artifact = {
        digest,
        baseSha: sourceBase,
        changedPaths: [fixture.paths.payload],
        outcome: "succeeded",
        patch,
      };
      const proof = proveFailureContent({
        repository: root,
        baseSha: sourceBase,
        artifact,
        fixture,
      ...
      expect(proof.conflict?.output).toContain(
        `CONFLICT (content): Merge conflict in ${fixture.paths.payload}`,
      );
      ...
      expect(() =>
        proveFailureContent({
          repository: root,
          baseSha: sourceBase,
          artifact: { ...artifact, patch: patch.replace("+worker", "+operator") },
          fixture,
        }),
      ).toThrow();
      expect(() =>
        proveFailureContent({
          repository: root,
          baseSha: sourceBase,
          artifact: { ...artifact, changedPaths: [fixture.paths.test] },
          fixture,
        }),
      ).toThrow();
      expect(() =>
