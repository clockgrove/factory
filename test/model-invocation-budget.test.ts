...
function budget(event: "BudgetReserved" | "BudgetReconciled", overrides: Record<string, unknown> = {}): BudgetEvent {
...
describe("durable model dispatch intent", () => {
  it("keeps a zero-valued marker unknown, not known-zero observed usage", () => {
    ...
    expect(() => assertModelInvocationAdmission([marker], policy)).toThrow(/consumption is unknown/);
    expect(() => assertModelInvocationAdmission([marker], DEFAULT_RUN_POLICY)).toThrow(/consumption is unknown/);
  });
  ...
  ])("rejects a malformed marker rather than assigning it fake budget authority: %j", (override) => {
    expect(() => budget("BudgetReserved", override)).toThrow();
  });

  it("rejects a reconciliation disguised as the marker's zero receipt", () => {
    expect(() => budget("BudgetReconciled", {
      amount: 0, usageId: `invocation-${invocation}`,
    })).toThrow();
  });
  ...
  it.each([
    ...
    expect(() => assertModelInvocationAdmission([marker, unrelated], policy)).toThrow(/consumption is unknown/);
  });
  ...
  it("collapses exact and equivalent marker replay, but rejects a conflicting authenticated sequence", () => {
    ...
    expect(() => unresolvedModelInvocations([
      marker, budget("BudgetReserved", { phase: "execution" }),
    ])).toThrow(/conflicting Factory events/i);
  });

  it.each([0, 30, 175])("closes only with actual counters and never clamps %i tokens to the threshold", (amount) => {
    const marker = budget("BudgetReserved");
    ...
    if (amount >= 100)
      expect(() => assertModelInvocationAdmission([marker, actual], policy)).toThrow(/threshold is exhausted/);
  ...
  it.each([{ amount: 31 }, { usageId: "different-paid-call" }])(
    ...
    (override) => {
      ...
      expect(() => unresolvedModelInvocations([marker, actual, conflicting])).toThrow(/conflicting actual usage/);
      expect(() => assertModelInvocationAdmission([marker, actual, conflicting], policy)).toThrow(/conflicting actual usage/);
    },
  );
  ...
  it.each([
    ...
    expect(() => unresolvedModelInvocations([marker, conflicting])).toThrow(/conflicting dispatch bindings/);
  });

  it.each([false, true])("does not use accounting preceding a dispatch as replacement authority (reversed input: %s)", (reversed) => {
  ...
  it("distinguishes active process ownership from restart and missing accounting", () => {
    ...
    expect(() => assertModelInvocationAdmission(restarted, policy)).toThrow(/consumption is unknown/);
    expect(() => assertModelInvocationAdmission([marker], policy, new Set([
      modelInvocationKey({ ...identity, attempt: 2 }),
    ]))).toThrow(/consumption is unknown/);
    expect(() => assertModelInvocationAdmission([marker, budget("BudgetReconciled")], policy)).not.toThrow();
  ...
  it("keeps historical unlinked usage readable, but cannot use it to resolve a new marker", () => {
    ...
    expect(() => assertModelInvocationAdmission([historical, marker], policy)).toThrow(/consumption is unknown/);
  });
  ...
  it("does not let live-marker ownership bypass unsupported hard intent", () => {
    ...
    expect(() => assertModelInvocationAdmission([marker], hard, new Set([
      modelInvocationKey({ ...marker, modelInvocationId: invocation }),
  ...
  it("releases only process-local ownership after a failed operation, not its durable dispatch liability", async () => {
    ...
    await expect(scopes.run(async () => {
      ...
      expect(() => assertModelInvocationAdmission([marker], policy, restarted.active)).toThrow(/consumption is unknown/);
    ...
    expect(unresolvedModelInvocations([marker])).toEqual([marker]);
    expect(() => assertModelInvocationAdmission([marker], policy, scopes.active)).toThrow(/consumption is unknown/);
  });
