...
async function fixture(
  ...
  const policy = parseRunPolicy({
    ...
    delivery: { mode: options.regular ? "regular-prs" : "stacked-prs", onUnavailable: "escalate", merge: "bottom-up" },
  });
  ...
  for (const [index, item] of graph.workItems.entries()) {
    ...
    const reviewIdentity = { kind: "artifact" as const, runId: "parallel", objective: 7,
      workItem: number, attempt: 1, artifactDigest: validationDigest, baseSha,
      outputTreeSha: tree, evidenceDigest: validationDigest };
    await new ReviewCheckpointManager(storage, leases).persist({ lease, identity: reviewIdentity,
      result: { review: { accepted: true, summary: "Original artifact accepted", unmetCriteria: [], risks: [] },
    ...
    snapshot.workItems.push({
      ...
      factoryEvents: [
        ...
        event({ kind: "budget", event: "BudgetReconciled", workItem: number, attempt: 1,
          phase: "management", unit: "model_tokens", amount: 15,
  ...
  const controller = { controllerId: "shared-controller", epoch: 1,
    expiresAt: new Date(Date.now() + 600_000).toISOString(), controllerPolicyDigest: "c".repeat(64) };
  ...
  if (options.peerAdvance) {
    ...
    git("add", "."); git("commit", "-qm", "peer artifact");
    peerHead = git("rev-parse", "HEAD");
    git("checkout", "-q", "main"); git("merge", "--squash", peerHead); git("commit", "-qm", "peer squash");
    ...
    const peerProjection = await graphManager.persistProjection({ lease: peerLease, graph: peerGraph,
      bindings: [{ compilerId: "peer", issueNodeId: "I_88", issueNumber: 88 }] });
    const validationDigest = createHash("sha256").update("peer").digest("hex");
    const reviewIdentity = { kind: "artifact" as const, runId: "peer", objective: 6, workItem: 88,
      attempt: 1, artifactDigest: validationDigest, baseSha, outputTreeSha: peerTree, evidenceDigest: validationDigest };
    if (!options.missingPeerReview) await new ReviewCheckpointManager(storage, leases).persist({
      lease: peerLease, identity: reviewIdentity, result: { review: { accepted: true, summary: "Peer accepted",
        unmetCriteria: [], risks: [] }, usage: { inputTokens: 10, outputTokens: 5 } } });
    const exact = bindValidationToPublishedHead({ validation: { passed: true, digest: validationDigest,
      baseSha, outputTreeSha: peerTree }, publishedBaseSha: baseSha, publishedTreeSha: peerTree, publishedHeadSha: peerHead });
    const peerEvent = (fields: Record<string, unknown>) => event({ ...fields, objective: 6, runId: "peer", sequence: sequence++ });
    const item = structuredClone(snapshot.workItems[0]!);
    Object.assign(item, { id: "I_88", number: 88, title: "peer", closed: true,
      body: renderWorkPacket(peerItem, { protocol: "clockgrove.factory/graph-v1", id: "peer",
        graphDigest: peerGraph.graphDigest, graphSize: 1, index: 0, dependsOn: [] }) });
    item.linkedPullRequests = [{ ...item.linkedPullRequests[0]!, id: "PR_88", number: 88,
      state: "MERGED", headSha: peerHead, changedFilePaths: ["peer.txt"] }];
    item.factoryEvents = item.factoryEvents!.map((original) => peerEvent({ ...original, workItem: 88,
      ...(original.kind === "attempt" && original.artifactDigest ? { artifactDigest: validationDigest } : {}),
      ...(original.kind === "attempt" && original.headSha ? { headSha: peerHead } : {}),
      ...(original.kind === "validation" ? { outputTreeSha: peerTree, evidenceDigest: validationDigest } : {}),
      ...(original.kind === "budget" && original.phase === "management" ? { usageId: `review-${reviewIdentityDigest(reviewIdentity)}` } : {}),
      ...(original.kind === "publication" ? { unitId: "delivery/peer", itemId: "peer", headSha: peerHead,
        branch: publicationBranch(6, 88, 1), pullRequest: 88, validationDigest,
        exactHeadValidationDigest: exact.digest } : {}),
    }));
    ...
    commits.set(reservedOid, { oid: reservedOid, treeOid: (await readCommit(baseSha)).treeOid,
      parentOids: [baseSha], message: encodeEventTrailer(reserved), serverTime: now });
    item.factoryEvents.push(peerEvent({ ...reserved, event: "AttemptIntegrated", headSha: peerMergeSha }));
    peerSnapshot = { ...structuredClone(snapshot), number: 6, id: "I_6", title: "Peer Objective", closed: true,
      workItems: [item], factoryEvents: [
        peerEvent({ kind: "run", event: "ActivationRequested", requestId: "peer-activation", requestedBy: "operator",
          repository: "o/r", baseSha, policy, policyDigest: pd, controllerProtocolMin: "clockgrove.factory/v2",
          controllerProtocolMax: "clockgrove.factory/v2" }),
        peerEvent({ kind: "run", event: "FactoryRunStarted", activationRequestId: "peer-activation", actor: "operator",
          repository: "o/r", objectiveAuthor: "operator", fork: false, baseBranch: "main", baseSha, policy, policyDigest: pd }),
        peerEvent({ kind: "controller", event: "ControllerObserved", ...controller,
          ...(options.foreignPeerGeneration ? { controllerId: "foreign-controller" } : {}),
          protocolMin: "clockgrove.factory/v2", protocolMax: "clockgrove.factory/v2" }),
        peerEvent({ kind: "graph", event: "GraphCompiled", graphDigest: peerGraph.graphDigest,
          graphSize: 1, baseSha, graphRef: peerGraph.ref, graphBlobSha: peerGraph.blobOid }),
        peerEvent({ kind: "graph", event: "GraphProjected", graphDigest: peerGraph.graphDigest, graphSize: 1,
          projectionRef: peerProjection.ref, projectionBlobSha: peerProjection.blobOid }),
        peerEvent({ kind: "run", event: "FactoryRunCompleted", policyDigest: pd }),
      ] };
    const terminal = peerSnapshot.factoryEvents!.pop()!;
    let peerSequence = 1;
    peerSnapshot.factoryEvents = peerSnapshot.factoryEvents!.map((entry) => parseFactoryEvent({ ...entry,
      sequence: peerSequence++, ...(entry.event === "ActivationRequested" ? { runId: "peer-activation" } : {}) }));
    item.factoryEvents = item.factoryEvents.map((entry) => parseFactoryEvent({ ...entry, sequence: peerSequence++ }));
    peerSnapshot.factoryEvents.push(parseFactoryEvent({ ...terminal, sequence: peerSequence }));
    commits.get(reservedOid)!.message = encodeEventTrailer(item.factoryEvents.find((entry) => entry.event === "AttemptReserved")!);
  }
  ...
  vi.spyOn(GitHubControlStore.prototype, "readCommitObjectiveCandidates").mockImplementation(async (sha) =>
    sha === peerMergeSha ? [6] : []);
  ...
  const findPull = (number: number) =>
    [...snapshot.workItems, ...(peerSnapshot?.workItems ?? [])].find((item) => item.linkedPullRequests[0]!.number === number)!
...
describe("Supervisor parallel independent sibling integration", () => {
  ...
  it.each([false, true])("accepts an authenticated terminal co-owned Objective's exact squash (regular=%s) without resuming it", async (regular) => {
    ...
    expect(f.launch).not.toHaveBeenCalled();
    expect(vi.mocked(LeaseManager.prototype.acquire).mock.calls.every(([identity]) => identity.objective === 7)).toBe(true);
  });

  it.each(["foreignPeerGeneration", "missingPeerReview"] as const)("rejects peer history with %s before any merge or paid candidate review", async (fault) => {
  ...
  it.each(["failCombinedTests", "rejectReview", "externalAdvance"] as const)("regular concurrent integration does not waive %s", async (fault) => {
  ...
  it("refreshes only B's delivery head, then fully validates and reviews it on A's exact merge", async () => {
    ...
    expect(
      documents.find((record) => record.protocol === "clockgrove.factory/review-checkpoint-v1" && record.identity.kind === "integration-candidate")
        ?.identity.headSha,
    ).toBe(refreshed);
