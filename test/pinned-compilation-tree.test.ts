...
describe("hook/filter-free exact compilation trees", () => {
  it("accepts immediate index-bound patch application without an incidental status refresh", async () => {
    ...
    expect(
      execFileSync("git", ["show", ":input.txt"], { cwd: pinned.path, encoding: "utf8" }),
    ).toBe("changed\n");
  });
  it("strips inherited Git configuration injection and materializes a mixed-size batch", async () => {
    ...
    await Promise.all(
      Array.from({ length: 64 }, (_, index) =>
        writeFile(join(f.repository, `batch-${index}.txt`), "x".repeat(index)),
      ),
    );
    f.git("add", ".");
    f.git("commit", "-qm", "batch");
  ...
  it("pins files, index and HEAD independently of dirty or advancing shared trunk", async () => {
    ...
    expect(
      execFileSync("git", ["rev-parse", "HEAD"], { cwd: a.path, encoding: "utf8" }).trim(),
    ).toBe(f.base);
    expect(
      execFileSync("git", ["status", "--porcelain"], { cwd: a.path, encoding: "utf8" }).trim(),
    ).toBe("");
  ...
  it("does not run source hooks or smudge filters and preserves binary bytes", async () => {
    ...
    await writeFile(
      join(f.repository, ".git", "hooks", "post-checkout"),
      "#!/bin/sh\ntouch HOOK_RAN\nexit 1\n",
      { mode: 0o700 },
    );
    ...
    await expect(stat(join(pinned.path, "CLEAN_FILTER_RAN"))).rejects.toMatchObject({
      code: "ENOENT",
    });
    await expect(stat(join(pinned.path, ".git", "hooks", "post-checkout"))).rejects.toMatchObject({
      code: "ENOENT",
    });
  });
  ...
  it("rejects escaping symlink entries without materializing or executing them", async () => {
    ...
    f.git("add", ".");
    f.git("commit", "-qm", "unsafe entry");
    await expect(
      materializePinnedCompilationTree(f.repository, f.git("rev-parse", "HEAD")),
