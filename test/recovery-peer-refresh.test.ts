...
async function fixture(sameObjective = false, peerNonhost = false) {
  ...
  const storage: CompiledGraphStore = {
    ...
    readCommit: async (oid) => {
      ...
      if (!value) throw new Error("missing exact commit");
      return value;
    },
    readBlob: async (oid) => {
      const value = blobs.get(oid);
      if (!value) throw new Error("missing exact blob");
      return value;
    },
    readTreeEntry: async (oid, path) => trees.get(oid)?.get(path) ?? null,
    createBlob: async (bytes) => {
      const oid = sha(bytes.toString());
      blobs.set(oid, bytes);
      return oid;
    },
    ...
    createCommit: async (input) => {
      const oid = sha(`commit-${counter++}`);
      commits.set(oid, { ...input, oid, serverTime: new Date(at) });
      return oid;
    },
  ...
  async function member(objective: number, runId: string, numbers: number[]) {
    ...
    const compiled: CompiledObjective = {
      ...
      workItems: numbers.map((number) => ({
        id: `item-${number}`,
        title: `Item ${number}`,
        goal: "Implement bounded independent change",
        acceptance: ["Declared check passes"],
        scope: [`src/${number}.ts`],
        preconditions: [],
        outOfScope: [],
        conventions: [],
    ...
    const graph = await manager.persist({
      ...
      base,
      objective: compiled,
      compilation: { invocationId: `compile-${runId}`, inputTokens: 10, outputTokens: 5 },
    });
    const projection = await manager.persistProjection({
      lease,
      graph,
      bindings: numbers.map((number) => ({
        compilerId: `item-${number}`,
        issueNumber: number,
    ...
    const activation = event({
      ...
      runId: `request-${objective}`,
      requestedBy: "operator",
      requestId: `activate-${objective}`,
      repository: "o/r",
      baseSha: base.oid,
      policy,
    ...
    const start = event({
      ...
      policy,
      policyDigest: lease.policyDigest,
      activationRequestId: `activate-${objective}`,
    });
    ...
    const snapshot: FactoryReadSnapshot = {
      ...
      workItems: compiled.workItems.map((item, index) => ({
        id: `I_${numbers[index]}`,
  ...
  async function publish(
    ...
    const attempt = (fields: Record<string, unknown>) =>
      event({
        ...
        workItem,
        attempt: 1,
        backend: remote ? "codex-cli/daytona" : "codex-sdk/local-worktree",
        baseSha: base.oid,
    ...
    commits.set(head, {
      ...
      treeOid: tree,
      parentOids: [base.oid],
      message: `head\nFactory-Artifact: ${artifactDigest}\nFactory-Validation: ${validation.digest}`,
      serverTime: new Date(at),
    });
    ...
    item.factoryEvents!.push(
      ...
      event({
        ...
        attempt: 1,
        phase: "execution",
        unit: remote ? "sandbox_milliseconds" : "local_milliseconds",
        amount: 20,
      }),
      event({
        kind: "budget",
      ...
      attempt({ event: "AttemptPublished", artifactDigest, headSha: head }),
    );
    ...
    const pull: Pull = {
      ...
      baseSha: base.oid,
      baseRef: "main",
      mergeCommitSha: integrated ? merge : null,
    ...
    item.linkedPullRequests = [
      {
        ...
        mergeable: "MERGEABLE",
        createdAt: new Date(at),
        headCommittedAt: new Date(at),
        mergedAt: integrated ? new Date(at) : null,
        closedAt: integrated ? new Date(at) : null,
        agentWorkEvents: [],
      },
    ];
    return {
  ...
  const source: Parameters<typeof observeRecoverySiblingRefresh>[0]["source"] = {
    runId: receiver.lease.runId,
    attempt: 1,
    reservationRef: original.reservationRef,
    reservationCommitOid: original.reservationOid,
    reservationReceiptDigest: recoveryEventDigest(original.reserved),
    artifactDigest: original.validation.artifactDigest,
    review: null,
    validation: {
      receiptDigest: recoveryEventDigest(original.validationEvent),
      evidenceDigest: original.validation.digest,
      baseSha: base.oid,
      outputTreeSha: original.tree,
    },
    publication: {
      receiptDigest: recoveryEventDigest(original.publication),
      mode: "regular-prs",
      pullRequest: 29,
      pullRequestNodeId: "PR_29",
  ...
  const record = await new SiblingRefreshStore(storage, leases).persist({
    lease: receiver.lease,
    identity: {
      repository: "o/r",
      runId: receiver.lease.runId,
      sourceRunId: receiver.lease.runId,
      controllingPolicyDigest: receiver.lease.policyDigest,
      objective: 7,
      workItem: 9,
      attempt: 1,
      pullRequest: 29,
      ...
      branch: original.branch,
      reservationRef: original.reservationRef,
      reservationOid: original.reservationOid,
      leaseEpoch: 1,
      policyDigest: receiver.lease.policyDigest,
      sourcePublicationDigest: source.publication!.receiptDigest,
      sourceHeadSha: original.head,
      sourceExactHeadValidationDigest: original.exact.digest,
      targetBaseSha: peerPublication.merge,
    },
    source: original.exact,
    expectedOldHeadSha: original.head,
    outputTreeSha: sha("combined-tree"),
  });
  const validation = createValidationEvidence({
    protocol: "clockgrove.factory/validation-v1",
    artifactDigest: digest("candidate"),
    baseSha: peerPublication.merge,
  ...
  const candidate = await new MergeCandidateCheckpointStore(storage, leases).persist({
    lease: receiver.lease,
    identity,
    source: original.exact,
    validation,
  ...
  const store: RecoveryReadStore = {
    ...
    readPullRequest: async (number) => {
      ...
      if (!pull) throw new Error("missing exact pull");
      return pull;
    },
    ...
    getBranchHead: async () => commits.get(peerPublication.merge)!,
    ...
    readObjectiveSnapshot: vi.fn(async (number) => {
      const snapshot = snapshots.get(number);
      if (!snapshot) throw new Error("missing authenticated snapshot");
      return snapshot;
    }),
  };
  const observe = () =>
    observeRecoverySiblingRefresh({
      ...
      source,
      events: events(),
      controllingRunIds: [receiver.lease.runId],
      store,
      deliveryHeadSha: record.plannedHeadSha,
      requireCompletion: true,
    });
  const peerProof = (
    overrides: Partial<Parameters<typeof verifyRecoveryPeerTrunkIntegration>[0]> = {},
  ) => {
    if (receiver.start.event !== "FactoryRunStarted") throw new Error("fixture start");
    return verifyRecoveryPeerTrunkIntegration({
      repository: "o/r",
      receiverObjective: 7,
      receiverStart: receiver.start,
...
function peerEvent(f: Fixture, name: FactoryEvent["event"]) {
  const event = [
    ...f.peer.snapshot.factoryEvents!,
    ...f.peer.snapshot.workItems.flatMap((item) => item.factoryEvents!),
...
describe("retained regular publication refreshed onto authenticated peer integration", () => {
  it("replays exact captured cross-Objective refresh without relabelling either original acceptance", async () => {
    ...
    await expect(f.peerProof()).resolves.toEqual({
      parent: f.original.validation.baseSha,
      requiresIsolation: false,
      executionRequiresIsolation: false,
    });
  ...
  it("preserves a permitted non-host peer's isolation requirement instead of accepting local refreshed validation", async () => {
    ...
    expect(
      f.peerPublication.item.factoryEvents!.some(
        (event) =>
  ...
  const invalid: Array<[string, (f: Fixture) => void]> = [
    [
      "peer actor",
      (f) => {
        Object.assign(f.peer.start, { actor: "foreign" });
      },
    ],
    [
      "peer Objective author",
      (f) => {
        f.peer.snapshot.authorLogin = "foreign";
      },
    ],
    [
      "repository identity",
      (f) => {
        f.peer.snapshot.repositoryId = "R_foreign";
      },
    ],
    [
      "default branch",
    ...
    [
      "run policy digest",
      (f) => {
        Object.assign(f.peer.start, { policyDigest: digest("other-policy") });
      },
    ],
    [
      "activation actor",
      (f) => {
        Object.assign(f.peer.activation, { requestedBy: "foreign" });
      },
    ],
    [
      "activation identity",
      (f) => {
        Object.assign(f.peer.start, { activationRequestId: "missing-activation" });
      },
    ],
    [
      "activation base",
      (f) => {
        Object.assign(f.peer.activation, { baseSha: sha("other-base") });
      },
    ],
    [
      "activation policy",
      (f) => {
        Object.assign(f.peer.activation, { policyDigest: digest("other-policy") });
      },
    ],
    [
      "compiled graph receipt",
      (f) => {
        Object.assign(peerEvent(f, "GraphCompiled"), { graphDigest: digest("other-graph") });
      },
    ],
    [
      "projection receipt",
      (f) => {
        Object.assign(peerEvent(f, "GraphProjected"), {
          projectionBlobSha: sha("other-projection"),
        });
      },
    ],
    [
      "immutable projection reference",
      (f) => {
        f.refs.delete(f.peer.projection.ref);
      },
    ],
    [
      "projected issue identity",
      (f) => {
        f.peerPublication.item.id = "I_foreign";
      },
    ],
    [
      "projected packet",
      (f) => {
        f.peerPublication.item.body += "\nAdditional edit authority";
      },
    ],
    [
      "controller observed after reservation",
      (f) => {
        Object.assign(f.peer.controller, { sequence: f.peerPublication.reserved.sequence + 1 });
      },
    ],
    [
      "receiver observed after reservation",
      (f) => {
        Object.assign(f.receiver.controller, { sequence: f.original.reserved.sequence + 1 });
      },
    ],
    [
      "peer integration before reservation",
      (f) => {
        Object.assign(peerEvent(f, "AttemptIntegrated"), {
          sequence: f.peerPublication.reserved.sequence,
        });
      },
    ],
    [
      "peer integration after receiver horizon",
      (f) => {
        Object.assign(peerEvent(f, "AttemptIntegrated"), { at: "2026-09-07T00:00:01.000Z" });
      },
    ],
    [
      "original head parent",
      (f) => {
        f.commits.get(f.peerPublication.head)!.parentOids = [sha("foreign-base")];
      },
    ],
    [
      "original head tree",
      (f) => {
        f.commits.get(f.peerPublication.head)!.treeOid = sha("foreign-tree");
      },
    ],
    [
      "actual PR head",
      (f) => {
        f.peerPublication.pull.headSha = sha("unknown-head");
      },
    ],
    [
      "actual PR repository",
      (f) => {
        f.peerPublication.pull.headRepository = "foreign/repository";
      },
    ],
    [
      "actual PR branch",
      (f) => {
        f.peerPublication.pull.headRef = "provider-owned";
      },
    ],
    [
      "actual PR merge target",
      (f) => {
        f.peerPublication.pull.mergeCommitSha = sha("other-merge");
      },
    ],
    [
      "unmerged PR",
      (f) => {
        f.peerPublication.pull.merged = false;
      },
    ],
    [
      "squash parent",
      (f) => {
        f.commits.get(f.peerPublication.merge)!.parentOids = [sha("foreign-base")];
      },
    ],
    [
      "two-parent merge instead of squash",
      (f) => {
        f.commits.get(f.peerPublication.merge)!.parentOids.push(sha("second-parent"));
      },
    ],
    [
      "squash tree",
      (f) => {
        f.commits.get(f.peerPublication.merge)!.treeOid = sha("foreign-tree");
      },
    ],
    [
      "failed independent validation",
      (f) => {
        Object.assign(f.peerPublication.validationEvent, { passed: false });
      },
    ],
    [
      "validation evidence identity",
      (f) => {
        Object.assign(f.peerPublication.validationEvent, {
          evidenceDigest: digest("other-validation"),
        });
      },
    ],
    [
      "missing original semantic acceptance",
      (f) => {
        f.refs.delete(f.peerPublication.review.ref);
      },
    ],
    [
      "conflicting known review usage",
      (f) => {
        const usage = f.peerPublication.item.factoryEvents!.find(
          (event) => event.kind === "budget" && event.phase === "management",
        )!;
        Object.assign(usage, { amount: 14 });
      },
    ],
    [
      "missing original reservation",
      (f) => {
        f.refs.delete(f.peerPublication.reservationRef);
      },
    ],
    [
      "reservation immutable parent",
      (f) => {
        f.commits.get(f.peerPublication.reservationOid)!.parentOids = [sha("foreign-base")];
      },
    ],
  ];
  it.each(invalid)(
    "refuses %s provenance rather than trusting an exact-commit discovery hint",
    async (_name, mutate) => {
      const f = await fixture();
      mutate(f);
  ...
  it("refuses more than 100 discovery candidates before reading their histories", async () => {
    const f = await fixture();
    vi.mocked(f.store.readCommitObjectiveCandidates!).mockResolvedValue(
      Array.from({ length: 101 }, (_, index) => 1000 + index),
    );
    await expect(f.peerProof()).rejects.toThrow();
    expect(
      vi.mocked(f.store.readObjectiveSnapshot!).mock.calls.every(([number]) => number === 7),
    ).toBe(true);
  });
  ...
  it("refuses cyclic or exhausted recursive proof traversal", async () => {
    const f = await fixture();
    await expect(
      f.peerProof({ proofTraversal: new Set([`peer:7:receiver-run:${f.peerPublication.merge}`]) }),
    ).rejects.toThrow();
    await expect(
      f.peerProof({
        proofTraversal: new Set(Array.from({ length: 100 }, (_, index) => `visited-${index}`)),
      }),
    ).rejects.toThrow();
  });

  it.each(["prior-head", "target-base", "combined-tree"] as const)(
    "refuses altered refreshed %s despite authentic peer ancestry",
    async (field) => {
      const f = await fixture();
  ...
  it.each([false, true])(
    "uses the exact candidate capacity admission horizon, not a later terminal receipt (late peer: %s)",
    async (latePeer) => {
      const f = await fixture();
      ...
      f.original.item.factoryEvents!.push(
        parseFactoryEvent({
          protocol: "clockgrove.factory/v2",
          kind: "capacity",
      ...
      f.receiver.snapshot.factoryEvents!.push(
        parseFactoryEvent({
          ...
          reason: "retained work requires explicit recovery",
        }),
      );
      if (latePeer) {
        Object.assign(peerEvent(f, "AttemptIntegrated"), { at: "2026-09-07T00:00:01.000Z" });
