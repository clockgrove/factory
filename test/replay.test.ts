...
describe("pure admission replay", () => {
  it.each([
    ...
    const snapshot = pinAdmissionSnapshot({
      ...input,
      resource: { ...input.resource!, ...observed },
    });
  ...
  it.each([
    ...
    expect(() =>
      pinAdmissionSnapshot({ ...input, resource: { ...input.resource!, ...invalid } }),
    ).toThrow();
  });
  ...
  it("binds prior queue observations and reason transitions without changing legacy snapshots", () => {
      ...
      workItems: [
        {
          ...item(101),
          queuedSince: waiting,
          previousQueueObservation: { code: "local-pressure", gate: "capacity" },
        },
      ],
    };
    const snapshot = pinAdmissionSnapshot(pinnedInput);
    expect(snapshot.input.workItems[0]?.previousQueueObservation).toEqual({
      code: "local-pressure",
      gate: "capacity",
    });
    expect(replayAdmissions(snapshot)).toMatchObject({
      reproduced: true,
      decisions: {
        queued: [
          {
    ...
    tampered.input.workItems[0]!.previousQueueObservation = {
      code: "lease-unavailable",
      gate: "authority",
    };
    ...
    expect(replayAdmissions(unchanged).decisions.queued[0]).not.toHaveProperty(
      "recordQueueReasonChange",
    );
    const legacy = pinAdmissionSnapshot({
      ...input,
      leaseValid: false,
      workItems: [{ ...item(101), queuedSince: waiting }],
    });
