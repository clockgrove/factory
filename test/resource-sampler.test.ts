...
describe("Linux/WSL resource sampler", () => {
  it("recognizes v1 and v2 unlimited CPU and memory sentinels", () => {
    ...
    expect(
      parseCgroupBytes("9223372036854771712", "memory.limit_in_bytes", false, true),
    ).toBeNull();
  ...
  it.each([
    ...
    "preserves host ceilings and zero capacity for limit=$limit/current=$current",
    async (entry) => {
      const reads: string[] = [];
      const source = files({
        "/proc/self/cgroup": "0::/\n",
  ...
  it.each([
    ...
    {
      "/proc/self/cgroup": "0::/parent/child",
      "/sys/fs/cgroup/parent/child/cpu.max": "100000 100000",
    },
    { "/proc/self/cgroup": "2:cpu:/job", "/sys/fs/cgroup/cpu/job/cpu.cfs_quota_us": "-1" },
    {
      ...
      "/sys/fs/cgroup/cpu/job/cpu.cfs_period_us": "broken",
    },
    {
      "/proc/self/cgroup": "3:memory:/parent/child",
      "/sys/fs/cgroup/memory/parent/child/memory.limit_in_bytes": "1",
      "/sys/fs/cgroup/memory/parent/child/memory.usage_in_bytes": "0",
      "/sys/fs/cgroup/memory/parent/memory.limit_in_bytes": "1",
      "/sys/fs/cgroup/memory/parent/memory.usage_in_bytes": "0",
    },
    {
      "/proc/self/cgroup": "3:memory:/job",
      "/sys/fs/cgroup/memory/job/memory.limit_in_bytes": "1",
      "/sys/fs/cgroup/memory/job/memory.usage_in_bytes": "0",
      "/sys/fs/cgroup/memory/job/memory.use_hierarchy": "2",
    },
  ])("rejects malformed or incomplete constrained ancestry", async (entry) => {
    const sampler = new LinuxResourceSampler({
      files: files(entry as Record<string, string>),
      os: host(),
    });
  ...
  it("bounds ancestry and rejects traversal, duplicate controllers and malformed memberships", () => {
    ...
    for (const value of [
      ...
      "0::/gone (deleted)",
      `0::/${"a".repeat(4096)}`,
      "x".repeat(MAX_CGROUP_MEMBERSHIP_BYTES + 1),
    ]) {
  ...
  it("distinguishes a missing kernel file from denied observation", async () => {
    ...
    await expect(new LinuxResourceSampler({ os: host() }).sample()).resolves.toMatchObject({
      source: "host",
    });
  ...
  it("caps bytes actually read and closes the read-only descriptor on overflow", async () => {
    ...
    await expect(new LinuxResourceSampler({ os: host() }).sample()).rejects.toThrow(
      "observation bounds",
    );
    ...
    expect(
      flags & (constants.O_WRONLY | constants.O_RDWR | constants.O_CREAT | constants.O_TRUNC),
    ).toBe(0);
  });
