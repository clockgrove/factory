...
const at = (seconds: number) =>
  new Date(Date.parse("2026-09-06T12:00:00.000Z") + seconds * 1000).toISOString();
...
function concurrent(): FactoryEvent[] {
  return [
    start(),
    attempt("AttemptReserved", 2, 1),
    attempt("AttemptStarted", 3, 2),
    attempt("AttemptReserved", 4, 3, 9),
    attempt("AttemptStarted", 5, 4, 9),
    attempt("AttemptSucceeded", 6, 6),
    capacity("CapacityReserved", 7, 7),
    attempt("AttemptCollected", 8, 8, 8, { artifactDigest: artifact }),
    attempt("AttemptSucceeded", 9, 9, 9),
    capacity("CapacityReserved", 10, 10, 9),
    attempt("AttemptCollected", 11, 11, 9, { artifactDigest: artifact }),
    validation(12, 12),
    capacity("CapacityReconciled", 13, 13),
    validation(14, 14, 9),
    capacity("CapacityReconciled", 15, 15, 9),
    attempt("AttemptIntegrated", 16, 16),
    attempt("AttemptIntegrated", 17, 17, 9),
    budget(18, 8, 100),
    budget(19, 9, 200),
    terminal(),
  ];
...
describe("durable runtime economics", () => {
  it("keeps conservative recovered reservation charges separate from measured time", () => {
    const bound = budget(21, 8, 60000, {
      unit: "local_milliseconds",
      usageId: "worker-8-1",
      usageEvidence: "conservative-reservation",
      directorEpoch: 1,
      policyDigest: policy,
      reason: "Original reserved charge after exact absence; elapsed usage unavailable",
    });
    ...
    expect(
      economics.nativeUnits.find((value) => value.unit === "local_milliseconds"),
    ).toMatchObject({ reconciled: 64000, conservativeReconciled: 60000 });
    ...
    expect(summarize(history).consumptionByDeliveryOutcome).toMatchObject({
  ...
  it.each(["local_milliseconds", "sandbox_milliseconds", "validation_milliseconds"])(
    ...
    (unit) => {
      const overrides = {
        ...
        directorEpoch: 1,
        policyDigest: policy,
        reason: "Exact resources absent; original reserved timed budget charge",
      };
  ...
  it("separates active worker time from all admitted execution/validation occupancy", () => {
    ...
    expect(result.execution.startedAttempts).toMatchObject({
      availability: "observed",
      value: { local: 2, cloud: 0, unclassified: 0, total: 2 },
    });
    expect(result.execution.closedIntervalTime).toMatchObject({
      availability: "observed",
      value: {
        summedWorkerMilliseconds: 9000,
        wallMillisecondsWithAnyWorker: 7000,
      },
    });
    ...
    expect(result.admittedLocalConcurrency).toMatchObject({
      availability: "observed",
      value: {
        peak: 2,
        average: 1.2,
        localReservationMilliseconds: 24000,
        denominatorMilliseconds: 20000,
        windowStart: at(0),
        windowEnd: at(20),
        intervals: 4,
      },
  ...
  it("reconstructs duplicate transport observations and a fresh restart without doubling work", () => {
    ...
    const duplicate = history.filter(
      (value) => value.kind === "attempt" || value.kind === "capacity",
    );
    expect(summarize([...history, ...duplicate].reverse())).toEqual(summarize(history));
    expect(summarize(JSON.parse(JSON.stringify(history)) as FactoryEvent[])).toEqual(
      summarize(history),
    );
  ...
  it("does not turn an empty or open history into zero measured activity", () => {
    ...
    const open = summarizeRuntimeEconomics(
      concurrent().filter((value) => value.event !== "FactoryRunCompleted"),
      "runtime-7",
      at(0),
    );
    ...
    expect(summarize(concurrent(), at(0)).admittedLocalConcurrency.availability).toBe(
      "unavailable",
    );
  });
      ...
      concurrent().filter(
        (value) =>
          !(
            "workItem" in value &&
            value.workItem === 9 &&
            ["AttemptStarted", "CapacityReconciled"].includes(value.event)
          ),
      ),
    );
  ...
  it.each(["policyDigest", "directorEpoch", "requestedCpu"])(
    "does not close capacity using a mismatched %s",
    (field) => {
      const result = summarize(
        concurrent().map((value) =>
          value.kind === "capacity" && value.event === "CapacityReconciled" && value.workItem === 8
            ? parseFactoryEvent({
                ...value,
                [field]: field === "policyDigest" ? "e".repeat(64) : 2,
              })
            : value,
        ),
      );
  ...
  it("counts sequential candidate validators independently and leaves ambiguous overlapping reservations unresolved", () => {
    ...
    again.push(
      capacity("CapacityReserved", 21, 21),
      capacity("CapacityReconciled", 22, 23),
      terminal("FactoryRunCompleted", 24, 24),
    );
    ...
    expect(result.admittedLocalConcurrency).toMatchObject({
      value: { localReservationMilliseconds: 26000 },
    });
  ...
  it("does not classify unknown backend identities as local or use unsupported execution capacity", () => {
    const unknown = summarize(
      concurrent().map((value) =>
        value.kind === "attempt" && value.workItem === 8
          ? parseFactoryEvent({ ...value, backend: "future/worker" })
          : value,
      ),
    );
    expect(unknown.execution.startedAttempts).toMatchObject({
      value: { local: 1, unclassified: 1, total: 2 },
    });
    expect(unknown.admittedLocalConcurrency.availability).toBe("unavailable");
    const unsupported = summarize([
      ...concurrent(),
      capacity("CapacityReserved", 21, 21, 10, { phase: "execution" }),
    ]);
  ...
  it("classifies actual cloud execution and local independent validation separately", () => {
    const mixed = summarize(
      concurrent().map((value) =>
        value.kind === "attempt" && value.workItem === 8
          ? parseFactoryEvent({ ...value, backend: "codex-cli/daytona" })
          : value,
      ),
    );
    expect(mixed.execution.startedAttempts).toMatchObject({
      value: { local: 1, cloud: 1, total: 2 },
    });
    expect(mixed.validation.admittedInvocations).toMatchObject({
      value: { local: 2, cloud: 0, total: 2 },
    });
    expect(mixed.admittedLocalConcurrency).toMatchObject({
      value: { localReservationMilliseconds: 18000, average: 0.9 },
    });
  });
  ...
  it("refuses negative timestamps and conflicting repeated starts instead of inventing durations", () => {
    const backwards = summarize(
      concurrent().map((value) =>
        value.event === "AttemptSucceeded" && value.workItem === 8
          ? parseFactoryEvent({ ...value, at: at(1) })
          : value,
      ),
    );
    expect(backwards.execution.completeIntervals).toBe(1);
    expect(backwards.execution.unresolvedOrConflictingAttempts).toBe(1);
    const conflict = summarize([
      ...concurrent(),
      attempt("AttemptStarted", 21, 2, 8, { providerResourceId: "different-process" }),
    ]);
  ...
  it("reports retry launches, not maximum attempt numbers or deferrals as new worker starts", () => {
    const values = [
      ...
      attempt("AttemptReserved", 2, 1),
      attempt("AttemptStarted", 3, 2),
      attempt("AttemptFailed", 4, 3),
      attempt("AttemptReserved", 5, 4, 8, { attempt: 2 }),
      attempt("AttemptStarted", 6, 5, 8, { attempt: 2 }),
      attempt("AttemptFailed", 7, 6, 8, { attempt: 2 }),
      attempt("AttemptDeferred", 8, 7, 9),
      attempt("AttemptStarted", 9, 8, 10, { attempt: 4 }),
      terminal("FactoryRunEscalated"),
    ];
    const result = summarize(values);
    expect(result.retries).toMatchObject({
      value: {
        observedStartsAfterFailedAttempt: 1,
        otherAdditionalStarts: 0,
        startsWithMissingEarlierAttemptEvidence: 1,
      },
    });
  ...
  it("partitions known native consumption by exact run/attempt delivery without zero filling missing usage", () => {
    ...
    values.push(
      ...
      budget(23, 100, 7),
      budget(24, 9, 1000, { event: "BudgetReserved", usageId: "unobserved" }),
      budget(25, 8, 9999, { phase: "management", usageId: "compiler" }),
    );
    const result = summarize(values);
    expect(result.consumptionByDeliveryOutcome).toMatchObject({
      value: [
        {
          unit: "model_tokens",
          integrated: 150,
          unintegratedAtTerminalRun: 200,
    ...
    const open = summarizeRuntimeEconomics(
      values.filter((value) => value.event !== "FactoryRunCompleted"),
      "runtime-7",
      at(0),
    );
    expect(open.consumptionByDeliveryOutcome).toMatchObject({
      value: [
        { unit: "model_tokens", unresolvedDelivery: 200, unintegratedAtTerminalRun: 0 },
        { unit: "validation_milliseconds", unresolvedDelivery: 3 },
      ],
    });
  });
  ...
  it("records known zero usage, rejects arithmetic overflow, and never adds attempt token copies", () => {
    ...
    expect(zero.consumptionByDeliveryOutcome).toMatchObject({
      value: [{ integrated: 0, usageIdentities: 1 }],
    });
    expect(
      summarize([...values, budget(21, 8, Number.MAX_VALUE), budget(22, 9, Number.MAX_VALUE)])
        .consumptionByDeliveryOutcome.availability,
    ).toBe("unavailable");
    expect(
      summarize(
        concurrent().map((value) =>
          value.event === "AttemptSucceeded"
            ? parseFactoryEvent({ ...value, reportedModelTokens: 500 })
            : value,
        ),
  ...
  it("binds successor candidate validation without inventing a successor worker or importing predecessor cost", () => {
    ...
    const source = {
      ...
      targetBaseSha: base,
      backend: `factory/integration-validation-${candidate}`,
    };
    const outcome = event(5, 5, {
      kind: "recovery",
      event: "RecoverySourceIntegrated",
      recoveryRequestId: "request-1",
      planDigest: artifact,
      claimRef: "refs/factory/recovery/claim",
      claimOid: base,
      workItem: 8,
      sourceRunId: "original-run",
      sourceAttempt: 1,
      sourceReservationRef: "refs/factory/attempt",
      sourceReservationCommitOid: base,
      sourceReservationReceiptDigest: artifact,
      sourcePublicationReceiptDigest: artifact,
      sourceHeadSha: base,
      mergeCommitSha: "e".repeat(40),
      mergeCandidateIdentityDigest: candidate,
    });
    const usage = event(4, 4, {
      kind: "budget",
      event: "BudgetReconciled",
      workItem: 8,
      phase: "validation",
      unit: "validation_milliseconds",
      amount: 1500,
      usageId: `integration-validation-${candidate}`,
    });
    const values = [
      start(),
      capacity("CapacityReserved", 2, 1, 8, source),
      capacity("CapacityReconciled", 3, 3, 8, source),
      usage,
      outcome,
      terminal(),
    ];
    ...
    expect(result.admittedLocalConcurrency).toMatchObject({
      value: { peak: 1, average: 0.1, intervals: 1 },
    });
    expect(result.consumptionByDeliveryOutcome).toMatchObject({
      value: [{ unit: "validation_milliseconds", integrated: 1500, unattributed: 0 }],
    });
    const wrongCandidate = summarize(
      values.map((value) =>
        value.event === "RecoverySourceIntegrated"
          ? parseFactoryEvent({ ...value, mergeCandidateIdentityDigest: "e".repeat(64) })
          : value,
      ),
    );
    expect(wrongCandidate.consumptionByDeliveryOutcome).toMatchObject({
      value: [{ integrated: 0, unintegratedAtTerminalRun: 1500 }],
    });
    const unbound = summarize(values.filter((value) => value.kind !== "capacity"));
    expect(unbound.consumptionByDeliveryOutcome).toMatchObject({
      value: [{ integrated: 0, unattributed: 1500 }],
    });
  });

  it("distinguishes rejected artifacts from deletion and never claims generated output as authored yield", () => {
    const values = concurrent().map((value) =>
      value.kind === "validation" ? parseFactoryEvent({ ...value, passed: false }) : value,
    );
    const result = summarize([...values, validation(21, 18, 8, false)]);
    expect(result.rejectedValidationArtifacts).toMatchObject({
      value: { distinctArtifacts: 2, failedResults: 2, failedResultsWithoutArtifactBinding: 0 },
    });
    ...
    expect(sparse.rejectedValidationArtifacts).toMatchObject({
      value: { distinctArtifacts: 0, failedResults: 2, failedResultsWithoutArtifactBinding: 2 },
    });
  });
  ...
  it("reports evidenced intervention codes without echoing free-form reasons or claiming human effort", () => {
    ...
    const values = [
      ...
      event(4, 3, {
        ...
        reason: `budget-exhausted: ${secret}`,
      }),
    ];
    const result = summarize(values);
    expect(result.interventions).toMatchObject({
      value: {
        operatorRequests: 2,
        escalationBoundaries: 1,
        entries: [
          { reason: { availability: "observed", code: null } },
          { workItem: 8, reason: { availability: "unavailable" } },
