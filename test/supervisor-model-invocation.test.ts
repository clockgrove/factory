...
import { reviewCheckpointRef, reviewIdentityDigest, type ReviewIdentity } from "../src/control/reviews.js";
...
const modelBudgets = (f: Fixture): BudgetEvent[] => f.events().filter(
...
describe("Supervisor model dispatch journal", () => {
  it("persists exact worker and semantic-review intent before each model call and links actual counters", async () => {
    ...
    const f = await providerSupervisorFixture("daytona-burst", {
      ...
      configureLocalBackend: (backend) => ({
        ...
        launch: async (context) => {
          ...
          const markers = modelBudgets(fixture).filter((event) =>
            isModelInvocationMarker(event) && event.modelInvocationId === invocationId,
          );
          expect(markers).toHaveLength(1);
          expect(markers[0]).toMatchObject({
            objective: context.objective, runId: context.runId,
            workItem: context.workItem, attempt: context.attempt, phase: "execution",
            amount: 0, usageId: `invocation-${invocationId}`,
            policyDigest: context.policyDigest, directorEpoch: context.directorEpoch,
          });
    ...
    f.management.review = async (context, checkpoint) => {
      const reservation = f.events().find((event) =>
      ...
      const identity: ReviewIdentity = {
        kind: "artifact", runId: f.runId, objective: context.objectiveNumber,
        workItem: context.workItemNumber, attempt: reservation.attempt,
        artifactDigest: context.artifact.digest, baseSha: context.evidence.baseSha,
        outputTreeSha: context.evidence.outputTreeSha, evidenceDigest: context.evidence.digest,
      };
      const invocationId = `review-${reviewIdentityDigest(identity)}`;
      const markers = modelBudgets(f).filter((event) =>
        isModelInvocationMarker(event) && event.modelInvocationId === invocationId,
      );
      expect(markers).toHaveLength(1);
      expect(markers[0]).toMatchObject({
        runId: f.runId, objective: context.objectiveNumber, workItem: context.workItemNumber,
        attempt: reservation.attempt, phase: "management", amount: 0,
        policyDigest: reservation.policyDigest, directorEpoch: reservation.directorEpoch,
      });
    ...
    try {
      ...
      for (const marker of markers) {
        const actual = budgets.filter((event) => event.event === "BudgetReconciled" &&
          event.modelInvocationId === marker.modelInvocationId && event.phase === marker.phase &&
          event.workItem === marker.workItem && event.attempt === marker.attempt,
        );
        expect(actual).toHaveLength(1);
        expect(actual[0]).toMatchObject({
          amount: 6, runId: marker.runId, objective: marker.objective,
          policyDigest: marker.policyDigest, directorEpoch: marker.directorEpoch,
  ...
  it("retains unknown worker dispatch across restart without another launch or invented usage", async () => {
    ...
    const f = await providerSupervisorFixture("daytona-burst", {
      ...
      configureLocalBackend: (backend) => ({
        ...backend,
        observe: async () => { throw new Error("fixture: terminal counters and output unavailable"); },
      }),
    });
    ...
    try {
      ...
      expect(markers).toHaveLength(1);
      expect(markers[0]).toMatchObject({
        modelInvocationId: "worker-8-1", phase: "execution", amount: 0,
      });
      ...
      expect(f.events().filter((event) =>
        ["FactoryRunCompleted", "FactoryRunCancelled", "FactoryRunEscalated", "AttemptFailed", "AttemptDeferred"].includes(event.event),
      )).toEqual([]);
  ...
  it("repairs an accepted immutable review's missing linked usage after restart without repeating that model call", async () => {
    ...
    vi.mocked(GitHubControlStore.prototype.addIssueComment).mockImplementation(async (node, body) => {
      const receipts = decodeEventComments(body);
      if (held && receipts.some((event) => event.kind === "budget" &&
        event.event === "BudgetReconciled" && event.unit === "model_tokens" &&
        event.phase === "management" && event.workItem === 8,
      )) {
    ...
    f.management.review = async (context, checkpoint) => {
      if (context.workItemNumber === 8) {
        checkpointRef = reviewCheckpointRef({
          kind: "artifact", runId: f.runId, objective: 7, workItem: 8, attempt: 1,
          artifactDigest: context.artifact.digest, baseSha: context.evidence.baseSha,
          outputTreeSha: context.evidence.outputTreeSha, evidenceDigest: context.evidence.digest,
        });
      }
    ...
    try {
      ...
      expect(f.activity.filter((entry) => entry.operation === "review" && entry.workItem === 8)).toHaveLength(1);
      expect(modelBudgets(f).filter((event) => event.event === "BudgetReconciled" && event.workItem === 8 && event.phase === "management")).toEqual([]);
      held = false;
      expect(await f.run()).toMatchObject({ status: "completed" });
      ...
      const actual = modelBudgets(f).filter((event) => event.event === "BudgetReconciled" &&
        event.modelInvocationId === pending[0]!.modelInvocationId,
      );
      expect(actual).toHaveLength(1);
      expect(actual[0]).toMatchObject({
        amount: 6, workItem: 8, attempt: 1, phase: "management",
        directorEpoch: pending[0]!.directorEpoch, policyDigest: pending[0]!.policyDigest,
      });
