...
describe("Supervisor durable queue reason transitions", () => {
  it("journals only changed gates across a same-run restart without resetting waiting age or attempts", async () => {
    ...
    const queued = vi
      .spyOn(AttemptManager.prototype, "recordQueued")
      .mockImplementation(async function (this: AttemptManager, args) {
        ...
        if (
          args.workItem === 9 &&
          args.reasonCode === (restarted ? "local-cooldown" : "local-pressure")
        )
          shutdown.abort();
        return receipt;
      });
    ...
    try {
      ...
      const original = queuedState(
        derive(f.snapshot).items.find((item) => item.number === 8)!,
        f.runId,
      )!;
      ...
      for (const number of [8, 9]) {
        ...
        expect(writes.map(([args]) => args.reasonCode)).toEqual([
          "local-capacity",
          "local-pressure",
          "local-cooldown",
        ]);
