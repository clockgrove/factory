...
import { capacityReservationKey, type CapacityReservation } from "../src/scheduling/capacity-ledger.js";
...
function retainPeer(f: Fixture, phase: "execution" | "validation" = "execution", paths = ["peer/"]) {
  const identity = { objective: 121, workItem: 123, attempt: 1, phase, backendId: phase === "execution" ? LOCAL : "factory/local-validation" };
  const peer: CapacityReservation = {
    ...identity, key: capacityReservationKey(identity), admissionClass: "local", local: true,
    cpu: 1, memoryMb: 512, paidUnits: 0, paths, exclusiveResources: [],
  };
...
it.each(["execution", "validation"] as const)(
  ...
  async (phase) => {
    ...
    try {
      ...
      expect(f.activity.filter((entry) => entry.operation === "launch").map((entry) => entry.workItem)).toEqual([8]);
      ...
      expect(f.events().some((event) => event.kind === "scheduling" && event.workItem === 9 && event.reasonCode === "global-capacity")).toBe(true);
    } finally {
      shutdown.abort();
      release.resolve();
      await running.catch(() => {});
...
it.each([
  ...
  ["paths", "path-conflict"],
] as const)("retains the %s admission fence across Objectives", async (constraint, reason) => {
  const shutdown = new AbortController();
  const f = await providerSupervisorFixture("daytona-burst", { localOnly: true, controllerActivation: true });
  const peer = retainPeer(f, "execution", constraint === "paths" ? ["a.txt", "b.txt"] : ["peer/"]);
  ...
  try {
    await vi.waitFor(() => {
      const queued = f.events().filter((event) => event.kind === "scheduling" &&
        event.reasonCode === (constraint === "paths" ? "path-conflict" : "local-capacity") &&
