...
it("every fresh ordinary Supervisor review inspects the exact candidate rather than the controller", async () => {
  const f = await providerSupervisorFixture("daytona-burst", { localOnly: true, dependencyChain: true });
  fixtures.push(f);
  ...
  vi.spyOn(f.management, "review").mockImplementation(async (context, checkpoint) => {
    ...
    expect(execFileSync("git", ["write-tree"], { cwd: context.repository, encoding: "utf8" }).trim()).toBe(context.evidence.outputTreeSha);
